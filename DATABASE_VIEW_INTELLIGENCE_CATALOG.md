...
11. [**Election Cycle Views (v1.51)** ⭐ NEW](#election-cycle-views-v151)
12. [Application & Audit Views](#application--audit-views)
13. [Common Usage Patterns](#common-usage-patterns)
14. [View Dependency Diagram](#view-dependency-diagram)
15. [Performance Optimization Guide](#performance-optimization-guide)
16. [Cross-Reference to Intelligence Frameworks](#cross-reference-to-intelligence-frameworks)
...
The Citizen Intelligence Agency (CIA) platform employs **92 database views** (64 regular views + 28 materialized views) across 10 major categories to support comprehensive political intelligence analysis, open-source intelligence (OSINT) collection, and democratic accountability monitoring.

✅ **Documentation Status**: This catalog now provides **comprehensive documentation** for all 92 database views (100% coverage). **11 views** have detailed examples with complex queries, while **81 views** have structured documentation with purpose, key metrics, sample queries, and intelligence applications. All views are now documented and discoverable.
...
**Documentation Coverage**: 100% (92/92 views)  
**Validation Details**: See [Validation History](#-validation-history) section below

**Note**: Total view count increased from 84 to 92 views with the addition of 8 new Election Cycle Views in v1.51 (2026-01-13).

### Key Statistics (UPDATED 2026-01-13)

| Metric | Count | Description |
|--------|-------|-------------|
| **Total Views** | 92 | ✅ UPDATED: +8 Election Cycle Views (v1.51) |
| **Regular Views** | 64 | ✅ UPDATED: +8 standard SQL views |
...
| **Views Documented (Structured)** | 81 | ✅ UPDATED: Purpose, metrics, queries, product mappings |
| **Documentation Coverage** | 100% | All 92 views documented |
| **Intelligence Views** | 15 | ✅ UPDATED: 7 original + 8 election cycle views |
| **Election Cycle Views** | 8 | ⭐ NEW in v1.51: Historical trend analysis (2002-2026) |
...
## Election Cycle Views (v1.51)

### Overview

Election Cycle Views (v1.51) provide comprehensive longitudinal analysis across 7 Swedish election cycles spanning 24 years (2002-2026). These views leverage advanced PostgreSQL window functions, statistical aggregation, and temporal analysis to enable historical comparative analysis, trend forecasting, and electoral behavior tracking.

**Total Election Cycle Views:** 8  
**Intelligence Value:** ⭐⭐⭐⭐⭐ VERY HIGH  
**Primary Use Cases:** Historical trend analysis, election forecasting, politician career tracking, party momentum analysis, coalition prediction, behavioral volatility assessment

### Advanced PostgreSQL Features

These views showcase enterprise-grade database capabilities:

| Feature Category | Functions Used | Purpose |
|-----------------|----------------|---------|
| **Window Functions** | RANK(), DENSE_RANK(), PERCENT_RANK(), NTILE(4), LAG(), LEAD() | Rankings, percentiles, quartiles, period comparisons |
| **Statistical Functions** | STDDEV(), VARIANCE(), AVG() with frames, Z-scores, Coefficient of Variation | Volatility, moving averages, anomaly detection |
| **Temporal Analysis** | FIRST_VALUE(), LAST_VALUE(), date_trunc(), cumulative sums | Baseline comparisons, historical context |
| **Analytical Patterns** | Velocity/acceleration, cohort analysis, performance scoring | Momentum tracking, experience grouping, composite metrics |

### Election Cycle Definitions

```sql
CASE 
  WHEN EXTRACT(YEAR FROM date_column) BETWEEN 2002 AND 2005 THEN 2002
  WHEN EXTRACT(YEAR FROM date_column) BETWEEN 2006 AND 2009 THEN 2006
  WHEN EXTRACT(YEAR FROM date_column) BETWEEN 2010 AND 2013 THEN 2010
  WHEN EXTRACT(YEAR FROM date_column) BETWEEN 2014 AND 2017 THEN 2014
  WHEN EXTRACT(YEAR FROM date_column) BETWEEN 2018 AND 2021 THEN 2018
  WHEN EXTRACT(YEAR FROM date_column) BETWEEN 2022 AND 2025 THEN 2022
  ELSE 2026
...
| View Type | Views | Purpose |
|-----------|-------|---------|
| **Summary Analysis** | election_cycle_summary, comparative_analysis | Overall cycle metrics, historical context |
| **Individual Performance** | politician_election_cycle_performance, politician_rankings | MP tracking, performance tiers |
| **Party Evolution** | party_election_cycle_trends, party_momentum | Party trajectories, momentum analysis |
| **Committee Activity** | committee_election_cycle_activity | Legislative productivity |
| **Volatility Assessment** | volatility_analysis | Behavioral stability, risk metrics |
...
**Type:** Standard View  
**Intelligence Value:** HIGH - Historical Overview  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Provides comprehensive summary metrics for each of the 7 election cycles (2002-2026), including voting behavior, participation statistics, demographic trends, and cycle-to-cycle comparisons using LAG() window functions. Enables longitudinal analysis of Swedish democracy over 24 years.

#### Key Columns

| Column | Type | Description | Example |
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle identifier | 2022 |
| `cycle_start_date` | DATE | Cycle start (post-election) | 2022-09-01 |
| `cycle_end_date` | DATE | Cycle end (pre-election) | 2026-08-31 |
| `total_ballots` | BIGINT | Total ballots voted during cycle | 12450 |
| `active_politicians` | BIGINT | Distinct politicians participating | 349 |
| `avg_yes_rate` | NUMERIC | Average yes vote percentage | 52.34 |
| `avg_absent_rate` | NUMERIC | Average absence percentage | 8.21 |
| `ballot_change` | NUMERIC | Change from previous cycle (LAG) | 1250 |
| `ballot_change_pct` | NUMERIC | Percentage change from previous cycle | 11.18 |
| `ballot_count_rank` | BIGINT | Ranking by ballot volume (RANK) | 3 |
| `cumulative_ballots` | NUMERIC | Running total across all cycles | 87,340 |

#### Intelligence Applications

- **Longitudinal Democracy Analysis**: Track evolution of parliamentary activity over 24 years
- **Participation Trends**: Identify cycles of high/low engagement
- **Historical Benchmarking**: Compare current cycle to historical averages
- **Cycle Ranking**: Identify most/least active legislative periods
- **Coalition Context**: Understand activity levels during different government formations

#### Sample Queries

```sql
-- Compare most recent cycle to baseline (2002)
SELECT 
    election_year,
    total_ballots,
    ballot_change_pct,
...
WHERE election_year IN (2002, 2026)
ORDER BY election_year;

-- Identify cycles with highest legislative activity
SELECT 
    election_year,
    total_ballots,
    ballot_count_rank,
...
**Category:** Election Cycle Views (v1.51)  
**Type:** Standard View  
**Intelligence Value:** VERY HIGH - Individual Career Tracking  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Tracks individual politician performance metrics across multiple election cycles, enabling career trajectory analysis, cohort comparisons, and performance trend identification. Uses PERCENT_RANK() for percentile calculations (0.0-1.0) and NTILE(4) for quartile groupings.

#### Key Columns

| Column | Type | Description | Example |
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle | 2022 |
| `person_id` | VARCHAR | Politician unique ID | 0123456789 |
| `first_name` | VARCHAR | First name | Anna |
| `last_name` | VARCHAR | Last name | Johansson |
| `party` | VARCHAR | Political party | S |
| `entry_cycle` | INTEGER | First election cycle as MP | 2014 |
| `cycles_since_entry` | INTEGER | Years in parliament | 8 |
| `attendance_score` | NUMERIC | Attendance percentage (100 - absent_rate) | 94.56 |
| `attendance_percentile` | NUMERIC | Percentile rank (0.0-1.0) | 0.8523 |
| `attendance_quartile` | INTEGER | Performance quartile (1-4) | 4 |
| `experience_level` | VARCHAR | Career stage classification | SENIOR_MP |
| `attendance_change` | NUMERIC | Change from previous cycle | 2.34 |

#### Experience Level Classifications

| Classification | Criteria | Description |
|---------------|----------|-------------|
| `NEW_MP` | entry_cycle = election_year | First term in parliament |
| `JUNIOR_MP` | cycles_since_entry ≤ 4 | 0-4 years experience |
| `SENIOR_MP` | 4 < cycles_since_entry ≤ 8 | 5-8 years experience |
| `VETERAN_MP` | cycles_since_entry > 8 | 9+ years experience |

#### Intelligence Applications

- **Politician Scorecards**: Multi-cycle performance summaries
- **Career Trajectory Analysis**: Track improvement/decline over time
- **Cohort Analysis**: Compare performance by entry year
- **Percentile Rankings**: Identify top/bottom performers (e.g., top 10% = percentile > 0.90)
- **Experience Impact**: Analyze correlation between tenure and performance

#### Sample Queries

```sql
-- Track specific politician across all cycles
SELECT 
    election_year,
...
WHERE person_id = '<politician_id>'
ORDER BY election_year;

-- Top 10% performers in most recent cycle
SELECT 
    CONCAT(first_name, ' ', last_name) as name,
    party,
    attendance_score,
...
WHERE election_year = 2026 
    AND attendance_percentile >= 0.90
ORDER BY attendance_percentile DESC;

-- Compare NEW_MP vs VETERAN_MP average performance
SELECT 
    experience_level,
    COUNT(*) as count,
    ROUND(AVG(attendance_score), 2) as avg_attendance,
    ROUND(AVG(ballot_participation), 0) as avg_participation
FROM view_riksdagen_politician_election_cycle_performance
WHERE election_year = 2026
...
**Category:** Election Cycle Views (v1.51)  
**Type:** Standard View  
**Intelligence Value:** VERY HIGH - Party Evolution Analysis  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Analyzes party evolution across 7 election cycles (2002-2026) using moving averages for trend smoothing, STDDEV() for volatility measurement, and momentum indicators for growth/decline classification. Essential for coalition forecasting and electoral analysis.
...
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle | 2022 |
| `party` | VARCHAR | Party abbreviation | S |
| `party_size` | BIGINT | Number of MPs | 107 |
| `party_market_share` | NUMERIC | Percentage of total MPs | 30.66 |
| `party_attendance_rate` | NUMERIC | Party average attendance | 92.34 |
| `size_moving_avg_3cycles` | NUMERIC | 3-cycle moving average | 105.33 |
| `size_volatility_3cycles` | NUMERIC | Standard deviation (3 cycles) | 4.23 |
| `size_change_pct` | NUMERIC | Growth/decline from previous cycle | -2.75 |
| `momentum` | VARCHAR | Growth classification | DECLINE |
| `party_status` | VARCHAR | Entry/exit status | ESTABLISHED |

#### Momentum Classifications

| Momentum | Criteria | Interpretation |
|----------|----------|----------------|
| `STRONG_GROWTH` | size_change > 5 MPs | Rapid expansion |
| `GROWTH` | 0 < size_change ≤ 5 | Moderate growth |
| `STABLE` | size_change = 0 | No change |
| `DECLINE` | -5 ≤ size_change < 0 | Moderate decline |
| `STRONG_DECLINE` | size_change < -5 | Rapid contraction |

#### Intelligence Applications

- **Coalition Forecasting**: Predict viable government formations
- **Electoral Trajectory**: Identify parties in growth/decline
- **Volatility Assessment**: Measure party stability over time
- **Market Share Trends**: Track relative party strength evolution
- **Historical Patterns**: Identify recurring electoral cycles

#### Sample Queries

```sql
-- Major parties trajectory over all cycles
SELECT 
    election_year,
    party,
    party_size,
...
WHERE party IN ('S', 'M', 'SD', 'C', 'V', 'MP', 'KD', 'L')
ORDER BY election_year DESC, party_size DESC;

-- Most volatile parties (unstable support)
SELECT 
    party,
    COUNT(DISTINCT election_year) as cycles_present,
    ROUND(AVG(size_volatility_3cycles), 2) as avg_volatility,
    MAX(party_size) - MIN(party_size) as size_range
FROM view_riksdagen_party_election_cycle_trends
GROUP BY party
HAVING COUNT(DISTINCT election_year) >= 3
ORDER BY avg_volatility DESC;

-- New party emergence and establishment
SELECT 
    party,
    MIN(election_year) as first_cycle,
    MAX(election_year) as last_cycle,
    MAX(party_market_share) as peak_market_share
FROM view_riksdagen_party_election_cycle_trends
GROUP BY party
ORDER BY first_cycle DESC;
```
...
**Category:** Election Cycle Views (v1.51)  
**Type:** Standard View  
**Intelligence Value:** HIGH - Legislative Productivity  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Measures committee legislative productivity across election cycles using NTILE(4) for performance tier classification and trend indicators (INCREASING/DECREASING/STABLE) for activity trajectory analysis.
...
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle | 2022 |
| `committee` | VARCHAR | Committee name | Finansutskottet |
| `documents_produced` | BIGINT | Total documents in cycle | 1,234 |
| `productivity_rank` | BIGINT | Ranking within cycle | 2 |
| `productivity_tier` | INTEGER | Performance quartile (1-4) | 4 |
| `activity_level` | VARCHAR | Classification | VERY_ACTIVE |
| `trend` | VARCHAR | Trajectory indicator | INCREASING |
| `document_change_pct` | NUMERIC | Change from previous cycle | 12.45 |

#### Activity Level Classifications

| Level | Tier | Description |
|-------|------|-------------|
| `VERY_ACTIVE` | 4 (top 25%) | Highest productivity quartile |
| `ACTIVE` | 3 | Above-average productivity |
| `MODERATE` | 2 | Below-average productivity |
| `LOW_ACTIVITY` | 1 (bottom 25%) | Lowest productivity quartile |

#### Intelligence Applications

- **Committee Effectiveness**: Identify most productive committees
- **Workload Distribution**: Analyze legislative burden across committees
- **Trend Detection**: Monitor productivity increases/decreases
- **Institutional Health**: Track overall legislative capacity over time

#### Sample Queries

```sql
-- Top performing committees in latest cycle
SELECT 
    committee,
    documents_produced,
    productivity_rank,
...
-- Committees with consistent high productivity
SELECT 
    committee,
    COUNT(*) as cycles_analyzed,
    ROUND(AVG(documents_produced), 0) as avg_documents,
    ROUND(AVG(CASE WHEN productivity_tier = 4 THEN 1 ELSE 0 END) * 100, 2) as pct_top_quartile
FROM view_riksdagen_committee_election_cycle_activity
GROUP BY committee
HAVING COUNT(*) >= 3
...
**Category:** Election Cycle Views (v1.51)  
**Type:** Standard View  
**Intelligence Value:** VERY HIGH - Historical Context  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Provides cross-cycle comparative analysis with baseline (2002) comparisons, historical averages, z-score anomaly detection, and cycle classifications. Essential for understanding current cycle in historical context.
...
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle | 2022 |
| `total_ballots` | BIGINT | Cycle ballot count | 12,450 |
| `baseline_ballots` | BIGINT | 2002 baseline | 10,230 |
| `ballots_vs_baseline_pct` | NUMERIC | % change from baseline | 21.70 |
| `historical_avg_ballots` | NUMERIC | Average of previous cycles | 11,340 |
| `pct_of_historical_avg_ballots` | NUMERIC | % of historical average | 109.78 |
| `ballots_z_score` | NUMERIC | Standard deviations from mean | 1.24 |
| `cycle_classification` | VARCHAR | Anomaly/average classification | ABOVE_AVERAGE |

#### Cycle Classifications

| Classification | Criteria | Interpretation |
|---------------|----------|----------------|
| `ANOMALOUS` | \|z_score\| > 2 | Statistical outlier (rare) |
| `ABOVE_AVERAGE` | Value > 110% of historical avg | Higher than typical |
| `BELOW_AVERAGE` | Value < 90% of historical avg | Lower than typical |
| `TYPICAL` | 90-110% of historical avg | Within normal range |

#### Intelligence Applications

- **Historical Benchmarking**: Compare any cycle to 2002 baseline
- **Anomaly Detection**: Identify statistically unusual cycles (z-score > 2)
- **Trend Context**: Understand current cycle relative to history
- **Forecasting**: Use historical patterns for future predictions

#### Sample Queries

```sql
-- Current cycle vs baseline and historical average
SELECT 
    election_year,
    total_ballots,
    baseline_ballots,
...
-- Identify anomalous cycles (outliers)
SELECT 
    election_year,
    total_ballots,
    ballots_z_score,
    cycle_classification
FROM view_riksdagen_election_cycle_comparative_analysis
WHERE ABS(ballots_z_score) > 2
ORDER BY ABS(ballots_z_score) DESC;
```
...
**Category:** Election Cycle Views (v1.51)  
**Type:** Standard View  
**Intelligence Value:** VERY HIGH - Performance Leaderboards  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Creates comprehensive politician performance rankings with Hall of Fame/Shame identification (top/bottom 5%), party MVP designation, and consistency ratings. Critical for public accountability and politician scorecards.
...
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle | 2022 |
| `person_id` | VARCHAR | Politician ID | 0123456789 |
| `overall_rank` | BIGINT | Overall ranking in cycle | 12 |
| `party_rank` | BIGINT | Ranking within party | 2 |
| `performance_tier` | VARCHAR | Top 5%/bottom 5% classification | HALL_OF_FAME |
| `special_recognition` | VARCHAR | Party MVP designation | PARTY_MVP |
| `consistency_rating` | VARCHAR | Volatility classification | VERY_CONSISTENT |
| `longevity_tier` | VARCHAR | Career stage | VETERAN |

#### Performance Tiers

| Tier | Criteria | Description |
|------|----------|-------------|
| `HALL_OF_FAME` | Percentile ≤ 0.05 (top 5%) | Elite performers |
| `EXCELLENT` | Percentile ≤ 0.25 (top 25%) | Strong performers |
| `AVERAGE` | 0.25 < percentile < 0.75 | Typical performance |
| `BELOW_AVERAGE` | Percentile ≥ 0.75 (bottom 25%) | Underperformers |
| `NEEDS_IMPROVEMENT` | Percentile ≥ 0.95 (bottom 5%) | Critical concerns |

#### Intelligence Applications

- **Public Accountability**: Identify best/worst performers
- **Party Internal Competition**: Rank within party for leadership selection
- **Media Watchdog**: Highlight politicians needing scrutiny
- **Voter Information**: Provide performance comparisons
- **Party MVP Recognition**: Identify top contributor per party

#### Sample Queries

```sql
-- Hall of Fame (top 5%) in latest cycle
SELECT 
    CONCAT(first_name, ' ', last_name) as name,
    party,
    performance_score,
...
WHERE election_year = 2026 
    AND performance_tier = 'HALL_OF_FAME'
ORDER BY overall_rank;

-- Party MVPs across all cycles
SELECT 
    election_year,
    party,
    CONCAT(first_name, ' ', last_name) as name,
    performance_score
FROM view_riksdagen_election_cycle_politician_rankings
WHERE special_recognition = 'PARTY_MVP'
ORDER BY election_year DESC, party;
```
...
**Category:** Election Cycle Views (v1.51)  
**Type:** Standard View  
**Intelligence Value:** VERY HIGH - Electoral Forecasting  
**Changelog:** v1.51 Election Cycle Trend Analysis

#### Purpose

Advanced party momentum analysis using velocity (rate of change), acceleration (change in velocity), trajectory classification, and coalition potential scoring. Essential for electoral forecasting.
...
|--------|------|-------------|---------|
| `election_year` | INTEGER | Election cycle | 2022 |
| `party` | VARCHAR | Party abbreviation | SD |
| `party_size` | BIGINT | Current MPs | 73 |
| `velocity_1cycle` | NUMERIC | Size change from prev cycle | 11 |
| `acceleration` | NUMERIC | Change in velocity | 3 |
| `momentum_score` | NUMERIC | Weighted momentum metric | 8.2 |
| `momentum_classification` | VARCHAR | Momentum strength | SURGING |
| `trajectory` | VARCHAR | Growth pattern | ACCELERATING_GROWTH |
| `coalition_potential_score` | NUMERIC | Market share + momentum | 25.1 |

#### Momentum Classifications

| Classification | Momentum Score | Interpretation |
|---------------|----------------|----------------|
| `SURGING` | > 10 | Explosive growth |
| `RISING` | 5 to 10 | Strong growth |
| `GROWING` | 0 to 5 | Moderate growth |
| `DECLINING` | 0 to -5 | Moderate decline |
| `FALLING` | -5 to -10 | Strong decline |
| `COLLAPSING` | < -10 | Severe decline |

#### Trajectory Classifications

| Trajectory | Velocity | Acceleration | Pattern |
|-----------|----------|--------------|---------|
| `ACCELERATING_GROWTH` | Positive | Positive | Speeding up growth |
| `DECELERATING_GROWTH` | Positive | Negative | Slowing down growth |
| `STABLE` | Zero | Zero | No change |
| `DECELERATING_DECLINE` | Negative | Positive | Slowing down decline |
| `ACCELERATING_DECLINE` | Negative | Negative | Speeding up decline |

#### Intelligence Applications

- **Electoral Forecasting**: Predict party performance in next election
- **Coalition Strategy**: Identify growing/declining partners
- **Media Narrative**: Track "momentum" stories
- **Strategic Planning**: Understand competitive dynamics

#### Sample Queries

```sql
-- Parties with strongest momentum in latest cycle
SELECT 
    party,
    party_size,
    velocity_1cycle,
...
Analyzes behavioral volatility and stability using coefficient of variation, stability scores (0-100), and risk classifications. Critical for identifying erratic performers and assessing institutional stability.
...
|--------|------|-------------|---------|
| `person_id` | VARCHAR | Politician ID | 0123456789 |
| `cycles_active` | BIGINT | Number of cycles tracked | 5 |
| `attendance_volatility` | NUMERIC | Standard deviation of attendance | 7.23 |
| `attendance_cv` | NUMERIC | Coefficient of variation (%) | 7.89 |
| `individual_stability_score` | NUMERIC | Stability metric (0-100) | 92.11 |
| `individual_volatility_classification` | VARCHAR | Volatility level | STABLE |
| `volatility_risk_level` | VARCHAR | Risk assessment | LOW_RISK |
| `performance_pattern` | VARCHAR | Behavioral classification | CONSISTENTLY_GOOD |

#### Volatility Classifications

| Classification | Attendance Volatility | Description |
|---------------|----------------------|-------------|
| `VERY_STABLE` | < 5 | Minimal variation |
| `STABLE` | 5-10 | Low variation |
| `MODERATELY_VOLATILE` | 10-15 | Moderate variation |
| `VOLATILE` | 15-20 | High variation |
| `HIGHLY_VOLATILE` | > 20 | Extreme variation |

#### Performance Patterns

| Pattern | Criteria | Description |
|---------|----------|-------------|
| `CONSISTENTLY_EXCELLENT` | Low volatility + high attendance | Reliable top performers |
| `CONSISTENTLY_GOOD` | Low volatility + good attendance | Stable performers |
| `ERRATIC_PERFORMER` | High volatility | Unpredictable behavior |
| `ERRATIC_UNDERPERFORMER` | High volatility + low attendance | Unreliable underperformers |
| `AVERAGE_PERFORMER` | Other combinations | Typical patterns |

#### Intelligence Applications

- **Risk Assessment**: Identify politicians with erratic behavior
- **Reliability Scoring**: Find most consistent performers
- **Trend Prediction**: Low volatility = predictable behavior
- **Institutional Stability**: Track system-wide volatility trends

#### Sample Queries

```sql
-- Most consistent politicians (lowest volatility)
SELECT 
    CONCAT(first_name, ' ', last_name) as name,
    party,
    cycles_active,
...
SELECT 
    ...
    performance_pattern
FROM view_riksdagen_election_cycle_volatility_analysis
WHERE volatility_risk_level = 'HIGH_RISK'
ORDER BY attendance_volatility DESC;
```
