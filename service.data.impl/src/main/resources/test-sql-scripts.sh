...
test_script() {
    ...
    # Special handling for full_schema.sql
    # This file is generated by pg_dump and should never be manually edited
    # It may contain PostgreSQL version-specific settings
    if [ "$script_name" = "full_schema.sql" ]; then
        echo -e "  ${YELLOW}ℹ️  NOTE: full_schema.sql is a generated artifact from pg_dump${NC}"
        echo -e "  ${YELLOW}ℹ️  This file should NEVER be manually edited${NC}"
        echo -e "  ${YELLOW}ℹ️  See README-SCHEMA-MAINTENANCE.md for regeneration instructions${NC}"
        echo "$script_name,$category,SKIP,SKIP,SKIP,\"Generated artifact - not tested\"" >> "$RESULTS_FILE"
        SKIPPED_SCRIPTS=$((SKIPPED_SCRIPTS + 1))
        echo -e "  ${YELLOW}⊘ Overall: SKIPPED${NC}"
        return 0
    fi
