...
\restrict 4C9WIoMxwGifxVAkQyQgHVKIyTaHuLWCwTN6pemL9wbjiGJJLnpURdsCeNbJDOP
...
--
-- Name: mv_annual_document_metrics; Type: MATERIALIZED VIEW; Schema: public; Owner: -
--

CREATE MATERIALIZED VIEW public.mv_annual_document_metrics AS
 SELECT (EXTRACT(year FROM made_public_date))::integer AS year,
    count(DISTINCT id) AS documents_produced,
    count(DISTINCT
        CASE
            WHEN ((document_type)::text = 'mot'::text) THEN id
            ELSE NULL::character varying
        END) AS motions_filed,
    count(DISTINCT
        CASE
            WHEN ((document_type)::text = 'prop'::text) THEN id
            ELSE NULL::character varying
        END) AS proposals_filed
   FROM public.document_data dd
  WHERE ((made_public_date IS NOT NULL) AND ((EXTRACT(year FROM made_public_date) >= (2002)::numeric) AND (EXTRACT(year FROM made_public_date) <= (2026)::numeric)))
  GROUP BY ((EXTRACT(year FROM made_public_date))::integer)
  WITH NO DATA;


--
-- Name: MATERIALIZED VIEW mv_annual_document_metrics; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON MATERIALIZED VIEW public.mv_annual_document_metrics IS 'Pre-aggregated annual document metrics from document_data (2002-2026). 
...
--
-- Name: mv_annual_voting_metrics; Type: MATERIALIZED VIEW; Schema: public; Owner: -
--

CREATE MATERIALIZED VIEW public.mv_annual_voting_metrics AS
 ...
 SELECT (EXTRACT(year FROM vd.vote_date))::integer AS year,
    ...
    round((avg(
        CASE
            WHEN (upper((vd.vote)::text) = 'JA'::text) THEN 1
            ELSE 0
        END) * (100)::numeric), 2) AS avg_yes_rate,
    round((avg(
        CASE
            WHEN (upper((vd.vote)::text) = 'NEJ'::text) THEN 1
            ELSE 0
        END) * (100)::numeric), 2) AS avg_no_rate,
    round((avg(
        CASE
            WHEN (upper((vd.vote)::text) = 'AVSTÅR'::text) THEN 1
            ELSE 0
        END) * (100)::numeric), 2) AS avg_abstain_rate
   FROM (public.vote_data vd
     LEFT JOIN election_years ey ON (((ey.election_year)::numeric = EXTRACT(year FROM vd.vote_date))))
  WHERE ((vd.vote_date IS NOT NULL) AND (EXTRACT(year FROM vd.vote_date) >= (2002)::numeric) AND (EXTRACT(year FROM vd.vote_date) <= (2026)::numeric))
...
--
-- Name: MATERIALIZED VIEW mv_annual_voting_metrics; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON MATERIALIZED VIEW public.mv_annual_voting_metrics IS 'Pre-aggregated annual voting metrics from vote_data (2002-2026). Uses UPPER() for vote comparisons. Refresh: Daily.';
...
--
-- Name: mv_decision_temporal_trends; Type: MATERIALIZED VIEW; Schema: public; Owner: -
--

CREATE MATERIALIZED VIEW public.mv_decision_temporal_trends AS
 SELECT decision_day,
    daily_decisions,
    daily_approval_rate,
    approved_decisions,
    ...
    ma_30day_decisions,
    ma_90day_decisions,
    ma_30day_approval_rate,
    decisions_last_year,
    yoy_decisions_change,
    yoy_decisions_change_pct,
    decision_year,
    decision_month,
    decision_week,
    ...
    decision_quarter
   FROM public.view_decision_temporal_trends
  WITH NO DATA;
...
--
-- Name: mv_ministry_decision_impact; Type: MATERIALIZED VIEW; Schema: public; Owner: -
--

CREATE MATERIALIZED VIEW public.mv_ministry_decision_impact AS
 SELECT ministry_code,
    ...
    approved_proposals,
    rejected_proposals,
    referred_back_proposals,
    committee_referral_proposals,
    other_decisions,
    approval_rate,
    rejection_rate,
    committee_referral_rate,
    earliest_proposal_date,
    latest_proposal_date
   FROM public.view_ministry_decision_impact
  WITH NO DATA;
...
--
-- Name: mv_party_decision_flow; Type: MATERIALIZED VIEW; Schema: public; Owner: -
--

CREATE MATERIALIZED VIEW public.mv_party_decision_flow AS
 SELECT party,
    committee,
    decision_type,
    committee_org,
   ...
   FROM public.view_riksdagen_party_decision_flow
  WITH NO DATA;
...
CREATE VIEW public.view_committee_productivity AS
 WITH committee_membership AS (
         SELECT rc.embedded_id_org_code AS committee_code,
           ...
           FROM (public.view_riksdagen_committee rc
             LEFT JOIN public.assignment_data ad ON ((((ad.org_code)::text = (rc.embedded_id_org_code)::text) AND ((ad.assignment_type)::text = ANY (ARRAY['uppdrag'::text, 'Riksdagsorgan'::text])) AND ((ad.to_date IS NULL) OR (ad.to_date >= CURRENT_DATE)))))
         ...
         SELECT upper((cdd.org)::text) AS committee_code,
            ...
            count(DISTINCT cdd.id) FILTER (WHERE ((cdd.sub_type)::text = 'bet'::text)) AS reports,
            count(DISTINCT cdd.id) FILTER (WHERE ((cdd.sub_type)::text = 'utl'::text)) AS statements,
            count(DISTINCT cdd.id) FILTER (WHERE (((cdd.sub_type)::text <> ALL (ARRAY['bet'::text, 'utl'::text])) OR (cdd.sub_type IS NULL))) AS other_documents
 ...
 SELECT committee_code,
    ...
    COALESCE(other_documents, (0)::bigint) AS motions_count,
...
CREATE VIEW public.view_committee_productivity_matrix AS
 WITH committee_documents AS (
         SELECT view_riksdagen_politician_document.org AS committee_code,
            ...
            count(DISTINCT
                CASE
                    WHEN ((view_riksdagen_politician_document.document_type)::text = 'bet'::text) THEN view_riksdagen_politician_document.doc_id
                    ELSE NULL::character varying
                END) AS committee_reports,
            count(DISTINCT
                CASE
                    WHEN ((view_riksdagen_politician_document.document_type)::text = 'mot'::text) THEN view_riksdagen_politician_document.doc_id
                    ELSE NULL::character varying
...
CREATE VIEW public.view_politician_risk_summary AS
 WITH party_ballot_consensus AS (
         SELECT vd.embedded_id_ballot_id,
            vd.party,
            mode() WITHIN GROUP (ORDER BY (upper((vd.vote)::text))) AS party_consensus_vote
           FROM public.vote_data vd
          WHERE ((upper((vd.vote)::text) <> 'FRÅNVARANDE'::text) AND (vd.party IS NOT NULL) AND (vd.vote_date >= (CURRENT_DATE - '2 years'::interval)))
         ...
         SELECT p.id AS person_id,
            ...
            count(DISTINCT vd.embedded_id_ballot_id) FILTER (WHERE ((pbc.party_consensus_vote IS NOT NULL) AND (upper((vd.vote)::text) <> pbc.party_consensus_vote) AND (upper((vd.vote)::text) <> 'FRÅNVARANDE'::text))) AS rebel_votes
           FROM ((public.person_data p
             ...
             LEFT JOIN party_ballot_consensus pbc ON ((((pbc.embedded_id_ballot_id)::text = (vd.embedded_id_ballot_id)::text) AND ((pbc.party)::text = (vd.party)::text))))
          WHERE ((p.status)::text = ANY (ARRAY['Tjänstgörande riksdagsledamot'::text, 'Tjänstgörande ersättare'::text, 'Tillgänglig ersättare'::text]))
          GROUP BY p.id
         ...
         SELECT p.id AS person_id,
            ...
            count(DISTINCT rv.id) FILTER (WHERE ((rv.rule_group)::text = 'Behavior'::text)) AS behavior_violations,
            count(DISTINCT rv.id) FILTER (WHERE ((rv.rule_group)::text = 'Assignments'::text)) AS assignments_violations,
            count(DISTINCT rv.id) FILTER (WHERE ((rv.rule_group)::text = 'Experience'::text)) AS experience_violations,
            count(DISTINCT rv.id) FILTER (WHERE ((rv.rule_group)::text = 'Party'::text)) AS party_violations,
            count(DISTINCT rv.id) FILTER (WHERE ((rv.rule_group)::text = 'Role'::text)) AS role_violations,
           ...
           FROM (((public.person_data p
             ...
             LEFT JOIN public.rule_violation rv ON ((((rv.reference_id)::text = (p.id)::text) AND ((rv.resource_type)::text = 'POLITICIAN'::text) AND ((rv.status)::text = ANY (ARRAY['MINOR'::text, 'MAJOR'::text, 'CRITICAL'::text])))))
          WHERE ((p.status)::text = ANY (ARRAY['Tjänstgörande riksdagsledamot'::text, 'Tjänstgörande ersättare'::text, 'Tillgänglig ersättare'::text]))
 ...
 SELECT person_id,
    ...
    behavior_violations AS absenteeism_violations,
    experience_violations AS effectiveness_violations,
    assignments_violations AS discipline_violations,
    party_violations AS productivity_violations,
    role_violations AS collaboration_violations,
    annual_absence_rate,
    annual_rebel_rate,
    annual_vote_count,
    documents_last_year,
...
CREATE VIEW public.view_riksdagen_voting_anomaly_detection AS
 WITH party_consensus AS (
         SELECT vote_data.embedded_id_ballot_id,
            vote_data.party,
            upper((vote_data.vote)::text) AS vote,
            count(*) AS vote_count,
            row_number() OVER (PARTITION BY vote_data.embedded_id_ballot_id, vote_data.party ORDER BY (count(*)) DESC) AS rank
           FROM public.vote_data
          WHERE ((upper((vote_data.vote)::text) = ANY (ARRAY['JA'::text, 'NEJ'::text, 'AVSTÅR'::text])) AND (vote_data.party IS NOT NULL) AND (vote_data.vote_date >= (CURRENT_DATE - '25 years'::interval)))
          GROUP BY vote_data.embedded_id_ballot_id, vote_data.party, (upper((vote_data.vote)::text))
         ...
         SELECT vote_data.embedded_id_ballot_id,
           ...
           FROM public.vote_data
          WHERE ((vote_data.party IS NOT NULL) AND (vote_data.vote_date >= (CURRENT_DATE - '25 years'::interval)))
         ...
         SELECT vd.embedded_id_intressent_id AS person_id,
          ...
          WHERE ((upper((vd.vote)::text) <> pmv.party_consensus_vote) AND (upper((vd.vote)::text) = ANY (ARRAY['JA'::text, 'NEJ'::text, 'AVSTÅR'::text])) AND (vd.vote_date >= (CURRENT_DATE - '25 years'::interval)))
        ), person_rebellions AS (
         SELECT rv.person_id,
 ...
 SELECT p.id AS person_id,
    ...
    p.party,
    pr.total_rebellions,
    pr.strong_consensus_rebellions,
    pr.very_strong_consensus_rebellions,
    pr.avg_consensus_strength_rebelled_against,
        CASE
            WHEN (pr.very_strong_consensus_rebellions >= 5) THEN 'FREQUENT_STRONG_REBEL'::text
            WHEN (pr.strong_consensus_rebellions >= 10) THEN 'CONSISTENT_REBEL'::text
            WHEN (pr.total_rebellions >= 20) THEN 'MODERATE_REBEL'::text
            WHEN (pr.total_rebellions >= 5) THEN 'OCCASIONAL_REBEL'::text
            ELSE 'PARTY_ALIGNED'::text
        END AS anomaly_classification,
        CASE
            WHEN (pr.very_strong_consensus_rebellions >= 5) THEN 'HIGH ANOMALY: Frequent rebellion against very strong party consensus (90%+)'::text
            WHEN (pr.strong_consensus_rebellions >= 10) THEN 'MODERATE ANOMALY: Consistent rebellion against strong party consensus (80%+)'::text
            WHEN (pr.total_rebellions >= 20) THEN 'MILD ANOMALY: Moderate pattern of voting against party majority'::text
            WHEN (pr.total_rebellions >= 5) THEN 'LOW ANOMALY: Occasional independent voting behavior'::text
            ELSE 'NO ANOMALY: Votes consistently with party consensus'::text
        END AS anomaly_assessment
   FROM (public.person_data p
     JOIN person_rebellions pr ON (((pr.person_id)::text = (p.id)::text)))
  ORDER BY pr.very_strong_consensus_rebellions DESC, pr.total_rebellions DESC, p.last_name, p.first_name;
...
CREATE VIEW public.view_risk_score_evolution AS
 WITH party_ballot_majority AS (
         SELECT vote_data.embedded_id_ballot_id,
            ...
            sum(
                CASE
                    WHEN (upper((vote_data.vote)::text) = 'JA'::text) THEN 1
                    ELSE 0
                END) AS party_yes_count,
            sum(
                CASE
                    WHEN (upper((vote_data.vote)::text) = 'NEJ'::text) THEN 1
                    ELSE 0
         ...
         SELECT vd.embedded_id_intressent_id,
            ...
            pbm.party_approved,
                CASE
                    WHEN ((upper((vd.vote)::text) = 'NEJ'::text) AND (pbm.party_approved = true)) THEN true
                    WHEN ((upper((vd.vote)::text) = 'JA'::text) AND (pbm.party_approved = false)) THEN true
                    ELSE false
                END AS is_rebel
           FROM (public.vote_data vd
         ...
         SELECT p.id AS person_id,
            ...
            round((((count(*) FILTER (WHERE (upper((pvr.vote)::text) = 'FRÅNVARANDE'::text)))::numeric / (NULLIF(count(*), 0))::numeric) * (100)::numeric), 2) AS absence_rate,
            round((((count(*) FILTER (WHERE (pvr.is_rebel = true)))::numeric / (NULLIF(count(*) FILTER (WHERE (upper((pvr.vote)::text) = ANY (ARRAY['JA'::text, 'NEJ'::text]))), 0))::numeric) * (100)::numeric), 2) AS rebel_rate,
         ...
         SELECT rule_violation.reference_id AS person_id,
          ...
          WHERE (((rule_violation.resource_type)::text = 'POLITICIAN'::text) AND ((rule_violation.status)::text = ANY (ARRAY['MINOR'::text, 'MAJOR'::text, 'CRITICAL'::text])) AND (rule_violation.detected_date >= (CURRENT_DATE - '3 years'::interval)))
...
CREATE VIEW public.view_election_cycle_anomaly_pattern AS
 WITH election_cycle_periods AS (
         ...
         SELECT count(DISTINCT view_riksdagen_voting_anomaly_detection.person_id) FILTER (WHERE (view_riksdagen_voting_anomaly_detection.anomaly_classification = ANY (ARRAY['FREQUENT_STRONG_REBEL'::text, 'CONSISTENT_REBEL'::text]))) AS high_anomaly_count,
            round(avg(view_riksdagen_voting_anomaly_detection.total_rebellions), 2) AS avg_total_rebellions,
            count(DISTINCT view_riksdagen_voting_anomaly_detection.person_id) FILTER (WHERE (view_riksdagen_voting_anomaly_detection.strong_consensus_rebellions >= 5)) AS strong_consensus_rebels
           FROM public.view_riksdagen_voting_anomaly_detection
        ), risk_stats AS (
         SELECT round(avg(view_politician_risk_summary.risk_score), 2) AS avg_risk_score_prs,
            count(DISTINCT view_politician_risk_summary.person_id) FILTER (WHERE (view_politician_risk_summary.risk_level = ANY (ARRAY['HIGH'::text, 'CRITICAL'::text]))) AS high_risk_politicians
           FROM public.view_politician_risk_summary
        ), v_base AS (
...
CREATE VIEW public.view_election_cycle_decision_intelligence AS
 WITH v151_base AS (
         ...
         SELECT ecp.election_cycle_id,
           ...
           FROM (((election_cycle_periods ecp
             LEFT JOIN public.mv_party_decision_flow pdf ON ((pdf.decision_year = (ecp.calendar_year)::numeric)))
             LEFT JOIN public.mv_decision_temporal_trends dtt ON ((dtt.decision_year = (ecp.calendar_year)::numeric)))
             LEFT JOIN public.mv_ministry_decision_impact mdi ON ((mdi.decision_year = (ecp.calendar_year)::numeric)))
...
--
-- Name: VIEW view_election_cycle_network_analysis; Type: COMMENT; Schema: public; Owner: -
--

COMMENT ON VIEW public.view_election_cycle_network_analysis IS 'Election cycle network analysis showing coalition alignment patterns over time. 
                 OPTIMIZED: Removed cartesian product from always-true JOIN (pim ON 1=1) that caused statement timeout. 
                 Politician influence metrics (influential_politicians, avg_network_centrality, power_broker_count) set to 0 
                 pending proper party/year dimensions in source view_riksdagen_politician_influence_metrics. 
                 Features: coalition strength classification, rank by alignment, percent rank, ntile quartiles, 
                 lag analysis for trend detection, coalition stability trends, network density calculation. 
                 Framework: Network Analysis. Use Case: Identify coalition formation patterns, track coalition stability 
                 over election cycles, predict future coalition scenarios.
                 Performance: Eliminated 212K cartesian product rows, query completes in <1 second (was timeout).';
...
CREATE VIEW public.view_ministry_risk_evolution AS
 WITH ministry_base AS (
         ...
         SELECT mq.org_code,
            ...
            count(DISTINCT
                CASE
                    WHEN (lower((doc.document_type)::text) = ANY (ARRAY['prop'::text, 'bet'::text])) THEN doc.id
                    ELSE NULL::bigint
...
CREATE VIEW public.view_party_effectiveness_trends AS
 WITH quarterly_party_voting AS (
         ...
         SELECT pd.party,
          ...
          WHERE (((rv.resource_type)::text = 'POLITICIAN'::text) AND ((rv.status)::text = ANY (ARRAY['MINOR'::text, 'MAJOR'::text, 'CRITICAL'::text])) AND (rv.detected_date >= (CURRENT_DATE - '3 years'::interval)) AND (pd.party IS NOT NULL))
...
CREATE VIEW public.view_ministry_effectiveness_trends AS
 WITH ministry_base AS (
         ...
         SELECT m.org_code,
            ...
            count(DISTINCT
                CASE
                    WHEN (lower((doc.document_type)::text) = 'bet'::text) THEN doc.id
                    ELSE NULL::bigint
...
CREATE VIEW public.view_party_performance_metrics AS
 WITH party_member_counts AS (
         SELECT person_data.party,
            count(*) FILTER (WHERE ((person_data.status)::text = ANY (ARRAY['Tjänstgörande riksdagsledamot'::text, 'Tjänstgörande ersättare'::text, 'Tillgänglig ersättare'::text]))) AS active_members,
            count(*) FILTER (WHERE (((person_data.status)::text ~~ 'Tidigare%'::text) OR ((person_data.status)::text = 'Inga uppdrag'::text) OR ((person_data.status)::text = 'Avgången riksdagsledamot'::text))) AS inactive_members
         ...
         SELECT p.party,
           ...
           FROM (public.person_data p
             JOIN public.rule_violation rv ON ((((rv.reference_id)::text = (p.id)::text) AND ((rv.resource_type)::text = 'POLITICIAN'::text) AND ((rv.status)::text = ANY (ARRAY['MINOR'::text, 'MAJOR'::text, 'CRITICAL'::text])))))
         ...
         SELECT p.party,
            ...
            count(DISTINCT
                CASE
                    WHEN ((vpd.document_type)::text = 'mot'::text) THEN vpd.doc_id
                    ELSE NULL::character varying
                END) AS motions_produced,
            count(DISTINCT
                CASE
                    WHEN ((vpd.document_type)::text = 'prop'::text) THEN vpd.doc_id
                    ELSE NULL::character varying
                END) AS propositions_produced
           FROM (public.person_data p
 ...
 SELECT party,
    ...
    concat_ws(' | '::text,
        CASE
            WHEN (COALESCE(avg_participation_rate, (0)::numeric) < (80)::numeric) THEN 'Low participation'::text
            ELSE NULL::text
        END,
        CASE
            WHEN (COALESCE(avg_win_rate, (0)::numeric) < (50)::numeric) THEN 'Low effectiveness'::text
            ELSE NULL::text
        END,
        CASE
            WHEN (COALESCE(avg_rebel_rate, (0)::numeric) > (10)::numeric) THEN 'Discipline concerns'::text
            ELSE NULL::text
        END,
        CASE
  ...
  ORDER BY calculated_performance_score DESC;
...
--
-- Name: view_riksdagen_election_year_behavioral_patterns; Type: VIEW; Schema: public; Owner: -
--
...
CREATE VIEW public.view_riksdagen_election_year_behavioral_patterns AS
 ...
 SELECT am.year,
    ...
    round(eb.midterm_avg_attendance, 2) AS midterm_avg_attendance,
        ...
        END AS attendance_z_score,
        CASE
            WHEN am.is_election_year THEN 'ELECTION_YEAR'::text
            ELSE 'MIDTERM_YEAR'::text
        END AS year_classification,
        CASE
            WHEN (am.is_election_year AND ((am.total_ballots)::numeric > COALESCE(eb.election_avg_ballots, (0)::numeric))) THEN 'ABOVE_ELECTION_AVERAGE'::text
            WHEN am.is_election_year THEN 'BELOW_ELECTION_AVERAGE'::text
            WHEN ((am.total_ballots)::numeric > COALESCE(eb.midterm_avg_ballots, (0)::numeric)) THEN 'ABOVE_MIDTERM_AVERAGE'::text
            ELSE 'BELOW_MIDTERM_AVERAGE'::text
        END AS ballot_assessment,
        CASE
            WHEN (((am.documents_produced)::numeric > COALESCE(eb.election_avg_docs, (0)::numeric)) AND ((am.motions_filed)::numeric > COALESCE(eb.election_avg_motions, (0)::numeric))) THEN 'HIGH_LEGISLATIVE_ACTIVITY'::text
            WHEN ((am.documents_produced)::numeric > COALESCE(eb.midterm_avg_docs, (0)::numeric)) THEN 'ABOVE_AVERAGE_ACTIVITY'::text
            WHEN ((am.documents_produced)::numeric < (COALESCE(eb.midterm_avg_docs, (0)::numeric) * 0.5)) THEN 'LOW_LEGISLATIVE_ACTIVITY'::text
            ELSE 'NORMAL_LEGISLATIVE_ACTIVITY'::text
...
CREATE VIEW public.view_riksdagen_party_coalition_evolution AS
 WITH election_cycle_periods AS (
         ...
         SELECT ecp.election_cycle_id,
           ...
           FROM ((election_cycle_periods ecp
             JOIN public.view_riksdagen_vote_data_ballot_party_summary_monthly vbps1 ON ((date_part('year'::text, vbps1.embedded_id_vote_date) = (ecp.calendar_year)::double precision)))
             JOIN public.view_riksdagen_vote_data_ballot_party_summary_monthly vbps2 ON (((vbps2.embedded_id_vote_date = vbps1.embedded_id_vote_date) AND ((vbps2.embedded_id_party)::text > (vbps1.embedded_id_party)::text))))
         ...
         HAVING (count(DISTINCT vbps1.embedded_id_vote_date) >= 3)
...
CREATE VIEW public.view_riksdagen_party_transition_history AS
 WITH known_parties AS (
         SELECT sweden_political_party.short_code
           FROM public.sweden_political_party
          WHERE ((sweden_political_party.short_code IS NOT NULL) AND ((sweden_political_party.short_code)::text <> ''::text) AND ((sweden_political_party.short_code)::text <> '-'::text))
        UNION
         SELECT unnest(ARRAY['FP'::text, 'NYD'::text, 'VPK'::text, 'K'::text, 'H'::text, 'BF'::text, 'KrVP'::text]) AS short_code
        ), party_name_map AS (
         SELECT t.old_code,
            t.new_code
           FROM ( VALUES ('FP'::text,'L'::text), ('VPK'::text,'V'::text), ('K - VPK'::text,'V'::text), ('K'::text,'V'::text), ('BF - C'::text,'C'::text), ('H - M'::text,'M'::text)) t(old_code, new_code)
         ...
         SELECT a.intressent_id AS person_id,
            ...
            a.status
           FROM public.assignment_data a
          WHERE (((a.org_code)::text IN ( SELECT known_parties.short_code
                   FROM known_parties)) AND (a.from_date IS NOT NULL) AND (a.from_date >= '1990-01-01'::date))
        ), person_party_timeline AS (
         SELECT assignment_party_periods.person_id,
            assignment_party_periods.party,
            min(assignment_party_periods.from_date) AS first_date,
            max(assignment_party_periods.to_date) AS last_date
           FROM assignment_party_periods
          GROUP BY assignment_party_periods.person_id, assignment_party_periods.party
        ), transitions_raw AS (
         SELECT t1.person_id,
           ...
           FROM (person_party_timeline t1
             JOIN person_party_timeline t2 ON ((((t1.person_id)::text = (t2.person_id)::text) AND ((t1.party)::text <> (t2.party)::text) AND (t2.first_date > t1.first_date) AND (NOT (EXISTS ( SELECT 1
                   FROM person_party_timeline t3
                  WHERE (((t3.person_id)::text = (t1.person_id)::text) AND ((t3.party)::text = (t2.party)::text) AND (t3.first_date > t1.first_date) AND (t3.first_date < t2.first_date))))))))
          WHERE (COALESCE(( SELECT pm.new_code
                   FROM party_name_map pm
                  WHERE (pm.old_code = (t1.party)::text)), (t1.party)::text) <> COALESCE(( SELECT pm.new_code
                   FROM party_name_map pm
                  WHERE (pm.old_code = (t2.party)::text)), (t2.party)::text))
         ...
         SELECT vd.embedded_id_intressent_id AS person_id,
           ...
           FROM public.vote_data vd
          WHERE ((vd.party IS NOT NULL) AND ((vd.party)::text <> '-'::text) AND ((vd.party)::text <> ''::text) AND (vd.vote_date IS NOT NULL) AND (vd.vote_date >= '2002-01-01'::date))
         ...
         SELECT v1.person_id,
           ...
           FROM (vote_party_periods v1
             JOIN vote_party_periods v2 ON ((((v1.person_id)::text = (v2.person_id)::text) AND ((v1.party)::text <> (v2.party)::text) AND (v2.first_vote_date > v1.last_vote_date) AND (NOT (EXISTS ( SELECT 1
                   FROM vote_party_periods v3
                  WHERE (((v3.person_id)::text = (v1.person_id)::text) AND (v3.first_vote_date > v1.last_vote_date) AND (v3.first_vote_date < v2.first_vote_date) AND ((v3.party)::text <> (v1.party)::text) AND ((v3.party)::text <> (v2.party)::text))))))))
        ), all_transitions AS (
         SELECT transitions_raw.person_id,
            transitions_raw.previous_party,
            transitions_raw.new_party,
            transitions_raw.transition_date
           FROM transitions_raw
        UNION
         SELECT vote_transitions.person_id,
            vote_transitions.previous_party,
            vote_transitions.new_party,
            vote_transitions.transition_date
           FROM vote_transitions
        ), deduplicated AS (
         SELECT all_transitions.person_id,
            all_transitions.previous_party,
            all_transitions.new_party,
            min(all_transitions.transition_date) AS transition_date
           FROM all_transitions
          GROUP BY all_transitions.person_id, all_transitions.previous_party, all_transitions.new_party
 ...
 SELECT dt.person_id,
    ...
    dt.transition_date,
        CASE
            WHEN (( SELECT pm.new_code
               FROM party_name_map pm
              WHERE (pm.old_code = (dt.previous_party)::text)) = COALESCE(( SELECT pm.new_code
               FROM party_name_map pm
              WHERE (pm.old_code = (dt.new_party)::text)), (dt.new_party)::text)) THEN 'PARTY_RENAMED'::text
            WHEN (EXISTS ( SELECT 1
               FROM public.vote_data vd
              WHERE (((vd.embedded_id_intressent_id)::text = (dt.person_id)::text) AND (vd.vote_date >= (dt.transition_date - '60 days'::interval)) AND (vd.vote_date < dt.transition_date)))) THEN 'SWITCHED_WHILE_SERVING'::text
    ...
    (((EXTRACT(year FROM age((( SELECT min(ed.election_date) AS min
           FROM election_proximity ed
          WHERE (ed.election_date > dt.transition_date)))::timestamp with time zone, (dt.transition_date)::timestamp with time zone)) * (12)::numeric) + EXTRACT(month FROM age((( SELECT min(ed.election_date) AS min
           FROM election_proximity ed
          WHERE (ed.election_date > dt.transition_date)))::timestamp with time zone, (dt.transition_date)::timestamp with time zone))))::integer AS months_until_next_election,
    ( SELECT max(ed.election_date) AS max
           FROM election_proximity ed
          WHERE (ed.election_date < dt.transition_date)) AS previous_election,
    (((EXTRACT(year FROM age((dt.transition_date)::timestamp with time zone, (( SELECT max(ed.election_date) AS max
           FROM election_proximity ed
          WHERE (ed.election_date < dt.transition_date)))::timestamp with time zone)) * (12)::numeric) + EXTRACT(month FROM age((dt.transition_date)::timestamp with time zone, (( SELECT max(ed.election_date) AS max
           FROM election_proximity ed
...
CREATE VIEW public.view_riksdagen_party_defector_analysis AS
 WITH defector_performance AS (
         SELECT pt.person_id,
            ...
            round((avg(
                CASE
                    WHEN ((vd.vote_date >= (pt.transition_date - '1 year'::interval)) AND (vd.vote_date < pt.transition_date)) THEN
                    CASE
                        WHEN (upper((vd.vote)::text) <> 'FRÅNVARANDE'::text) THEN 1
                        ELSE 0
                    END
            ...
            round((avg(
                CASE
                    WHEN ((vd.vote_date >= pt.transition_date) AND (vd.vote_date <= (pt.transition_date + '1 year'::interval))) THEN
                    CASE
                        WHEN (upper((vd.vote)::text) <> 'FRÅNVARANDE'::text) THEN 1
                        ELSE 0
                    END
            ...
            count(DISTINCT
                CASE
                    WHEN ((vd.vote_date < pt.transition_date) AND (vd.vote_date >= (pt.transition_date - '1 year'::interval))) THEN vd.embedded_id_ballot_id
                    ELSE NULL::character varying
                END) AS docs_before,
            count(DISTINCT
                CASE
          ...
          GROUP BY pt.person_id, pt.first_name, pt.last_name, pt.previous_party, pt.new_party, pt.transition_date, pt.transition_type, pt.months_until_next_election
        )
 SELECT person_id,
    first_name,
    last_name,
    ...
    round((COALESCE(post_transition_attendance, (0)::numeric) - COALESCE(pre_transition_attendance, (0)::numeric)), 2) AS attendance_change,
    docs_before,
    docs_after,
        CASE
...
CREATE VIEW public.view_riksdagen_party_electoral_trends AS
 WITH election_cycle_periods AS (
         ...
         SELECT ecp.election_cycle_id,
            ...
            (avg((vbps.party_total_votes / NULLIF(vbps.number_ballots, (0)::numeric))))::integer AS seat_count_proxy,
            COALESCE(max(pyd.docs_produced), (0)::bigint) AS documents_produced,
            round(COALESCE(avg(pyr.avg_rebel_rate), (0)::numeric), 2) AS avg_rebel_rate
           FROM (((election_cycle_periods ecp
             ...
             LEFT JOIN party_year_documents pyd ON ((((pyd.party)::text = (vbps.embedded_id_party)::text) AND (pyd.doc_year = ecp.calendar_year))))
             LEFT JOIN party_year_rebel pyr ON (((pyr.party = (vbps.embedded_id_party)::text) AND (pyr.rebel_year = ecp.calendar_year))))
          WHERE ((vbps.embedded_id_party IS NOT NULL) AND (ecp.election_year IS NOT NULL))
          GROUP BY ecp.election_cycle_id, ecp.cycle_year, ecp.calendar_year, ecp.election_year,
                CASE
                    WHEN ((EXTRACT(month FROM vbps.embedded_id_vote_date) >= (9)::numeric) OR (EXTRACT(month FROM vbps.embedded_id_vote_date) <= (1)::numeric)) THEN 'autumn'::text
                    ELSE 'spring'::text
         ...
         SELECT esd.election_cycle_id,
            ...
            rank() OVER (PARTITION BY esd.election_cycle_id, esd.semester ORDER BY (((esd.win_rate * 0.5) + (esd.approval_rate * 0.3)) + (esd.participation_rate * 0.2)) DESC NULLS LAST) AS rank_by_effectiveness,
            ...
            ntile(4) OVER (PARTITION BY esd.election_cycle_id, esd.semester ORDER BY (((esd.win_rate * 0.4) + (esd.approval_rate * 0.3)) + (esd.participation_rate * 0.3)) DESC NULLS LAST) AS quartile_by_performance,
 ...
 SELECT election_cycle_id,
    ...
    ma_3semester_win_rate,
        ...
        CASE
            WHEN (prev_semester_seats IS NOT NULL) THEN
            CASE
                WHEN ((seat_count_proxy > prev_semester_seats) AND (win_rate > COALESCE(prev_semester_win_rate, (0)::numeric))) THEN 'ASCENDING'::text
                WHEN ((seat_count_proxy < prev_semester_seats) AND (win_rate < COALESCE(prev_semester_win_rate, (0)::numeric))) THEN 'DESCENDING'::text
                WHEN (seat_count_proxy > prev_semester_seats) THEN 'GROWING'::text
                WHEN (seat_count_proxy < prev_semester_seats) THEN 'DECLINING'::text
                ELSE 'STABLE'::text
            END
            ELSE 'NEW_ENTRY'::text
        END AS electoral_trend,
        CASE
            WHEN (seat_count_proxy >= 100) THEN 'DOMINANT'::text
            WHEN (seat_count_proxy >= 50) THEN 'MAJOR'::text
            WHEN (seat_count_proxy >= 20) THEN 'MEDIUM'::text
            WHEN (seat_count_proxy >= 10) THEN 'SMALL'::text
            ELSE 'MINOR'::text
    ...
    round(((seat_count_proxy)::numeric - ma_3semester_seats), 2) AS seat_deviation_from_ma,
        ...
        CASE
            ...
            WHEN ((quartile_by_size <= 2) AND (rank_by_win_rate <= 3)) THEN 'INFLUENTIAL'::text
            WHEN (quartile_by_size <= 2) THEN 'SIGNIFICANT'::text
            WHEN (rank_by_win_rate <= 3) THEN 'EFFECTIVE_MINOR'::text
            ELSE 'MARGINAL'::text
    ...
    round(((((percentile_seats * (0.4)::double precision) + (percentile_win_rate * (0.4)::double precision)) + (percentile_productivity * (0.2)::double precision)))::numeric, 4) AS composite_electoral_score,
    ...
    round(((((win_rate * 0.3) + (participation_rate * 0.3)) + (approval_rate * 0.2)) + (((100)::numeric - COALESCE(avg_rebel_rate, (0)::numeric)) * 0.2)), 2) AS election_readiness_score,
        ...
        CASE
            ...
            WHEN ((prev_semester_win_rate IS NOT NULL) AND (stddev_win_rate_party > (0)::numeric) AND (abs(((win_rate - prev_semester_win_rate) / NULLIF(stddev_win_rate_party, (0)::numeric))) > (2)::numeric)) THEN 'PERFORMANCE_SHIFT'::text
            ELSE NULL::text
        END AS electoral_warning_flag,
    ((cycle_year = 4) AND (semester = 'spring'::text)) AS is_pre_election_period,
    (((cycle_year = 4) AND (semester = 'autumn'::text)) OR ((cycle_year = 1) AND (semester = 'autumn'::text))) AS is_election_period,
    ((cycle_year = 1) AND (semester = 'spring'::text)) AS is_post_election_period
   FROM windowed_statistics;
...
CREATE VIEW public.view_riksdagen_party_longitudinal_performance AS
 WITH election_cycle_calendar AS (
         ...
         SELECT psd.election_cycle_id,
            ...
            (avg((psd.total_votes / NULLIF(psd.total_ballots, (0)::numeric))))::integer AS active_members,
            COALESCE(max(pyd.docs_produced), (0)::bigint) AS documents_last_year,
            round(COALESCE(avg(pyr.avg_rebel_rate), (0)::numeric), 2) AS avg_rebel_rate,
            round(((((psd.win_rate * 0.30) + (psd.participation_rate * 0.25)) + (psd.approval_rate * 0.25)) + (((100)::numeric - COALESCE(avg(pyr.avg_rebel_rate), (0)::numeric)) * 0.20)), 2) AS current_performance_score
           FROM ((party_semester_data psd
             LEFT JOIN party_year_documents pyd ON ((((pyd.party)::text = (psd.party)::text) AND (pyd.doc_year = psd.calendar_year))))
             LEFT JOIN party_year_rebel pyr ON (((pyr.party = (psd.party)::text) AND (pyr.rebel_year = psd.calendar_year))))
          GROUP BY psd.election_cycle_id, psd.cycle_year, psd.calendar_year, psd.semester, psd.is_election_year, psd.party, psd.total_ballots, psd.participation_rate, psd.win_rate, psd.yes_rate, psd.no_rate, psd.approval_rate, psd.total_votes, psd.yes_votes, psd.ballots_won, psd.decisions_approved
        ), windowed_statistics AS (
 ...
 SELECT election_cycle_id,
    ...
    ma_3semester_participation,
        ...
        CASE
            WHEN (prev_semester_win_rate IS NOT NULL) THEN
            CASE
                WHEN (win_rate > (prev_semester_win_rate + (3)::numeric)) THEN 'IMPROVING'::text
                WHEN (win_rate < (prev_semester_win_rate - (3)::numeric)) THEN 'DECLINING'::text
                ELSE 'STABLE'::text
            END
            ELSE 'NEW_ENTRY'::text
        END AS trajectory_win_rate,
        CASE
            WHEN (prev_semester_participation IS NOT NULL) THEN
            CASE
                WHEN (participation_rate > (prev_semester_participation + (3)::numeric)) THEN 'IMPROVING'::text
                WHEN (participation_rate < (prev_semester_participation - (3)::numeric)) THEN 'DECLINING'::text
                ELSE 'STABLE'::text
            END
            ELSE 'NEW_ENTRY'::text
    ...
    round(((((win_rate * 0.30) + (participation_rate * 0.25)) + (approval_rate * 0.25)) + (((100)::numeric - COALESCE(avg_rebel_rate, (0)::numeric)) * 0.20)), 2) AS legislative_effectiveness_score,
        CASE
            WHEN (stddev_win_rate_party > (10)::numeric) THEN 'HIGHLY_VOLATILE'::text
            WHEN (stddev_win_rate_party > (5)::numeric) THEN 'MODERATELY_VOLATILE'::text
            ELSE 'STABLE'::text
        END AS volatility_classification,
        CASE
            WHEN ((stddev_participation_party IS NOT NULL) AND (stddev_participation_party < (3)::numeric)) THEN 'VERY_STABLE'::text
            WHEN (stddev_participation_party < (5)::numeric) THEN 'STABLE'::text
            WHEN (stddev_participation_party < (10)::numeric) THEN 'MODERATE'::text
            ELSE 'UNSTABLE'::text
        END AS stability_classification,
        CASE
            WHEN (next_semester_win_rate IS NOT NULL) THEN
            CASE
                WHEN (next_semester_win_rate > win_rate) THEN 'IMPROVING'::text
                WHEN (next_semester_win_rate < win_rate) THEN 'DECLINING'::text
                ELSE 'STABLE'::text
            END
            ELSE NULL::text
        END AS forecast_trend,
        CASE
            WHEN (ma_3semester_win_rate IS NOT NULL) THEN round((win_rate - ma_3semester_win_rate), 2)
            ELSE NULL::numeric
        END AS trend_deviation_from_ma,
        CASE
            WHEN ((stddev_win_rate_party > (0)::numeric) AND (prev_semester_win_rate IS NOT NULL)) THEN round(((win_rate - prev_semester_win_rate) / NULLIF(stddev_win_rate_party, (0)::numeric)), 2)
            ELSE NULL::numeric
        END AS momentum_z_score_win_rate,
        CASE
            WHEN ((stddev_participation_party > (0)::numeric) AND (prev_semester_participation IS NOT NULL)) THEN round(((participation_rate - prev_semester_participation) / NULLIF(stddev_participation_party, (0)::numeric)), 2)
            ELSE NULL::numeric
        END AS momentum_z_score_participation,
        CASE
            WHEN ((prev_semester_win_rate IS NOT NULL) AND (prev_semester_participation IS NOT NULL)) THEN round(LEAST(1.0, (((4)::numeric - (abs((win_rate - prev_semester_win_rate)) / GREATEST(stddev_win_rate_party, (1)::numeric))) / (4)::numeric)), 2)
            ELSE NULL::numeric
        END AS trajectory_confidence_score,
        CASE
            WHEN ((quartile_by_win_rate = 1) AND (rank_by_productivity <= 3)) THEN 'TOP_PERFORMER'::text
            WHEN (quartile_by_win_rate <= 2) THEN 'STRONG'::text
            WHEN (quartile_by_win_rate <= 3) THEN 'AVERAGE'::text
            ELSE 'UNDERPERFORMING'::text
        END AS performance_tier,
        CASE
            WHEN ((rank_by_productivity <= 3) AND (documents_last_year > 100)) THEN 'HIGHLY_PRODUCTIVE'::text
            WHEN (rank_by_productivity <= 5) THEN 'PRODUCTIVE'::text
            ELSE 'LOW_PRODUCTIVITY'::text
        ...
        CASE
            ...
            WHEN ((prev_semester_win_rate IS NOT NULL) AND ((win_rate - prev_semester_win_rate) > (5)::numeric) AND ((participation_rate - COALESCE(prev_semester_participation, participation_rate)) > (3)::numeric)) THEN 'SURGING'::text
            ELSE NULL::text
        END AS early_warning_flag,
    ((cycle_year = 4) AND (semester = 'spring'::text)) AS is_pre_election_spring,
    ((cycle_year = 4) AND (semester = 'autumn'::text)) AS is_election_autumn,
    (cycle_year = 4) AS is_election_cycle_end
   FROM windowed_statistics;
...
CREATE VIEW public.view_riksdagen_party_momentum_analysis AS
 WITH quarterly_support AS (
         SELECT vote_data.party,
            ...
            count(
                CASE
                    WHEN (upper((vote_data.vote)::text) = 'AVSTÅR'::text) THEN 1
                    ELSE NULL::integer
                END) AS abstain_votes,
            count(
                CASE
                    WHEN (upper((vote_data.vote)::text) = 'FRÅNVARANDE'::text) THEN 1
                    ELSE NULL::integer
                END) AS absent_votes,
            ((count(
                CASE
...
CREATE VIEW public.view_riksdagen_party_switcher_outcomes AS
 ...
 SELECT ssa.person_id,
    ...
    max(
        CASE
            WHEN ((ssa.subsequent_status)::text = 'Tjänstgörande'::text) THEN 1
            ELSE 0
    ...
    max(
        CASE
            WHEN ((ssa.subsequent_role)::text = ANY (ARRAY['Partiledare'::text, 'Gruppledare'::text, 'Partisekreterare'::text, 'Ordförande'::text, 'Vice ordförande'::text])) THEN 1
            ELSE 0
...
CREATE VIEW public.view_riksdagen_politician_career_trajectory AS
 WITH person_leadership AS (
         SELECT assignment_data.intressent_id AS person_id,
            count(DISTINCT assignment_data.hjid) AS leadership_roles
           FROM public.assignment_data
          WHERE ((assignment_data.role_code)::text = ANY ((ARRAY['Ordförande'::character varying, 'Vice ordförande'::character varying, 'Talman'::character varying, 'Statsminister'::character varying, 'Vice statsminister'::character varying, 'Partiledare'::character varying, 'Gruppledare'::character varying])::text[]))
          GROUP BY assignment_data.intressent_id
        ), person_documents AS (
         SELECT document_person_reference_da_0.person_reference_id AS person_id,
            count(*) AS documents_authored
           FROM public.document_person_reference_da_0
          GROUP BY document_person_reference_da_0.person_reference_id
        ), career_cycles AS (
         SELECT p.id AS person_id,
            p.first_name,
            p.last_name,
            ...
            COALESCE(max(pl.leadership_roles), (0)::bigint) AS leadership_roles,
            COALESCE(max(pd.documents_authored), (0)::bigint) AS documents_authored
           FROM ((((public.person_data p
             ...
             LEFT JOIN person_leadership pl ON (((pl.person_id)::text = (p.id)::text)))
             LEFT JOIN person_documents pd ON (((pd.person_id)::text = (p.id)::text)))
 ...
 SELECT qa.year,
    ...
    bc.q_stddev_attendance,
        ...
        CASE
            ...
            WHEN (abs(COALESCE((((qa.total_ballots)::numeric - bc.q_baseline_ballots) / NULLIF(bc.q_stddev_ballots, (0)::numeric)), (0)::numeric)) > 1.5) THEN 'ELEVATED'::text
            WHEN (abs(COALESCE((((qa.total_ballots)::numeric - bc.q_baseline_ballots) / NULLIF(bc.q_stddev_ballots, (0)::numeric)), (0)::numeric)) > (1)::numeric) THEN 'NOTABLE'::text
            ELSE 'NORMAL'::text
        END AS activity_classification
...
--
-- Name: view_riksdagen_seasonal_activity_patterns; Type: VIEW; Schema: public; Owner: -
--

CREATE VIEW public.view_riksdagen_seasonal_activity_patterns AS
 WITH election_years AS (
         SELECT unnest(ARRAY[2002, 2006, 2010, 2014, 2018, 2022, 2026]) AS election_year
        ), election_cycle_mapping AS (
         SELECT year.year,
                CASE
                    WHEN ((year.year >= 2002) AND (year.year <= 2005)) THEN '2002-2006'::text
                    WHEN ((year.year >= 2006) AND (year.year <= 2009)) THEN '2006-2010'::text
                    WHEN ((year.year >= 2010) AND (year.year <= 2013)) THEN '2010-2014'::text
                    WHEN ((year.year >= 2014) AND (year.year <= 2017)) THEN '2014-2018'::text
                    WHEN ((year.year >= 2018) AND (year.year <= 2021)) THEN '2018-2022'::text
                    WHEN ((year.year >= 2022) AND (year.year <= 2025)) THEN '2022-2026'::text
                    WHEN (year.year >= 2026) THEN '2026-2030'::text
                    ELSE NULL::text
                END AS election_cycle
           FROM generate_series(2002, 2030) year(year)
        ), seasonal_base AS (
         SELECT sqa.year,
            sqa.quarter,
            sqa.is_election_year,
            ...
            ecm.election_cycle
           FROM (public.view_riksdagen_seasonal_quarterly_activity sqa
             JOIN election_cycle_mapping ecm ON ((sqa.year = (ecm.year)::numeric)))
        ), seasonal_with_trends AS (
         SELECT sb.year,
            sb.quarter,
            sb.is_election_year,
            sb.total_ballots,
            sb.active_politicians,
            sb.attendance_rate,
            sb.documents_produced,
            sb.q_baseline_ballots,
            sb.q_stddev_ballots,
            sb.ballot_z_score,
            sb.q_baseline_docs,
            sb.q_stddev_docs,
            sb.doc_z_score,
            sb.q_baseline_attendance,
            sb.q_stddev_attendance,
            sb.attendance_z_score,
            sb.activity_classification,
            sb.election_cycle,
 ...
 SELECT year,
    ...
    q_stddev_attendance,
    attendance_z_score,
    activity_classification AS base_activity_classification,
    prev_quarter_ballots,
    next_quarter_ballots,
    prev_quarter_attendance,
    prev_quarter_documents,
    cross_year_quarter_avg_ballots,
    cross_year_quarter_stddev_ballots,
    cross_year_quarter_avg_docs,
    activity_quartile_cycle,
        CASE
            WHEN ((prev_quarter_ballots IS NOT NULL) AND (prev_quarter_ballots > 0)) THEN round(((((total_ballots - prev_quarter_ballots))::numeric / (prev_quarter_ballots)::numeric) * (100)::numeric), 2)
            ELSE NULL::numeric
        END AS qoq_ballot_change_pct,
    ((total_ballots)::numeric - cross_year_quarter_avg_ballots) AS deviation_from_typical_quarter,
        CASE
            WHEN (cross_year_quarter_stddev_ballots > (0)::numeric) THEN round((((total_ballots)::numeric - cross_year_quarter_avg_ballots) / cross_year_quarter_stddev_ballots), 2)
            ELSE (0)::numeric
        END AS cross_year_z_score,
        CASE
            WHEN (quarter = (1)::numeric) THEN 'Q1_WINTER'::text
            WHEN (quarter = (2)::numeric) THEN 'Q2_SPRING'::text
            WHEN (quarter = (3)::numeric) THEN 'Q3_SUMMER'::text
            WHEN (quarter = (4)::numeric) THEN 'Q4_AUTUMN'::text
            ELSE NULL::text
        END AS quarter_label,
        CASE
            WHEN (quarter = (1)::numeric) THEN 'Winter Session (Jan-Mar)'::text
            WHEN (quarter = (2)::numeric) THEN 'Spring Session (Apr-Jun)'::text
            WHEN (quarter = (3)::numeric) THEN 'Summer Recess/Election (Jul-Sep)'::text
            WHEN (quarter = (4)::numeric) THEN 'Autumn Session (Oct-Dec)'::text
            ELSE NULL::text
        END AS parliamentary_period,
        CASE
            WHEN ((quarter = (4)::numeric) AND is_election_year AND (ballot_z_score > 1.5)) THEN 'Q4_PRE_ELECTION_SURGE'::text
            WHEN ((quarter = (3)::numeric) AND is_election_year) THEN 'Q3_ELECTION_QUARTER'::text
            WHEN ((quarter = (4)::numeric) AND (ballot_z_score > 1.0)) THEN 'Q4_ELEVATED_ACTIVITY'::text
            WHEN ((quarter = (3)::numeric) AND (ballot_z_score < '-1.0'::numeric)) THEN 'Q3_SUMMER_LULL'::text
            WHEN (ballot_z_score > 1.5) THEN 'UNUSUALLY_HIGH_ACTIVITY'::text
            WHEN (ballot_z_score < '-1.5'::numeric) THEN 'UNUSUALLY_LOW_ACTIVITY'::text
            ELSE 'NORMAL_SEASONAL_PATTERN'::text
...
--
-- Name: idx_mv_annual_document_metrics_year; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_mv_annual_document_metrics_year ON public.mv_annual_document_metrics USING btree (year);


--
-- Name: idx_mv_annual_voting_metrics_year; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX idx_mv_annual_voting_metrics_year ON public.mv_annual_voting_metrics USING btree (year);


--
-- Name: idx_mv_decision_temporal_trends_day; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_mv_decision_temporal_trends_day ON public.mv_decision_temporal_trends USING btree (decision_day);


--
-- Name: idx_mv_decision_temporal_trends_year; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_mv_decision_temporal_trends_year ON public.mv_decision_temporal_trends USING btree (decision_year);


--
-- Name: idx_mv_ministry_decision_impact_year; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_mv_ministry_decision_impact_year ON public.mv_ministry_decision_impact USING btree (decision_year);


--
-- Name: idx_mv_party_decision_flow_party; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_mv_party_decision_flow_party ON public.mv_party_decision_flow USING btree (party);


--
-- Name: idx_mv_party_decision_flow_year; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_mv_party_decision_flow_year ON public.mv_party_decision_flow USING btree (decision_year);


--
-- Name: idx_mv_party_decision_flow_year_party; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX idx_mv_party_decision_flow_year_party ON public.mv_party_decision_flow USING btree (decision_year, party);
...
\unrestrict 4C9WIoMxwGifxVAkQyQgHVKIyTaHuLWCwTN6pemL9wbjiGJJLnpURdsCeNbJDOP

--
-- PostgreSQL database dump
--

\restrict L13uxewN80xjdACn5dbJvgLefssuk1zGEawwJOdmmwSZzqhW0guUfSUgQ7aDuF8
...
1414872417007-1	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.077567	1	EXECUTED	9:a17731710305111df20dcfac7e5b1160	createSequence sequenceName=hibernate_sequence		\N	5.0.1	\N	\N	8446037746
1414872417007-2	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.090479	2	EXECUTED	9:bebfee0168369dd117b920db419f5ba0	createTable tableName=against_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-3	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.102215	3	EXECUTED	9:3b16d9891a4d1d8ea22473bc46c7853c	createTable tableName=against_proposal_data		\N	5.0.1	\N	\N	8446037746
1414872417007-4	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.111253	4	EXECUTED	9:02e3a0b58935a93e8e22b859d6e244ff	createTable tableName=agency		\N	5.0.1	\N	\N	8446037746
1414872417007-5	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.120763	5	EXECUTED	9:3fe4740c8b6b63adf7e6cd409d028222	createTable tableName=aggregated_bug_data		\N	5.0.1	\N	\N	8446037746
1414872417007-6	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.1327	6	EXECUTED	9:cb7c902d90871400b0f50e6517ae2e8b	createTable tableName=aggregated_country_data		\N	5.0.1	\N	\N	8446037746
1414872417007-7	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.145558	7	EXECUTED	9:54b3e671310d1733185a14870793775c	createTable tableName=application_action_event		\N	5.0.1	\N	\N	8446037746
1414872417007-8	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.155444	8	EXECUTED	9:415d9684eeca14269142e3c4af51ef8a	createTable tableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-9	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.169091	9	EXECUTED	9:4e882f95194b32ed19be40bc1c5d81db	createTable tableName=application_view		\N	5.0.1	\N	\N	8446037746
1414872417007-10	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.181264	10	EXECUTED	9:8310007bdad3e542147e9f0c9e6280b2	createTable tableName=assignment_data		\N	5.0.1	\N	\N	8446037746
1414872417007-11	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.191504	11	EXECUTED	9:e2a725018be9c645cc40cee1bf741de4	createTable tableName=assignment_element		\N	5.0.1	\N	\N	8446037746
1414872417007-12	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.201714	12	EXECUTED	9:539351894788ad2df1f65ed4d9dc6394	createTable tableName=committee_document_data		\N	5.0.1	\N	\N	8446037746
1414872417007-13	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.211929	13	EXECUTED	9:0740ea87d5e697e23d68d86cded379f6	createTable tableName=committee_proposal_component_0		\N	5.0.1	\N	\N	8446037746
1414872417007-14	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.224289	14	EXECUTED	9:531e4b3e23b59219af608aa521c60e1d	createTable tableName=committee_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-15	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.234364	15	EXECUTED	9:0cddcb46baaa15f458ceaa0022e10daf	createTable tableName=committee_proposal_data		\N	5.0.1	\N	\N	8446037746
1414872417007-16	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.243291	16	EXECUTED	9:6fd8d778cf34128bd204c56041b7d25f	createTable tableName=countries_element		\N	5.0.1	\N	\N	8446037746
1414872417007-17	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.254678	17	EXECUTED	9:46c38f6afb69bb4d0a78fba4b029f24c	createTable tableName=country_element		\N	5.0.1	\N	\N	8446037746
1414872417007-18	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.266944	18	EXECUTED	9:6906125604ba9910f945c7b07fd92962	createTable tableName=data_element		\N	5.0.1	\N	\N	8446037746
1414872417007-19	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.275532	19	EXECUTED	9:687a141851b84ed8986487e2f3f8e4e7	createTable tableName=data_source_content		\N	5.0.1	\N	\N	8446037746
1414872417007-20	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.284105	20	EXECUTED	9:409c0b3f5c827c4bf3c7421647dadb6a	createTable tableName=detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-21	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.293067	21	EXECUTED	9:37c989d40b81428a93a2e8c29b6d8cb2	createTable tableName=detail_element		\N	5.0.1	\N	\N	8446037746
1414872417007-22	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.303533	22	EXECUTED	9:5553ce8a3c28a583d259d7daafa9492f	createTable tableName=document_activity_container		\N	5.0.1	\N	\N	8446037746
1414872417007-23	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.312647	23	EXECUTED	9:91ca8c9ef2f1a0d4038a8f00ee9cfe48	createTable tableName=document_activity_data		\N	5.0.1	\N	\N	8446037746
1414872417007-24	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.32188	24	EXECUTED	9:6a981fe99433f472f37d626f792fd93e	createTable tableName=document_attachment		\N	5.0.1	\N	\N	8446037746
1414872417007-25	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.33052	25	EXECUTED	9:67651a14797870e3d71860f7886fb744	createTable tableName=document_attachment_container		\N	5.0.1	\N	\N	8446037746
1414872417007-26	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.339394	26	EXECUTED	9:363d579e1857630034ab68bb1ba09a54	createTable tableName=document_container_element		\N	5.0.1	\N	\N	8446037746
1414872417007-27	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.349172	27	EXECUTED	9:9cac16f6bb7fb5311b1c8210c62e8c6b	createTable tableName=document_content_data		\N	5.0.1	\N	\N	8446037746
1414872417007-28	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.3577	28	EXECUTED	9:eb4858110e94dda8090c4a09e2f019f3	createTable tableName=document_data		\N	5.0.1	\N	\N	8446037746
1414872417007-29	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.366147	29	EXECUTED	9:50b71c994cff70c92759390afb6d2b27	createTable tableName=document_detail_container		\N	5.0.1	\N	\N	8446037746
1414872417007-30	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.375091	30	EXECUTED	9:5eb7cc4f4e5da72a6d201a157ba3fed5	createTable tableName=document_detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-31	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.386226	31	EXECUTED	9:9d2fbddccf267f2773209465ff52cd57	createTable tableName=document_element		\N	5.0.1	\N	\N	8446037746
1414872417007-32	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.39437	32	EXECUTED	9:2de9b22e4860ecb9a912012f0cd0e429	createTable tableName=document_person_reference_co_0		\N	5.0.1	\N	\N	8446037746
1414872417007-33	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.404602	33	EXECUTED	9:81a2a16ef0155742bd93f54156b15812	createTable tableName=document_person_reference_da_0		\N	5.0.1	\N	\N	8446037746
1414872417007-34	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.412658	34	EXECUTED	9:65bb5a1b564a86e6817effbab9bde0d3	createTable tableName=document_reference_container		\N	5.0.1	\N	\N	8446037746
1414872417007-35	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.421438	35	EXECUTED	9:b6f271d72874d68d804fbbcc89bedcaa	createTable tableName=document_reference_data		\N	5.0.1	\N	\N	8446037746
1414872417007-36	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.431781	36	EXECUTED	9:d0adee5ea53440f01cf604c105f9aa56	createTable tableName=document_status_container		\N	5.0.1	\N	\N	8446037746
1414872417007-37	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.439679	37	EXECUTED	9:40d45e65bc2539a7d1faf45c6b9af3b2	createTable tableName=domain_portal		\N	5.0.1	\N	\N	8446037746
1414872417007-38	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.450138	38	EXECUTED	9:6a1b6c5054ffe7b83bf458b6d9d63010	createTable tableName=indicator_element		\N	5.0.1	\N	\N	8446037746
1414872417007-39	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.458434	39	EXECUTED	9:df16293062101f9e2f2552533409513c	createTable tableName=indicators_element		\N	5.0.1	\N	\N	8446037746
1414872417007-40	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.46746	40	EXECUTED	9:2924e43a2bfc8ab8ece4c7b7fc886751	createTable tableName=language_content_data		\N	5.0.1	\N	\N	8446037746
1414872417007-41	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.477476	41	EXECUTED	9:ad01e46b6ba2c604286127713ac8dbbd	createTable tableName=language_data		\N	5.0.1	\N	\N	8446037746
1414872417007-42	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.485606	42	EXECUTED	9:79ef7742f91be7455bd9dfa79657131b	createTable tableName=operational_information_cont_0		\N	5.0.1	\N	\N	8446037746
1414872417007-43	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.495584	43	EXECUTED	9:fa41612149189e9606f6ef67cbb39911	createTable tableName=performance_indicator_content		\N	5.0.1	\N	\N	8446037746
1414872417007-44	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.503524	44	EXECUTED	9:0e9e417a0f56444002cbcd5958e1263e	createTable tableName=person_assignment_data		\N	5.0.1	\N	\N	8446037746
1414872417007-45	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.511576	45	EXECUTED	9:2fefd56ec64a6b4c49b1def045d6396a	createTable tableName=person_assignment_element		\N	5.0.1	\N	\N	8446037746
1414872417007-46	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.52321	46	EXECUTED	9:9a28a213df35828e2be714b4d835e8d7	createTable tableName=person_container_data		\N	5.0.1	\N	\N	8446037746
1414872417007-47	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.53137	47	EXECUTED	9:1c0bac7e7ad922b52a618676b42062a8	createTable tableName=person_container_element		\N	5.0.1	\N	\N	8446037746
1414872417007-48	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.541976	48	EXECUTED	9:10b817f4fcc09c63904d708e1461c5b7	createTable tableName=person_data		\N	5.0.1	\N	\N	8446037746
1414872417007-49	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.55031	49	EXECUTED	9:38d2239d109bf8b3b63dced29595a4f0	createTable tableName=person_detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-50	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.558744	50	EXECUTED	9:0cfe316d63aba7817bbc73a2430ff7dc	createTable tableName=person_detail_element		\N	5.0.1	\N	\N	8446037746
1414872417007-51	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.568953	51	EXECUTED	9:ad721b60c177dedbbc3fdf5ccbefa90c	createTable tableName=person_element		\N	5.0.1	\N	\N	8446037746
1414872417007-52	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.581244	52	EXECUTED	9:96452dd1a3d5f9b7484eee940aa6c845	createTable tableName=portal		\N	5.0.1	\N	\N	8446037746
1414872417007-53	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.589944	53	EXECUTED	9:4bd5e7ade89c1e867b432998577ee25a	createTable tableName=quality_assurance_content		\N	5.0.1	\N	\N	8446037746
1414872417007-54	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.599861	54	EXECUTED	9:8d843eb1a26a4558446e4be5914dd5fe	createTable tableName=sweden_county_data		\N	5.0.1	\N	\N	8446037746
1414872417007-55	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.612019	55	EXECUTED	9:a671b76e045284f9ed125b93cd347528	createTable tableName=sweden_county_data_container		\N	5.0.1	\N	\N	8446037746
2414872417007-327	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.2992	318	EXECUTED	9:49044a256f906a54d7ccb6cdf3eeba70	addColumn tableName=rule_violation		\N	5.0.1	\N	\N	8446037746
1414872417007-56	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.62431	56	EXECUTED	9:5d88b4f2bc6c9b00fd7dcfacb1848770	createTable tableName=sweden_county_electoral_area		\N	5.0.1	\N	\N	8446037746
1414872417007-57	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.633032	57	EXECUTED	9:c2b1be2b1568b0adffeadb4e60e07aa4	createTable tableName=sweden_county_electoral_regi_0		\N	5.0.1	\N	\N	8446037746
1414872417007-58	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.642223	58	EXECUTED	9:df3cb3fdfce9bf4f6805fb84e1b9ed8a	createTable tableName=sweden_county_electoral_regi_1		\N	5.0.1	\N	\N	8446037746
1414872417007-59	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.655133	59	EXECUTED	9:d14dfb21a1e78aad5fb77cb561fce391	createTable tableName=sweden_election_region		\N	5.0.1	\N	\N	8446037746
1414872417007-60	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.663517	60	EXECUTED	9:3bba4dd4cd8837bef66ccbad72ba7e48	createTable tableName=sweden_election_type		\N	5.0.1	\N	\N	8446037746
1414872417007-61	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.672581	61	EXECUTED	9:5324df8331f298acdc38fd7cf12b8258	createTable tableName=sweden_election_type_contain_0		\N	5.0.1	\N	\N	8446037746
1414872417007-62	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.680829	62	EXECUTED	9:633ea9271e4ae43b4821f604df1f59fa	createTable tableName=sweden_municipality_data		\N	5.0.1	\N	\N	8446037746
1414872417007-63	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.689342	63	EXECUTED	9:6d1944c67dd25f3a0648f9b1a5aa38b4	createTable tableName=sweden_municipality_election_0		\N	5.0.1	\N	\N	8446037746
1414872417007-64	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.702235	64	EXECUTED	9:b95825c026136a5346b16b9353ac5096	createTable tableName=sweden_parliament_electoral__0		\N	5.0.1	\N	\N	8446037746
1414872417007-65	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.71532	65	EXECUTED	9:08c4da69bcd3b32d1f3f73121e4cf374	createTable tableName=sweden_parliament_electoral__1		\N	5.0.1	\N	\N	8446037746
1414872417007-66	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.726525	66	EXECUTED	9:39166b5937a6de6e1e119be89a5768d4	createTable tableName=sweden_political_party		\N	5.0.1	\N	\N	8446037746
1414872417007-67	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.734669	67	EXECUTED	9:206e57b2f1ab489aedd5f7f77b76f5e8	createTable tableName=target_profile_content		\N	5.0.1	\N	\N	8446037746
1414872417007-68	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.74528	68	EXECUTED	9:36d1a12fecba42815db61876f9ffee82	createTable tableName=topic		\N	5.0.1	\N	\N	8446037746
1414872417007-69	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.753339	69	EXECUTED	9:74df0d4b5c616a83e5949698a4e5a8cc	createTable tableName=topics		\N	5.0.1	\N	\N	8446037746
1414872417007-70	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.763561	70	EXECUTED	9:0b270579251a30288c9ed0f7ce9cff3f	createTable tableName=user_account		\N	5.0.1	\N	\N	8446037746
1414872417007-71	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.771519	71	EXECUTED	9:d6f00140ffba825b869701ef3f17acc8	createTable tableName=user_account_address		\N	5.0.1	\N	\N	8446037746
1414872417007-72	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.780707	72	EXECUTED	9:3daaf3a53dcab5c0643c4d341da21ec7	createTable tableName=vote_data		\N	5.0.1	\N	\N	8446037746
1414872417007-73	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.790916	73	EXECUTED	9:f846b11d91b49720ec2c460922d362f7	createTable tableName=vote_meta_data		\N	5.0.1	\N	\N	8446037746
1414872417007-74	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.799401	74	EXECUTED	9:aa5adf76544d09e8999dabd5bba4718d	createTable tableName=world_bank_data		\N	5.0.1	\N	\N	8446037746
1414872417007-75	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.809185	75	EXECUTED	9:5f830995c29a8f2e62b79c93b739826a	addPrimaryKey constraintName=against_proposal_container_pkey, tableName=against_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-76	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.817241	76	EXECUTED	9:bba09af773c137dde53bcec477dc7be1	addPrimaryKey constraintName=against_proposal_data_pkey, tableName=against_proposal_data		\N	5.0.1	\N	\N	8446037746
1414872417007-77	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.825545	77	EXECUTED	9:e1e45c8df3139a437fa9282e167b2a49	addPrimaryKey constraintName=agency_pkey, tableName=agency		\N	5.0.1	\N	\N	8446037746
1414872417007-78	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.834181	78	EXECUTED	9:74ab57d4e742f0f890d089863fe99534	addPrimaryKey constraintName=aggregated_bug_data_pkey, tableName=aggregated_bug_data		\N	5.0.1	\N	\N	8446037746
1414872417007-79	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.842254	79	EXECUTED	9:43c9e9f0ca5431b8014b60e80d39d58f	addPrimaryKey constraintName=aggregated_country_data_pkey, tableName=aggregated_country_data		\N	5.0.1	\N	\N	8446037746
1414872417007-80	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.854602	80	EXECUTED	9:e40121efa7c2cb31c5579a83867222b9	addPrimaryKey constraintName=application_action_event_pkey, tableName=application_action_event		\N	5.0.1	\N	\N	8446037746
1414872417007-81	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.862652	81	EXECUTED	9:5e7a0cc84a0279421abc59b37fac7cbc	addPrimaryKey constraintName=application_session_pkey, tableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-82	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.872551	82	EXECUTED	9:8e761f77ad53316ecde3ae62be2f56e5	addPrimaryKey constraintName=application_view_pkey, tableName=application_view		\N	5.0.1	\N	\N	8446037746
1414872417007-83	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.882113	83	EXECUTED	9:d65482d7b31647dc4109f47a336ca0ba	addPrimaryKey constraintName=assignment_data_pkey, tableName=assignment_data		\N	5.0.1	\N	\N	8446037746
1414872417007-84	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.88988	84	EXECUTED	9:5fd2d99287f3fc7dc27fdeefa086f79c	addPrimaryKey constraintName=assignment_element_pkey, tableName=assignment_element		\N	5.0.1	\N	\N	8446037746
1414872417007-85	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.897691	85	EXECUTED	9:ee126e8d0870490635cafe2ce677b999	addPrimaryKey constraintName=committee_document_data_pkey, tableName=committee_document_data		\N	5.0.1	\N	\N	8446037746
1414872417007-86	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.905338	86	EXECUTED	9:80032ba89b6bd19cfd978fa3956b3863	addPrimaryKey constraintName=committee_proposal_component_0_pkey, tableName=committee_proposal_component_0		\N	5.0.1	\N	\N	8446037746
1414872417007-87	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.915523	87	EXECUTED	9:233f9d1a6e53489679d9a65fe698416a	addPrimaryKey constraintName=committee_proposal_container_pkey, tableName=committee_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-88	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.923356	88	EXECUTED	9:997901d2eee29b9ec4d46710ab3131a4	addPrimaryKey constraintName=committee_proposal_data_pkey, tableName=committee_proposal_data		\N	5.0.1	\N	\N	8446037746
1414872417007-89	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.933193	89	EXECUTED	9:c1f8f3587b607f95154d9d70467c9c5e	addPrimaryKey constraintName=countries_element_pkey, tableName=countries_element		\N	5.0.1	\N	\N	8446037746
1414872417007-90	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.941208	90	EXECUTED	9:40ccd4df593a2a66cd59300c2f5c7f15	addPrimaryKey constraintName=country_element_pkey, tableName=country_element		\N	5.0.1	\N	\N	8446037746
1414872417007-91	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.949457	91	EXECUTED	9:8308944e4e2c08b501b936a559f8afe1	addPrimaryKey constraintName=data_element_pkey, tableName=data_element		\N	5.0.1	\N	\N	8446037746
1414872417007-92	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.96127	92	EXECUTED	9:dfe68f89fa66095a18d2bb701148737f	addPrimaryKey constraintName=data_source_content_pkey, tableName=data_source_content		\N	5.0.1	\N	\N	8446037746
1414872417007-93	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.969134	93	EXECUTED	9:100740b8d9c1ea7e9ff9e242ab5f091f	addPrimaryKey constraintName=detail_data_pkey, tableName=detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-94	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.978737	94	EXECUTED	9:155d55d6251bd89a8ef243dd207ff3dd	addPrimaryKey constraintName=detail_element_pkey, tableName=detail_element		\N	5.0.1	\N	\N	8446037746
1414872417007-95	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.98623	95	EXECUTED	9:1f5717b882823e279aab6ce2f3e14ff5	addPrimaryKey constraintName=document_activity_container_pkey, tableName=document_activity_container		\N	5.0.1	\N	\N	8446037746
1414872417007-96	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:39.993781	96	EXECUTED	9:0f77822e71c514105619468165aae6d5	addPrimaryKey constraintName=document_activity_data_pkey, tableName=document_activity_data		\N	5.0.1	\N	\N	8446037746
1414872417007-97	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.00134	97	EXECUTED	9:cd6f418a41124aea3c186ee765665519	addPrimaryKey constraintName=document_attachment_container_pkey, tableName=document_attachment_container		\N	5.0.1	\N	\N	8446037746
1414872417007-98	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.008965	98	EXECUTED	9:b5d8dffc4ae85d22f7f4787d36043f4b	addPrimaryKey constraintName=document_attachment_pkey, tableName=document_attachment		\N	5.0.1	\N	\N	8446037746
1414872417007-99	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.018526	99	EXECUTED	9:2bfd0b28a7b11c92c03453704e522bda	addPrimaryKey constraintName=document_container_element_pkey, tableName=document_container_element		\N	5.0.1	\N	\N	8446037746
1414872417007-100	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.026293	100	EXECUTED	9:6395d6fc88ce72da42c56ff1c5e07867	addPrimaryKey constraintName=document_content_data_pkey, tableName=document_content_data		\N	5.0.1	\N	\N	8446037746
1414872417007-101	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.036508	101	EXECUTED	9:b32a6f1fe5048a5e0e85a05e111727e5	addPrimaryKey constraintName=document_data_pkey, tableName=document_data		\N	5.0.1	\N	\N	8446037746
1414872417007-102	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.044449	102	EXECUTED	9:2d9aaf92aee97b16ac125a5fc0e8dfcb	addPrimaryKey constraintName=document_detail_container_pkey, tableName=document_detail_container		\N	5.0.1	\N	\N	8446037746
1414872417007-103	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.052399	103	EXECUTED	9:860b27b44274c68226953af12bb7c2ba	addPrimaryKey constraintName=document_detail_data_pkey, tableName=document_detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-104	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.059947	104	EXECUTED	9:9f5914ca5b49f8cf29c24b7a85407e6d	addPrimaryKey constraintName=document_element_pkey, tableName=document_element		\N	5.0.1	\N	\N	8446037746
1414872417007-105	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.069685	105	EXECUTED	9:646fe77744ba782ad29a05f9758c726b	addPrimaryKey constraintName=document_person_reference_co_0_pkey, tableName=document_person_reference_co_0		\N	5.0.1	\N	\N	8446037746
1414872417007-106	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.079495	106	EXECUTED	9:066a0972b05e23920e12face81e6578b	addPrimaryKey constraintName=document_person_reference_da_0_pkey, tableName=document_person_reference_da_0		\N	5.0.1	\N	\N	8446037746
1414872417007-107	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.087484	107	EXECUTED	9:624552adc8948bf35b58b1d9ef3a543a	addPrimaryKey constraintName=document_reference_container_pkey, tableName=document_reference_container		\N	5.0.1	\N	\N	8446037746
1414872417007-108	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.095412	108	EXECUTED	9:9517d4a16d854bb77a5a8364e456c6fa	addPrimaryKey constraintName=document_reference_data_pkey, tableName=document_reference_data		\N	5.0.1	\N	\N	8446037746
1414872417007-109	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.103124	109	EXECUTED	9:9c3886a2a01a213cc0388b42cefa9c31	addPrimaryKey constraintName=document_status_container_pkey, tableName=document_status_container		\N	5.0.1	\N	\N	8446037746
1414872417007-110	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.11286	110	EXECUTED	9:5a47eb0ef2899df6880ccf0181487f36	addPrimaryKey constraintName=domain_portal_pkey, tableName=domain_portal		\N	5.0.1	\N	\N	8446037746
1414872417007-111	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.120488	111	EXECUTED	9:9bb637485c9c31166e28ee005df28382	addPrimaryKey constraintName=indicator_element_pkey, tableName=indicator_element		\N	5.0.1	\N	\N	8446037746
1414872417007-112	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.130579	112	EXECUTED	9:94c16cc4cb00a952a848f47b4d537f35	addPrimaryKey constraintName=indicators_element_pkey, tableName=indicators_element		\N	5.0.1	\N	\N	8446037746
1414872417007-113	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.138104	113	EXECUTED	9:9008e44a163d1c21e27bfc226f5f3637	addPrimaryKey constraintName=language_content_data_pkey, tableName=language_content_data		\N	5.0.1	\N	\N	8446037746
1414872417007-114	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.145703	114	EXECUTED	9:c883a7c469d1ebb67a81bf9e174ab4cf	addPrimaryKey constraintName=language_data_pkey, tableName=language_data		\N	5.0.1	\N	\N	8446037746
1414872417007-115	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.153476	115	EXECUTED	9:13ceee93ff17b702a450945b3bcb3249	addPrimaryKey constraintName=operational_information_cont_0_pkey, tableName=operational_information_cont_0		\N	5.0.1	\N	\N	8446037746
1414872417007-116	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.164274	116	EXECUTED	9:1524d7c6bd18a8d8a719c4f573486968	addPrimaryKey constraintName=performance_indicator_content_pkey, tableName=performance_indicator_content		\N	5.0.1	\N	\N	8446037746
1414872417007-117	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.173534	117	EXECUTED	9:04a2103790734cfce99dff36d2609ef1	addPrimaryKey constraintName=person_assignment_data_pkey, tableName=person_assignment_data		\N	5.0.1	\N	\N	8446037746
1414872417007-118	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.181388	118	EXECUTED	9:1ab775e2f146e761344cdb951894eae3	addPrimaryKey constraintName=person_assignment_element_pkey, tableName=person_assignment_element		\N	5.0.1	\N	\N	8446037746
1414872417007-119	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.189102	119	EXECUTED	9:9b2fa87e2c12c84e1f4a6cfcbf20c839	addPrimaryKey constraintName=person_container_data_pkey, tableName=person_container_data		\N	5.0.1	\N	\N	8446037746
1414872417007-120	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.196952	120	EXECUTED	9:efc05607aaa1e96f1c09107d6f334c8e	addPrimaryKey constraintName=person_container_element_pkey, tableName=person_container_element		\N	5.0.1	\N	\N	8446037746
1414872417007-121	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.204576	121	EXECUTED	9:6e35a4dcd47a49efd33421bc9f20fb66	addPrimaryKey constraintName=person_data_pkey, tableName=person_data		\N	5.0.1	\N	\N	8446037746
1414872417007-122	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.216138	122	EXECUTED	9:cf067de81b62440865e1ceb695d746fa	addPrimaryKey constraintName=person_detail_data_pkey, tableName=person_detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-123	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.224199	123	EXECUTED	9:4ecec7b11ded214c249fa3330edac6b0	addPrimaryKey constraintName=person_detail_element_pkey, tableName=person_detail_element		\N	5.0.1	\N	\N	8446037746
1414872417007-124	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.233526	124	EXECUTED	9:2cad1510173ec69b6b8cb232a97eab9b	addPrimaryKey constraintName=person_element_pkey, tableName=person_element		\N	5.0.1	\N	\N	8446037746
1414872417007-125	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.241535	125	EXECUTED	9:0d5aa0b601f6823fdeeb80895c8f0d28	addPrimaryKey constraintName=portal_pkey, tableName=portal		\N	5.0.1	\N	\N	8446037746
1414872417007-126	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.249445	126	EXECUTED	9:5e6770a50d8544af0afcb6c0d720715b	addPrimaryKey constraintName=quality_assurance_content_pkey, tableName=quality_assurance_content		\N	5.0.1	\N	\N	8446037746
1414872417007-127	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.257388	127	EXECUTED	9:b4e0e9778b2d4deeb4ece26f737744c5	addPrimaryKey constraintName=sweden_county_data_container_pkey, tableName=sweden_county_data_container		\N	5.0.1	\N	\N	8446037746
1414872417007-128	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.267048	128	EXECUTED	9:62edab145f5744b7ceb49858dc83eddd	addPrimaryKey constraintName=sweden_county_data_pkey, tableName=sweden_county_data		\N	5.0.1	\N	\N	8446037746
1414872417007-129	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.276492	129	EXECUTED	9:f29e2b6f4276458aa0d2337f97c7f320	addPrimaryKey constraintName=sweden_county_electoral_area_pkey, tableName=sweden_county_electoral_area		\N	5.0.1	\N	\N	8446037746
1414872417007-130	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.284327	130	EXECUTED	9:adf18b3ce957d1710d3a3337a3b3f995	addPrimaryKey constraintName=sweden_county_electoral_regi_0_pkey, tableName=sweden_county_electoral_regi_0		\N	5.0.1	\N	\N	8446037746
1414872417007-131	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.292127	131	EXECUTED	9:b1175e4a6a86b2a7013d246073480c44	addPrimaryKey constraintName=sweden_county_electoral_regi_1_pkey, tableName=sweden_county_electoral_regi_1		\N	5.0.1	\N	\N	8446037746
1414872417007-132	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.303472	132	EXECUTED	9:0ef7e7a7c0d5cda0a5dcbe5db56b7dfb	addPrimaryKey constraintName=sweden_election_region_pkey, tableName=sweden_election_region		\N	5.0.1	\N	\N	8446037746
1414872417007-133	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.313221	133	EXECUTED	9:95d0312014ad80f98a928a2e1ab77b6c	addPrimaryKey constraintName=sweden_election_type_contain_0_pkey, tableName=sweden_election_type_contain_0		\N	5.0.1	\N	\N	8446037746
1414872417007-134	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.320792	134	EXECUTED	9:2b80a19ab63216f2135737f548649efb	addPrimaryKey constraintName=sweden_election_type_pkey, tableName=sweden_election_type		\N	5.0.1	\N	\N	8446037746
1414872417007-135	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.328278	135	EXECUTED	9:d61ad75b5a17205bd28e961c19f9bdc4	addPrimaryKey constraintName=sweden_municipality_data_pkey, tableName=sweden_municipality_data		\N	5.0.1	\N	\N	8446037746
1414872417007-136	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.336194	136	EXECUTED	9:a9b60ba996dfcc86c556ecefd9434d07	addPrimaryKey constraintName=sweden_municipality_election_0_pkey, tableName=sweden_municipality_election_0		\N	5.0.1	\N	\N	8446037746
1414872417007-137	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.344001	137	EXECUTED	9:270f67f6d0e7885e13a29d873fb10753	addPrimaryKey constraintName=sweden_parliament_electoral__0_pkey, tableName=sweden_parliament_electoral__0		\N	5.0.1	\N	\N	8446037746
1414872417007-138	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.355715	138	EXECUTED	9:17e8429424a5fc7b90273685a0fee7a8	addPrimaryKey constraintName=sweden_parliament_electoral__1_pkey, tableName=sweden_parliament_electoral__1		\N	5.0.1	\N	\N	8446037746
1414872417007-139	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.363814	139	EXECUTED	9:05fc213ff95f0bacb5db6c1d213eb41a	addPrimaryKey constraintName=sweden_political_party_pkey, tableName=sweden_political_party		\N	5.0.1	\N	\N	8446037746
1414872417007-140	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.373736	140	EXECUTED	9:09a05025b9436e9f557c7fac3ab6d6a1	addPrimaryKey constraintName=target_profile_content_pkey, tableName=target_profile_content		\N	5.0.1	\N	\N	8446037746
1414872417007-141	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.381323	141	EXECUTED	9:d9a0d2ae0b49b01b86000dff4c08f92c	addPrimaryKey constraintName=topic_pkey, tableName=topic		\N	5.0.1	\N	\N	8446037746
1414872417007-142	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.389183	142	EXECUTED	9:c25699beb6697e89e15974720c428e76	addPrimaryKey constraintName=topics_pkey, tableName=topics		\N	5.0.1	\N	\N	8446037746
1414872417007-143	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.397266	143	EXECUTED	9:af73822f902ba605fc64ad455d4719d9	addPrimaryKey constraintName=user_account_address_pkey, tableName=user_account_address		\N	5.0.1	\N	\N	8446037746
1414872417007-144	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.405104	144	EXECUTED	9:8fcbc92628f8cba467c582d5ca7906e7	addPrimaryKey constraintName=user_account_pkey, tableName=user_account		\N	5.0.1	\N	\N	8446037746
1414872417007-145	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.416236	145	EXECUTED	9:f0838a94de3419c7ac3df04bd792c658	addPrimaryKey constraintName=vote_data_pkey, tableName=vote_data		\N	5.0.1	\N	\N	8446037746
1414872417007-146	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.425522	146	EXECUTED	9:5c275b66b7eed679e55738926e5b1536	addPrimaryKey constraintName=vote_meta_data_pkey, tableName=vote_meta_data		\N	5.0.1	\N	\N	8446037746
1414872417007-147	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.43345	147	EXECUTED	9:c43ec2fd8137eee136dc1c20828cbd70	addPrimaryKey constraintName=world_bank_data_pkey, tableName=world_bank_data		\N	5.0.1	\N	\N	8446037746
1414872417007-148	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.442562	148	EXECUTED	9:4508dab76977f081c98a65d7653e84b7	addForeignKeyConstraint baseTableName=person_element, constraintName=fk_13jay3yk8opnt33758httu9kb, referencedTableName=person_detail_element		\N	5.0.1	\N	\N	8446037746
1414872417007-149	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.451551	149	EXECUTED	9:91b8623f7717a1df4b4a2e6ca8b36512	addForeignKeyConstraint baseTableName=application_view, constraintName=fk_2ivjcdwosa63ant7jc5c6cojj, referencedTableName=target_profile_content		\N	5.0.1	\N	\N	8446037746
1414872417007-150	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.462486	150	EXECUTED	9:a42e80d7f82938b0012152684357dfbd	addForeignKeyConstraint baseTableName=country_element, constraintName=fk_3k0s1gih1msbej3bp2iotg52y, referencedTableName=countries_element		\N	5.0.1	\N	\N	8446037746
1414872417007-151	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.474431	151	EXECUTED	9:6590b1db53965caf6f41491e5d14069c	addForeignKeyConstraint baseTableName=person_element, constraintName=fk_3o85sqp9yss0nler1yl1umfl1, referencedTableName=person_container_element		\N	5.0.1	\N	\N	8446037746
1414872417007-152	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.482628	152	EXECUTED	9:5666684899b4932d7a7c5f10650b1504	addForeignKeyConstraint baseTableName=sweden_county_electoral_regi_1, constraintName=fk_4y4vi3cafmbdhvckntfn7qdps, referencedTableName=sweden_county_electoral_regi_0		\N	5.0.1	\N	\N	8446037746
1414872417007-153	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.490923	153	EXECUTED	9:5d7d31451a54ac259a8da618470e3019	addForeignKeyConstraint baseTableName=against_proposal_data, constraintName=fk_5u5u77qsrpa2qy6umqrph4tyf, referencedTableName=against_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-154	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.501636	154	EXECUTED	9:b5f757faef90de046c4cfeba596b2185	addForeignKeyConstraint baseTableName=person_container_data, constraintName=fk_5w4uvrhl3l7c441b7ra7p8txr, referencedTableName=person_data		\N	5.0.1	\N	\N	8446037746
1414872417007-155	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.509467	155	EXECUTED	9:d1716def0fa248256460135662cb0605	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_6crp887w8xy3e4i143yyydjqv, referencedTableName=document_reference_container		\N	5.0.1	\N	\N	8446037746
1414872417007-156	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.519878	156	EXECUTED	9:87739cc9152ce23f5b403ba407615209	addForeignKeyConstraint baseTableName=portal, constraintName=fk_7c8jfw8bnxrm2aj26w9qlx340, referencedTableName=agency		\N	5.0.1	\N	\N	8446037746
1414872417007-157	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.527586	157	EXECUTED	9:285821ed49a91195d9d6e0f0273b0778	addForeignKeyConstraint baseTableName=sweden_county_electoral_area, constraintName=fk_7h4feuc5bwu12tyaka1nx1ln, referencedTableName=sweden_county_electoral_regi_1		\N	5.0.1	\N	\N	8446037746
1414872417007-158	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.535505	158	EXECUTED	9:29999cfa40cbc2af86e5e6f41ce1ff4f	addForeignKeyConstraint baseTableName=assignment_data, constraintName=fk_84o1dcsfeyp1o25nfdpppa7oe, referencedTableName=person_assignment_data		\N	5.0.1	\N	\N	8446037746
1414872417007-159	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.547248	159	EXECUTED	9:04f0321d6a659c24367adf06b21490ec	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_86c52yf22uk0bpcs1qoc3aeyv, referencedTableName=document_attachment_container		\N	5.0.1	\N	\N	8446037746
1414872417007-160	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.555182	160	EXECUTED	9:cfb97bbb6da4e55006169e9879f04eea	addForeignKeyConstraint baseTableName=user_account_address, constraintName=fk_8931ymg13vy6vfkrichtst7bj, referencedTableName=user_account		\N	5.0.1	\N	\N	8446037746
1414872417007-161	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.565213	161	EXECUTED	9:35323e79371e0ed674f60d54da88447e	addForeignKeyConstraint baseTableName=indicator_element, constraintName=fk_8l1m1pum4e3catw4443rup4q5, referencedTableName=indicators_element		\N	5.0.1	\N	\N	8446037746
1414872417007-162	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.573081	162	EXECUTED	9:d5d46f2c32f0828c1ad2cab7488c0ced	addForeignKeyConstraint baseTableName=user_account, constraintName=fk_8mmnmcgjut9nc7dfhrgxi598f, referencedTableName=aggregated_country_data		\N	5.0.1	\N	\N	8446037746
1414872417007-163	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.580742	163	EXECUTED	9:1e64c99d1c965da030f1f77b7c65d3bd	addForeignKeyConstraint baseTableName=committee_proposal_component_0, constraintName=fk_90arga58ce9bnjkc6lws04uhw, referencedTableName=committee_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-164	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.588287	164	EXECUTED	9:4ff16e8d056a87f4c3ccd13acc819aa5	addForeignKeyConstraint baseTableName=indicator_element, constraintName=fk_92h99v4i1pmr69x0y43pocv2a, referencedTableName=topics		\N	5.0.1	\N	\N	8446037746
1414872417007-165	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.598159	165	EXECUTED	9:f030785453270c20bfdb38ca4dc425ee	addForeignKeyConstraint baseTableName=domain_portal, constraintName=fk_9ln0n5axxjuxtbpepyad69rel, referencedTableName=portal		\N	5.0.1	\N	\N	8446037746
1414872417007-166	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.607923	166	EXECUTED	9:4dacfe145969063359c67da63be19926	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_9q1ktfb77gieq0xugoqe2fidd, referencedTableName=document_detail_container		\N	5.0.1	\N	\N	8446037746
1414872417007-167	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.616193	167	EXECUTED	9:f9d7c86e3c465fe6034392bcaa0261a8	addForeignKeyConstraint baseTableName=application_view, constraintName=fk_9x5havflf3rdfkaw1hangbemd, referencedTableName=quality_assurance_content		\N	5.0.1	\N	\N	8446037746
1414872417007-168	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.624592	168	EXECUTED	9:120cbf4620307c3f7962a3727eea6d93	addForeignKeyConstraint baseTableName=person_detail_element, constraintName=fk_a6syxeadcisfnnfjqemog93qd, referencedTableName=detail_element		\N	5.0.1	\N	\N	8446037746
1414872417007-169	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.632635	169	EXECUTED	9:8ddf896e49f7932380e5ef7471892039	addForeignKeyConstraint baseTableName=sweden_political_party, constraintName=fk_c2f4dhdce9p61sg50rnww73c1, referencedTableName=sweden_election_region		\N	5.0.1	\N	\N	8446037746
1414872417007-170	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.643955	170	EXECUTED	9:2d5e1c62af16eec4c4539ddfb9100fec	addForeignKeyConstraint baseTableName=document_reference_data, constraintName=fk_c4uqb4d6xqa5d7afwen8sny67, referencedTableName=document_reference_container		\N	5.0.1	\N	\N	8446037746
1414872417007-171	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.653348	171	EXECUTED	9:4e6aea0b15d2a2ed7264ed313d9547be	addForeignKeyConstraint baseTableName=detail_data, constraintName=fk_diexjlb9hdrfv7g5y06cj6nu5, referencedTableName=person_detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-172	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.660869	172	EXECUTED	9:e8560158fc903b2b52877dcc82cadbed	addForeignKeyConstraint baseTableName=world_bank_data, constraintName=fk_e0yghurnnhmkahpt7ydf008fo, referencedTableName=data_element		\N	5.0.1	\N	\N	8446037746
1414872417007-173	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.668138	173	EXECUTED	9:cbec00c068a73c3e5d6597c95e437200	addForeignKeyConstraint baseTableName=committee_proposal_component_0, constraintName=fk_eofapva6jn5k3h5gnj4whyilb, referencedTableName=against_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-174	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.677683	174	EXECUTED	9:ad7250e1d861b489dd7ca50bb3f49df2	addForeignKeyConstraint baseTableName=application_view, constraintName=fk_f4bptktby95bygv359chn7lbn, referencedTableName=performance_indicator_content		\N	5.0.1	\N	\N	8446037746
1414872417007-175	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.689547	175	EXECUTED	9:818f5d9ba52f5b4b7c5e22b7880d5b31	addForeignKeyConstraint baseTableName=document_activity_data, constraintName=fk_gruc53dqu0smf6s1a0gkelvdm, referencedTableName=document_activity_container		\N	5.0.1	\N	\N	8446037746
1414872417007-176	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.699653	176	EXECUTED	9:2829a37686f398cbd8bde3720b9ffbcb	addForeignKeyConstraint baseTableName=sweden_municipality_data, constraintName=fk_gykahsnks6y9v8y5novlxagnf, referencedTableName=sweden_county_data		\N	5.0.1	\N	\N	8446037746
1414872417007-177	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.707322	177	EXECUTED	9:74506407fb2de37ce7110ed9e6e84899	addForeignKeyConstraint baseTableName=committee_proposal_data, constraintName=fk_hs04ji7kqvwd7313ryp20vo0x, referencedTableName=committee_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-178	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.715537	178	EXECUTED	9:ff2b41caa6a89d740f2df18f82af73e5	addForeignKeyConstraint baseTableName=aggregated_country_data, constraintName=fk_j7l4eldeihr8g7ax7rv2irgk1, referencedTableName=country_element		\N	5.0.1	\N	\N	8446037746
1414872417007-179	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.727076	179	EXECUTED	9:de72bc11dc8b5b4e708484799424e2f3	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_jjcxsqmdnjw0nbwducjyecdg4, referencedTableName=document_data		\N	5.0.1	\N	\N	8446037746
1414872417007-180	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.734878	180	EXECUTED	9:dd951cd80fae1d68360c4f14a0f8ae47	addForeignKeyConstraint baseTableName=person_data, constraintName=fk_jrgy7nw6n071uok8p1hkp03rh, referencedTableName=person_detail_data		\N	5.0.1	\N	\N	8446037746
1414872417007-181	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.74548	181	EXECUTED	9:54660e233cda4814c9a51132ed672342	addForeignKeyConstraint baseTableName=committee_proposal_component_0, constraintName=fk_k78eqmx2m3ja0267xhthfeio4, referencedTableName=committee_document_data		\N	5.0.1	\N	\N	8446037746
1414872417007-182	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.753793	182	EXECUTED	9:7300fd0907004ada150fc96cb7a0c74c	addForeignKeyConstraint baseTableName=assignment_element, constraintName=fk_ks1811fwuno6vqof0lskerpib, referencedTableName=person_assignment_element		\N	5.0.1	\N	\N	8446037746
1414872417007-183	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.761712	183	EXECUTED	9:60e718f175e5b30a169916f114881495	addForeignKeyConstraint baseTableName=document_attachment, constraintName=fk_lean1i0p0e5rv28my9297lq22, referencedTableName=document_attachment_container		\N	5.0.1	\N	\N	8446037746
1414872417007-184	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.772465	184	EXECUTED	9:06e2186d554b8ab401562b3ad4b848ae	addForeignKeyConstraint baseTableName=document_person_reference_da_0, constraintName=fk_lsfup3rosph7239t1idorm1cd, referencedTableName=document_person_reference_co_0		\N	5.0.1	\N	\N	8446037746
1414872417007-185	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.780264	185	EXECUTED	9:aa2334e93f5fe2a5ed438f52c7507776	addForeignKeyConstraint baseTableName=person_element, constraintName=fk_m6dcdojsb6iv9lrego5kurr7p, referencedTableName=person_assignment_element		\N	5.0.1	\N	\N	8446037746
1414872417007-186	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.789636	186	EXECUTED	9:cc82b3c05a5e51eedf05c87248cea759	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_ng4kjnv3cm4e6ud3fikwi6p7i, referencedTableName=document_person_reference_co_0		\N	5.0.1	\N	\N	8446037746
1414872417007-187	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.797723	187	EXECUTED	9:5eba99c12fbddf7df67ef71faf1a83de	addForeignKeyConstraint baseTableName=application_action_event, constraintName=fk_nlqlshlogsx2g8u5d3y28my28, referencedTableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-188	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.805605	188	EXECUTED	9:09fb71b57990439cfb427066fe1efddc	addForeignKeyConstraint baseTableName=document_element, constraintName=fk_o24n54auwa2xyflis6nkrajpd, referencedTableName=document_container_element		\N	5.0.1	\N	\N	8446037746
1414872417007-189	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.81708	189	EXECUTED	9:6e0bce434a5e5c4d0dc628b0529264db	addForeignKeyConstraint baseTableName=topic, constraintName=fk_o7ol28sotu1r12n8txv2gigok, referencedTableName=topics		\N	5.0.1	\N	\N	8446037746
1414872417007-190	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.825421	190	EXECUTED	9:8c38ea04b2f59bf71cf23a9896e12862	addForeignKeyConstraint baseTableName=sweden_election_type, constraintName=fk_ob50ibby6jamvitbxknoucifg, referencedTableName=sweden_election_region		\N	5.0.1	\N	\N	8446037746
1414872417007-191	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.834657	191	EXECUTED	9:093c24624d60c39f0c07c436100f5a40	addForeignKeyConstraint baseTableName=aggregated_bug_data, constraintName=fk_osdlir1nv0m8ckb1pbipgswj, referencedTableName=person_data		\N	5.0.1	\N	\N	8446037746
1414872417007-192	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.84237	192	EXECUTED	9:f82186446de4769ed31f4da13061611b	addForeignKeyConstraint baseTableName=application_view, constraintName=fk_p8b7gnxeglk71etbbql3j184s, referencedTableName=data_source_content		\N	5.0.1	\N	\N	8446037746
1414872417007-193	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.849998	193	EXECUTED	9:0ec91f1010fa7e1204a8124d5d8f5144	addForeignKeyConstraint baseTableName=sweden_county_data, constraintName=fk_pndlg3q6ly10qbs8e3s9wikyu, referencedTableName=sweden_county_data_container		\N	5.0.1	\N	\N	8446037746
1414872417007-194	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.857895	194	EXECUTED	9:452c2540e89f1802c787458cf6b48cd8	addForeignKeyConstraint baseTableName=document_detail_data, constraintName=fk_quor6wesrmk9ierjyr7ni8wch, referencedTableName=document_detail_container		\N	5.0.1	\N	\N	8446037746
1414872417007-195	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.865781	195	EXECUTED	9:cc1d30b724e685b87c77fc853efa1bc3	addForeignKeyConstraint baseTableName=sweden_parliament_electoral__1, constraintName=fk_qvgtilwwyipbrv6b2cv6fcp27, referencedTableName=sweden_parliament_electoral__0		\N	5.0.1	\N	\N	8446037746
1414872417007-196	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.873753	196	EXECUTED	9:ec3a97c733d887320cae227c31414d0d	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_r2dkprhp4xfhrcck9sf31b9xl, referencedTableName=document_activity_container		\N	5.0.1	\N	\N	8446037746
1414872417007-197	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.881732	197	EXECUTED	9:9d34aebdd2d3710bc4969a4b919b9e44	addForeignKeyConstraint baseTableName=sweden_municipality_election_0, constraintName=fk_r3jht5oci01uxhwaa39uxsg2t, referencedTableName=sweden_municipality_data		\N	5.0.1	\N	\N	8446037746
1414872417007-198	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.889621	198	EXECUTED	9:fc898e94f68547d5d6b575ba264843b3	addForeignKeyConstraint baseTableName=person_data, constraintName=fk_rd2pmyb4gmu2vbxklh6er8ayc, referencedTableName=person_assignment_data		\N	5.0.1	\N	\N	8446037746
1.52-drop-anomaly	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.121189	470	EXECUTED	9:9a0f73f4be1cffbe7460fcbeef739acb	sql		\N	5.0.1	\N	\N	8446037746
1414872417007-199	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.897025	199	EXECUTED	9:44b8fe081629b67789129dde1fad255d	addForeignKeyConstraint baseTableName=sweden_election_type, constraintName=fk_rp3tjh4jmpmoxh05oo9fq9qh6, referencedTableName=sweden_election_type_contain_0		\N	5.0.1	\N	\N	8446037746
1414872417007-200	pether (generated)	db-changelog-1.0.xml	2026-01-15 04:00:40.904036	200	EXECUTED	9:538e5077d44dd97071f9d407ac8812d0	addForeignKeyConstraint baseTableName=application_view, constraintName=fk_x8sbg6y7h0vavmun6i7h8oae, referencedTableName=operational_information_cont_0		\N	5.0.1	\N	\N	8446037746
1414872417007-201	pether	db-changelog-1.0.xml	2026-01-15 04:00:40.911367	201	EXECUTED	9:765cc5f3190db16a819f460b2c3a9835	modifyDataType columnName=content, tableName=document_content_data		\N	5.0.1	\N	\N	8446037746
1414872417007-202	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.924067	202	EXECUTED	9:08fb1bac7fdd7016ab7d47d2f70ebbfa	createView viewName=view_document_data_committee_report_url		\N	5.0.1	\N	\N	8446037746
1414872417007-203	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.93053	203	EXECUTED	9:534ad16ef62ab2164f305d17d8fba21b	modifyDataType columnName=proposal, tableName=committee_proposal_data		\N	5.0.1	\N	\N	8446037746
1414872417007-204	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.937922	204	EXECUTED	9:dad3fd6230b1f6b09a7ea190314f3e75	createView viewName=view_riksdagen_committee		\N	5.0.1	\N	\N	8446037746
1414872417007-205	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.945223	205	EXECUTED	9:cce5264c458c91c73e66133abbaca793	createView viewName=view_riksdagen_goverment		\N	5.0.1	\N	\N	8446037746
1414872417007-206	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.952838	206	EXECUTED	9:3c85c169ae80776fcc98b8be9fb3ed04	createView viewName=view_riksdagen_goverment_roles		\N	5.0.1	\N	\N	8446037746
1414872417007-207	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.960891	207	EXECUTED	9:e518955b8fdd65916aef6f8d2e13a803	createView viewName=view_riksdagen_committee_decisions		\N	5.0.1	\N	\N	8446037746
1414872417007-208	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.968102	208	EXECUTED	9:b261683ab49f67d5ad404b2cd5d5ec79	createView viewName=view_riksdagen_goverment_proposals		\N	5.0.1	\N	\N	8446037746
1414872417007-209	pether	db-changelog-1.1.xml	2026-01-15 04:00:40.975216	209	EXECUTED	9:a14b26657c5b09346ccfe984cc94d11d	createView viewName=view_riksdagen_member_proposals		\N	5.0.1	\N	\N	8446037746
1416258476613-210	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:40.982044	210	EXECUTED	9:c2effec0a954d668b8fd780eb04879d6	createTable tableName=document_proposal_container		\N	5.0.1	\N	\N	8446037746
1416258476613-211	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:40.98923	211	EXECUTED	9:908f681226f2fb5abe4b199126d26a6b	createTable tableName=document_proposal_data		\N	5.0.1	\N	\N	8446037746
1416258476613-212	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:40.996808	212	EXECUTED	9:3e0f9abc6d66f9b8582632524ec3806a	addColumn tableName=document_status_container; dropColumn columnName=document_proposal, tableName=document_status_container		\N	5.0.1	\N	\N	8446037746
1416258476613-213	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:41.003584	213	EXECUTED	9:f866ec1b7098b04250e282bf216ce46b	addPrimaryKey constraintName=document_proposal_container_pkey, tableName=document_proposal_container		\N	5.0.1	\N	\N	8446037746
1416258476613-214	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:41.010698	214	EXECUTED	9:758bf9de406141115f5a3dea3f38d224	addPrimaryKey constraintName=document_proposal_data_pkey, tableName=document_proposal_data		\N	5.0.1	\N	\N	8446037746
1416258476613-215	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:41.017652	215	EXECUTED	9:bc8c5511c4210514433a23a9802a16ea	addForeignKeyConstraint baseTableName=document_proposal_container, constraintName=fk_m55tt4vaimgb5qk7xj9mgxmry, referencedTableName=document_proposal_data		\N	5.0.1	\N	\N	8446037746
1416258476613-216	pether (generated)	db-changelog-1.1.xml	2026-01-15 04:00:41.024363	216	EXECUTED	9:824a2b5b89c1f8445fe03bce8a1ed4a3	addForeignKeyConstraint baseTableName=document_status_container, constraintName=fk_iirofquegnrpnuonvnydf6wfb, referencedTableName=document_proposal_container		\N	5.0.1	\N	\N	8446037746
1414872417007-217	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.031783	217	EXECUTED	9:dc12d88f692a0de46300f916366fcd3d	modifyDataType columnName=wording, tableName=document_proposal_data; modifyDataType columnName=wording_2, tableName=document_proposal_data; modifyDataType columnName=wording_3, tableName=document_proposal_data; modifyDataType columnName=wording_4,...		\N	5.0.1	\N	\N	8446037746
1414872417007-218	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.039295	218	EXECUTED	9:bf01ae3c4a683f06ebfb04fb51534659	createView viewName=view_riksdagen_committee_roles		\N	5.0.1	\N	\N	8446037746
1414872417007-219	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.050929	219	EXECUTED	9:40844e607d59d90e5e0cceea37549f78	dropView viewName=view_riksdagen_committee; createView viewName=view_riksdagen_committee; createView viewName=view_riksdagen_committee_parliament_member_proposal		\N	5.0.1	\N	\N	8446037746
1414872417007-220	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.061244	220	EXECUTED	9:2989f55d70d91a3b877bdc4fbde2bcc7	dropView viewName=view_riksdagen_goverment_roles; createView viewName=view_riksdagen_goverment_roles; dropView viewName=view_riksdagen_committee_roles; createView viewName=view_riksdagen_committee_roles		\N	5.0.1	\N	\N	8446037746
1414872417007-221	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.069521	221	EXECUTED	9:22e8c979c00998ab0584a65208d0070c	dropView viewName=view_riksdagen_goverment; createView viewName=view_riksdagen_goverment		\N	5.0.1	\N	\N	8446037746
1414872417007-222	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.077146	222	EXECUTED	9:a03f84a49600f94a9947b1b8d5831149	createView viewName=view_riksdagen_party_member		\N	5.0.1	\N	\N	8446037746
1414872417007-223	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.084508	223	EXECUTED	9:d9e3c3c85411b661c092f2f8922b9c06	createView viewName=view_riksdagen_party		\N	5.0.1	\N	\N	8446037746
1414872417007-224	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.091946	224	EXECUTED	9:b57642b4b204201b692e94822604822c	dropView viewName=view_riksdagen_party; createView viewName=view_riksdagen_party		\N	5.0.1	\N	\N	8446037746
1414872417007-225	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.100211	225	EXECUTED	9:ea6a0f678b722d37e8d2862d86044b1e	dropView viewName=view_riksdagen_goverment; createView viewName=view_riksdagen_goverment		\N	5.0.1	\N	\N	8446037746
1414872417007-226	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.110283	226	EXECUTED	9:f61f477649fb8c7055d2b374748e2459	dropView viewName=view_riksdagen_committee_parliament_member_proposal; dropView viewName=view_riksdagen_committee; createView viewName=view_riksdagen_committee; createView viewName=view_riksdagen_committee_parliament_member_proposal		\N	5.0.1	\N	\N	8446037746
1414872417007-227	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.118746	227	EXECUTED	9:0e86f855ff9c4aef94315ddd54a3023d	createView viewName=view_riksdagen_goverment_role_member; createView viewName=view_riksdagen_committee_role_member		\N	5.0.1	\N	\N	8446037746
1414872417007-228	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.127269	228	EXECUTED	9:c1f77e255a88bcc694bbe19fe721d57c	createView viewName=view_riksdagen_politician		\N	5.0.1	\N	\N	8446037746
1414872417007-279	pether (generated)	db-changelog-1.7.xml	2026-01-24 15:25:44.923898	524	EXECUTED	9:2ff509c4bb043c2b24857cb07bc3c708	addColumn tableName=document_element		\N	5.0.1	\N	\N	9264743943
1414872417007-229	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.136425	229	EXECUTED	9:02389fc7a55e20948a4b646d84108740	dropView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_politician		\N	5.0.1	\N	\N	8446037746
1414872417007-230	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.145974	230	EXECUTED	9:74cca167d30b5635d5d604d621881082	dropView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_politician		\N	5.0.1	\N	\N	8446037746
1414872417007-231	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.154225	231	EXECUTED	9:083ad5ea0ee0c5582da18d78f074043d	createView viewName=view_riksdagen_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-232	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.163274	232	EXECUTED	9:8c5e85cc7b9722b6cf11cc1e2370af47	dropView viewName=view_riksdagen_party_summary; createView viewName=view_riksdagen_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-233	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.176634	233	EXECUTED	9:e615358ab9818929ffe7a8d502e4bb85	dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-234	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.185259	234	EXECUTED	9:eaa93e3fb1b9d6745bc1bc4f6decb446	createView viewName=view_riksdagen_politician_document		\N	5.0.1	\N	\N	8446037746
1414872417007-235	pether	db-changelog-1.1.xml	2026-01-15 04:00:41.199959	235	EXECUTED	9:2a4aaa3ad3c5adf40fd1e09942d17ba2	dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-236	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.21469	236	EXECUTED	9:7a212611fdd15ee2fe72ba19866e2f68	dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-237	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.226146	237	EXECUTED	9:2b9a267cae0600616bd0fab522c2210b	dropView viewName=view_riksdagen_goverment_role_member; createView viewName=view_riksdagen_goverment_role_member; dropView viewName=view_riksdagen_committee_role_member; createView viewName=view_riksdagen_committee_role_member; createView viewName...		\N	5.0.1	\N	\N	8446037746
1414872417007-238	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.236334	238	EXECUTED	9:9b345a7e03fca2d72d77f5d6b3351409	dropForeignKeyConstraint baseTableName=user_account, constraintName=fk_8mmnmcgjut9nc7dfhrgxi598f; dropTable tableName=aggregated_bug_data; dropTable tableName=aggregated_country_data; dropColumn columnName=country_user_account_hjid, tableName=user...		\N	5.0.1	\N	\N	8446037746
1414872417007-239	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.247935	239	EXECUTED	9:2114de6439a5fe3157a923c3d8ee36c7	dropView viewName=view_riksdagen_goverment_role_member; createView viewName=view_riksdagen_goverment_role_member; dropView viewName=view_riksdagen_committee_role_member; createView viewName=view_riksdagen_committee_role_member; dropView viewName=v...		\N	5.0.1	\N	\N	8446037746
1414872417007-240	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.260065	240	EXECUTED	9:591c6d509fe1d4dde6b41c9ecc5105c9	dropView viewName=view_riksdagen_goverment_role_member; createView viewName=view_riksdagen_goverment_role_member; dropView viewName=view_riksdagen_committee_role_member; createView viewName=view_riksdagen_committee_role_member; dropView viewName=v...		\N	5.0.1	\N	\N	8446037746
1414872417007-241	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.27006	241	EXECUTED	9:76d4f14c8fcd7a22ede9b78231ff916b	createView viewName=view_riksdagen_vote_data_ballot_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-242	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.280722	242	EXECUTED	9:dc35246f6d762ea3fa127bf050173773	createView viewName=view_riksdagen_vote_data_ballot_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-243	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.294462	243	EXECUTED	9:bedaee0e37a39ab72fcf8b0a32ea1ac3	dropView viewName=view_riksdagen_vote_data_ballot_party_summary; dropView viewName=view_riksdagen_vote_data_ballot_summary; createView viewName=view_riksdagen_vote_data_ballot_summary; createView viewName=view_riksdagen_vote_data_ballot_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-244	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.30291	244	EXECUTED	9:475ac35a820437f1e15c480211d60234	createView viewName=view_riksdagen_vote_data_ballot_summary_daily		\N	5.0.1	\N	\N	8446037746
1414872417007-245	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.312518	245	EXECUTED	9:5068091c96f1fd1622105bb83527a07e	createView viewName=view_riksdagen_vote_data_ballot_summary_weekly; createView viewName=view_riksdagen_vote_data_ballot_summary_monthly; createView viewName=view_riksdagen_vote_data_ballot_summary_annual		\N	5.0.1	\N	\N	8446037746
1414872417007-246	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.326336	246	EXECUTED	9:68d9a9971d2ea9bd76d03d89521ad3d5	dropView viewName=view_riksdagen_vote_data_ballot_summary_weekly; dropView viewName=view_riksdagen_vote_data_ballot_summary_monthly; dropView viewName=view_riksdagen_vote_data_ballot_summary_annual; dropView viewName=view_riksdagen_vote_data_ballo...		\N	5.0.1	\N	\N	8446037746
1414872417007-247	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.33555	247	EXECUTED	9:98d9c264a755b9a836390a08d33878d7	createView viewName=view_riksdagen_vote_data_ballot_party_summary_daily		\N	5.0.1	\N	\N	8446037746
1414872417007-248	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.347614	248	EXECUTED	9:e26631ba948fed851e8ddafa9c91f959	createView viewName=view_riksdagen_vote_data_ballot_party_summary_weekly; createView viewName=view_riksdagen_vote_data_ballot_party_summary_monthly; createView viewName=view_riksdagen_vote_data_ballot_party_summary_annual		\N	5.0.1	\N	\N	8446037746
1414872417007-249	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.357785	249	EXECUTED	9:bc3ce6e9dfc901e50afc2fafa681b95a	createView viewName=view_riksdagen_vote_data_ballot_politician_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-250	pether	db-changelog-1.2.xml	2026-01-15 04:00:41.37546	250	EXECUTED	9:91e3908e9e189208b9758c3cc6c44c71	createView viewName=view_riksdagen_vote_data_ballot_politician_summary_daily; createView viewName=view_riksdagen_vote_data_ballot_politician_summary_weekly; createView viewName=view_riksdagen_vote_data_ballot_politician_summary_monthly; createView...		\N	5.0.1	\N	\N	8446037746
1414872417007-251	pether	db-changelog-1.3.xml	2026-01-15 04:00:41.415951	251	EXECUTED	9:ef6bbe1d06a5478a5c33db26bfd794f1	dropView viewName=view_riksdagen_vote_data_ballot_politician_summary_weekly; dropView viewName=view_riksdagen_vote_data_ballot_politician_summary_monthly; dropView viewName=view_riksdagen_vote_data_ballot_politician_summary_annual; dropView viewNa...		\N	5.0.1	\N	\N	8446037746
1414872417007-252	pether (generated)	db-changelog-1.4.xml	2026-01-15 04:00:41.425362	252	EXECUTED	9:76f1acc51844fae7e7ef064ee6c8fc08	dropView viewName=view_riksdagen_committee_decisions; modifyDataType columnName=title, tableName=committee_document_data; modifyDataType columnName=sub_title, tableName=committee_document_data; modifyDataType columnName=temp_label, tableName=commi...		\N	5.0.1	\N	\N	8446037746
25353872417007-324	party_trends-fix	db-changelog-1.26.xml	2026-01-15 04:00:42.765932	344	EXECUTED	9:60797ca4c12b59c5f244255c0bda44cf	createView viewName=view_riksdagen_party_ballot_support_annual_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-253	pether	db-changelog-1.4.xml	2026-01-15 04:00:41.467393	253	EXECUTED	9:4e4b64893fd4dae4fd2bffadc7163fb2	dropView viewName=view_riksdagen_vote_data_ballot_politician_summary_weekly; dropView viewName=view_riksdagen_vote_data_ballot_politician_summary_monthly; dropView viewName=view_riksdagen_vote_data_ballot_politician_summary_annual; dropView viewNa...		\N	5.0.1	\N	\N	8446037746
1414872417007-254	pether	db-changelog-1.4.xml	2026-01-15 04:00:41.477066	254	EXECUTED	9:c7d4e2eda2ed22d4092964cf1f6756ab	createView viewName=view_riksdagen_document_type_daily_summary; createView viewName=view_riksdagen_politician_document_daily_summary; createView viewName=view_riksdagen_party_document_daily_summary; createView viewName=view_riksdagen_org_document_...		\N	5.0.1	\N	\N	8446037746
1414872417007-255	pether	db-changelog-1.4.xml	2026-01-15 04:00:41.485152	255	EXECUTED	9:de1ff4d851c9cfb6e0597760c4cfe11b	createView viewName=view_riksdagen_committee_decision_type_summary; createView viewName=view_riksdagen_committee_decision_type_org_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-256	pether	db-changelog-1.4.xml	2026-01-15 04:00:41.49663	256	EXECUTED	9:55cb855446f25c527751dcb9eda7bf1d	dropView viewName=view_riksdagen_committee_decision_type_summary; createView viewName=view_riksdagen_committee_decision_type_summary; dropView viewName=view_riksdagen_committee_decision_type_org_summary; createView viewName=view_riksdagen_committe...		\N	5.0.1	\N	\N	8446037746
1414872417007-257	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.508233	257	EXECUTED	9:505f6fa901adab3bd33ddb3ad4a44967	dropView viewName=view_riksdagen_committee_decision_type_summary; dropView viewName=view_riksdagen_committee_decision_type_org_summary; dropView viewName=view_riksdagen_committee_decisions; createView viewName=view_riksdagen_committee_decisions; c...		\N	5.0.1	\N	\N	8446037746
1414872417007-258	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.518785	258	EXECUTED	9:4939cc4cd5b64ed9cfb0305c17b1ac42	createView viewName=view_riksdagen_committee_ballot_decision_summary; createView viewName=view_riksdagen_committee_ballot_decision_party_summary; createView viewName=view_riksdagen_committee_ballot_decision_politician_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-259	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.527142	259	EXECUTED	9:b574f8e847319078c295ab00b9db075a	dropView viewName=view_riksdagen_committee_decision_type_summary; createView viewName=view_riksdagen_committee_decision_type_summary; dropView viewName=view_riksdagen_committee_decision_type_org_summary; createView viewName=view_riksdagen_committe...		\N	5.0.1	\N	\N	8446037746
1414872417007-260	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.541467	260	EXECUTED	9:6b5a8faf027b4da44412ecda2cb4c87f	dropView viewName=view_riksdagen_committee_decision_type_summary; dropView viewName=view_riksdagen_committee_decision_type_org_summary; dropView viewName=view_riksdagen_committee_ballot_decision_summary; dropView viewName=view_riksdagen_committee_...		\N	5.0.1	\N	\N	8446037746
1414872417007-261	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.548993	261	EXECUTED	9:a898f1a6d116c317439ac2289b616988	createView viewName=view_worldbank_indicator_data_country_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-262	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.556959	262	EXECUTED	9:943858a57243b94eab7e4150c2fcd36b	dropView viewName=view_worldbank_indicator_data_country_summary; createView viewName=view_worldbank_indicator_data_country_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-263	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.564918	263	EXECUTED	9:7d34f8665567dbd826ceff8e38177cc9	dropView viewName=view_worldbank_indicator_data_country_summary; createView viewName=view_worldbank_indicator_data_country_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-264	pether	db-changelog-1.5.xml	2026-01-15 04:00:41.641285	264	EXECUTED	9:0aa77dc47d643b3882f2a4ca3238c070	dropView viewName=view_riksdagen_committee_decision_type_summary; dropView viewName=view_riksdagen_committee_decision_type_org_summary; dropView viewName=view_riksdagen_committee_ballot_decision_summary; dropView viewName=view_riksdagen_committee_...		\N	5.0.1	\N	\N	8446037746
1414872417007-265	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.658752	265	EXECUTED	9:6c82b7bad6f5bd4c961ed3cfee42cf1b	dropView viewName=view_riksdagen_document_type_daily_summary; dropView viewName=view_riksdagen_politician_document_daily_summary; dropView viewName=view_riksdagen_party_document_daily_summary; dropView viewName=view_riksdagen_org_document_daily_su...		\N	5.0.1	\N	\N	8446037746
1414872417007-266	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.676837	266	EXECUTED	9:345962d1e2cffcbe3413eb509fd97886	sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-267	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.686355	267	EXECUTED	9:9fc625a3d56571e98a8d7eea06980e65	dropView viewName=view_worldbank_indicator_data_country_summary; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-268	pether (generated)	db-changelog-1.6.xml	2026-01-15 04:00:41.693474	268	EXECUTED	9:53a2955ac5300632144a22b15a944a8e	addColumn tableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-269	pether (generated)	db-changelog-1.6.xml	2026-01-15 04:00:41.700786	269	EXECUTED	9:accbf9a9a30c5dd595a667a721f2ff89	addColumn tableName=application_action_event		\N	5.0.1	\N	\N	8446037746
1414872417007-270	pether (generated)	db-changelog-1.6.xml	2026-01-15 04:00:41.707194	270	EXECUTED	9:b415b0a981b9dfb061d94504b19b0878	addColumn tableName=user_account		\N	5.0.1	\N	\N	8446037746
1414872417007-271	pether (generated)	db-changelog-1.6.xml	2026-01-15 04:00:41.713972	271	EXECUTED	9:522e0502d2a4e5e8cebfbe7c193cdf24	addColumn tableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-272	pether (generated)	db-changelog-1.6.xml	2026-01-15 04:00:41.7204	272	EXECUTED	9:94728ce0aa674d7ef4fe9b0521b55fcb	addColumn tableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-273	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.729729	273	EXECUTED	9:b9bc2c08e551503123084c32e2839f50	sql; sql; sql; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-274	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.73928	274	EXECUTED	9:b257435f43a13311f319b53b205ef7c1	sql; sql; sql; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-275	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.748676	275	EXECUTED	9:92661264af048495b1a63824cc71e551	sql; sql; sql; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-276	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.755902	276	EXECUTED	9:765d8cd0cdacd034d8a0674eaa880023	createTable tableName=application_configuration		\N	5.0.1	\N	\N	8446037746
1414872417007-277	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.768038	277	EXECUTED	9:c60c41cdca28d541fd3a2b605b711d18	dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_politician; createView viewName=view_riksdagen_party_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-278	pether	db-changelog-1.6.xml	2026-01-15 04:00:41.783408	278	EXECUTED	9:302d61374938a5619b973c5f7a3fcd85	dropView viewName=view_riksdagen_party; dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; dropView viewName=view_riksdagen_party_member; createView viewName=view_riksdagen_party_member; createView viewNam...		\N	5.0.1	\N	\N	8446037746
2414872417007-328	pether (generated)	db-changelog-1.24.xml	2026-01-15 04:00:42.306274	319	EXECUTED	9:6246b135264fb22f812528142c12222c	addPrimaryKey constraintName=application_configuration_pkey, tableName=application_configuration		\N	5.0.1	\N	\N	8446037746
1414872417007-290	pether	db-changelog-1.10.xml	2026-01-15 04:00:41.906022	290	EXECUTED	9:3477a6ab16abd3502429f252c6f2c21a	dropView viewName=view_riksdagen_party; dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; dropView viewName=view_riksdagen_party_member; createView viewName=view_riksdagen_party_member; createView viewNam...		\N	5.0.1	\N	\N	8446037746
1414872417007-291	pether	db-changelog-1.11.xml	2026-01-15 04:00:41.913015	291	EXECUTED	9:96a236431ea32c987c4572cb54e71c19	renameTable newTableName=QRTZ_BLOB_TRIGGERS, oldTableName=QRTZ_bytea_TRIGGERS		\N	5.0.1	\N	\N	8446037746
1414872417007-292	add-column-application-session	db-changelog-1.12.xml	2026-01-15 04:00:41.920056	292	EXECUTED	9:956bf680aea08e30a78b8d768dae529c	addColumn tableName=application_session		\N	5.0.1	\N	\N	8446037746
1414872417007-294	gdpr-classify-data	db-changelog-1.13.xml	2026-01-15 04:00:42.09872	293	EXECUTED	9:dc2a0e2901c2d886086e508ff01826b6	sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sq...		\N	5.0.1	\N	\N	8446037746
1414872417007-295	fix-missing-value-for-model-object-version	db-changelog-1.14.xml	2026-01-15 04:00:42.106168	294	EXECUTED	9:0eddefb0cd23c53120a00749d09808f9	update tableName=agency; update tableName=portal; update tableName=language_data; update tableName=language_content_data; update tableName=application_action_event; update tableName=application_configuration; update tableName=application_session; ...		\N	5.0.1	\N	\N	8446037746
1414872417007-296	adduseraccountcolumns	db-changelog-1.15.xml	2026-01-15 04:00:42.114612	295	EXECUTED	9:941c29c388e300e20109c3cba3fb8945	addColumn tableName=user_account; update tableName=user_account; addColumn tableName=user_account; update tableName=user_account		\N	5.0.1	\N	\N	8446037746
1414872417007-297	adduseraccountcolumns	db-changelog-1.15.xml	2026-01-15 04:00:42.119076	296	EXECUTED	9:fc729e50a9ac4d8481cfc0e5e615c094	update tableName=user_account		\N	5.0.1	\N	\N	8446037746
1414872417007-301	createEncryptedValueTable	db-changelog-1.17.xml	2026-01-15 04:00:42.128905	297	EXECUTED	9:63965b31aa35ce0c2e58f92f14a071bc	createTable tableName=encrypted_value		\N	5.0.1	\N	\N	8446037746
1414872417007-302	encryptedValueTableAddColumns	db-changelog-1.18.xml	2026-01-15 04:00:42.136628	298	EXECUTED	9:9b23ce1c3d7f2d32d2cb725996846872	addColumn tableName=encrypted_value; addColumn tableName=encrypted_value		\N	5.0.1	\N	\N	8446037746
1414872417007-303	deleteUserAccountGoogleMFAColumns	db-changelog-1.18.xml	2026-01-15 04:00:42.145484	299	EXECUTED	9:1c6f160cd4532f7df63c6b6691d384dc	dropForeignKeyConstraint baseTableName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0, constraintName=fk_8931ymg13vy6vfkrichdsd4; dropPrimaryKey constraintName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0_pkey, tableName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0; dropTable tableName=USER...		\N	5.0.1	\N	\N	8446037746
1414872417007-304	gdpr-classification-update-account	db-changelog-1.18.xml	2026-01-15 04:00:42.153509	300	EXECUTED	9:a785758642766d317c33f7a42de5b391	sql; sql; sql; sql; sql; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-305	encryptedValueTableChangeStorageType	db-changelog-1.18.xml	2026-01-15 04:00:42.160744	301	EXECUTED	9:91dc17b871f8f01ef34ab9ab7bf2c363	modifyDataType columnName=storage, tableName=encrypted_value		\N	5.0.1	\N	\N	8446037746
1414872417007-312	auditViews	db-changelog-1.19.xml	2026-01-15 04:00:42.178093	302	EXECUTED	9:11cdf7f46afe7b66fc1f50bc92ec575a	createSequence sequenceName=jv_commit_pk_seq; createSequence sequenceName=jv_global_id_pk_seq; createSequence sequenceName=jv_snapshot_pk_seq; createTable tableName=jv_commit; createTable tableName=jv_commit_property; createTable tableName=jv_glob...		\N	5.0.1	\N	\N	8446037746
1414872417007-313	auditViews	db-changelog-1.20.xml	2026-01-15 04:00:42.186419	303	EXECUTED	9:1d1f97a288995bd7b6a0004330ef9ed3	createView viewName=view_audit_data_summary; createView viewName=view_audit_author_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-314	auditViews2	db-changelog-1.20.xml	2026-01-15 04:00:42.193837	304	EXECUTED	9:bcf4660ae87b9d67626071f78bc8e9d1	dropView viewName=view_audit_data_summary; createView viewName=view_audit_data_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-315	documentSummaryViews	db-changelog-1.21.xml	2026-01-15 04:00:42.201531	305	EXECUTED	9:6bd6a54409a0ca14cd36aaaa54ae8b58	createView viewName=view_riksdagen_person_signed_document_summary; createView viewName=view_riksdagen_party_signatures_document_summary		\N	5.0.1	\N	\N	8446037746
1414872417007-316	auditViews3	db-changelog-1.21.xml	2026-01-15 04:00:42.209359	306	EXECUTED	9:80be555ad4c46adc54270b99c8481315	dropView viewName=view_audit_data_summary; createView viewName=view_audit_data_summary		\N	5.0.1	\N	\N	8446037746
2414872417007-317	javersAddColumn	db-changelog-1.22.xml	2026-01-15 04:00:42.216167	307	EXECUTED	9:cadb829cc1c2e0dd2941a93cdd411563	addColumn tableName=jv_commit		\N	5.0.1	\N	\N	8446037746
2414872417007-318	javersChangeType	db-changelog-1.22.xml	2026-01-15 04:00:42.223688	308	EXECUTED	9:d795deb20b39b5a787ce1d98d48e65e5	modifyDataType columnName=commit_date_instant, tableName=jv_commit		\N	5.0.1	\N	\N	8446037746
2414872417007-319	javersDefaultValue2	db-changelog-1.22.xml	2026-01-15 04:00:42.227516	309	EXECUTED	9:423aa876ea8cd648b2cc511580862e4f	sql		\N	5.0.1	\N	\N	8446037746
2414872417007-320	javersDefaultValue	db-changelog-1.22.xml	2026-01-15 04:00:42.23448	310	EXECUTED	9:4e7a8dfeb87f3809922803b64ceded57	addDefaultValue columnName=commit_date_instant, tableName=jv_commit		\N	5.0.1	\N	\N	8446037746
2414872417007-321	party_trends	db-changelog-1.23.xml	2026-01-15 04:00:42.243514	311	EXECUTED	9:8909e0c5d3ef6ecfeb4af4e93d3295ef	createView viewName=view_riksdagen_party_ballot_support_annual_summary; createView viewName=view_riksdagen_party_coalation_against_annual_summary		\N	5.0.1	\N	\N	8446037746
2414872417007-322	party_trends	db-changelog-1.23.xml	2026-01-15 04:00:42.251673	312	EXECUTED	9:bdcfbe51f13c670188c9a003e84f7391	dropView viewName=view_riksdagen_party_ballot_support_annual_summary; createView viewName=view_riksdagen_party_ballot_support_annual_summary		\N	5.0.1	\N	\N	8446037746
2414872417007-323	party_trends	db-changelog-1.23.xml	2026-01-15 04:00:42.259385	313	EXECUTED	9:345a31d3673a4e908c0f842854b543e7	dropView viewName=view_riksdagen_party_coalation_against_annual_summary; createView viewName=view_riksdagen_party_coalation_against_annual_summary		\N	5.0.1	\N	\N	8446037746
2414872417007-324	party_trends	db-changelog-1.23.xml	2026-01-15 04:00:42.267405	314	EXECUTED	9:9aad9d408015cdda50e8624d708db21f	dropView viewName=view_riksdagen_party_ballot_support_annual_summary; createView viewName=view_riksdagen_party_ballot_support_annual_summary		\N	5.0.1	\N	\N	8446037746
2414872417007-324	party_trends	db-changelog-1.24.xml	2026-01-15 04:00:42.275915	315	EXECUTED	9:9aad9d408015cdda50e8624d708db21f	dropView viewName=view_riksdagen_party_ballot_support_annual_summary; createView viewName=view_riksdagen_party_ballot_support_annual_summary		\N	5.0.1	\N	\N	8446037746
2414872417007-325	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.284285	316	EXECUTED	9:a98bd9300cbce790409b1438eb5681ab	dropView viewName=view_riksdagen_party_role_member; createView viewName=view_riksdagen_party_role_member		\N	5.0.1	\N	\N	8446037746
2414872417007-326	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.291982	317	EXECUTED	9:769fd2cc719f8070361d0236c2f219ca	createTable tableName=rule_violation		\N	5.0.1	\N	\N	8446037746
1414872417007-329	pether (generated)	db-changelog-1.24.xml	2026-01-15 04:00:42.316476	320	EXECUTED	9:bbb17b2f46dd435a351912318ba127e7	sql; modifyDataType columnName=indicator_name, tableName=indicator_element; modifyDataType columnName=source_id, tableName=indicator_element; modifyDataType columnName=source_value, tableName=indicator_element; sql		\N	5.0.1	\N	\N	8446037746
1414872417007-330	pether (generated)	db-changelog-1.24.xml	2026-01-15 04:00:42.325021	321	EXECUTED	9:5dbf93e01244ac5381aab449179bfee3	createIndex indexName=application_action_event_created_date_idx, tableName=application_action_event; createIndex indexName=application_action_event_sessionid_idx, tableName=application_action_event; createIndex indexName=application_action_event_e...		\N	5.0.1	\N	\N	8446037746
1414872417007-331	pether (generated)	db-changelog-1.24.xml	2026-01-15 04:00:42.343665	322	EXECUTED	9:d98cea6bed923440f8c3a31871a66ca2	sql		\N	5.0.1	\N	\N	8446037746
extend-view-riksdagen-politician-20231002	pether 	db-changelog-1.24.xml	2026-01-15 04:00:42.367058	323	EXECUTED	9:c7e51eab90afc2cec2f73cd365e799bf	dropView viewName=view_riksdagen_party; dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_politician; dropView viewName=view_riksdagen_party_member; createView viewName=view_riksdagen_party_member; createView viewNam...		\N	5.0.1	\N	\N	8446037746
new_changeset_id_1	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.375313	324	EXECUTED	9:3111b7723aeefc1cbe0f17bb0fd23c0c	dropView viewName=view_application_action_event_page_annual_summary; createView viewName=view_application_action_event_page_annual_summary		\N	5.0.1	\N	\N	8446037746
new_changeset_id_2	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.383262	325	EXECUTED	9:50ec6175bbb9840ff5f9a86088f4cb4b	dropView viewName=view_application_action_event_page_daily_summary; createView viewName=view_application_action_event_page_daily_summary		\N	5.0.1	\N	\N	8446037746
new_changeset_id_3	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.391484	326	EXECUTED	9:79aafee120630bbaa7c54b9a557bc740	dropView viewName=view_application_action_event_page_element_annual_summary; createView viewName=view_application_action_event_page_element_annual_summary		\N	5.0.1	\N	\N	8446037746
new_changeset_id_4	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.399703	327	EXECUTED	9:3b0d31d9363ad768e1180b3b988c2fec	dropView viewName=view_application_action_event_page_element_daily_summary; createView viewName=view_application_action_event_page_element_daily_summary		\N	5.0.1	\N	\N	8446037746
new_changeset_id_5	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.40805	328	EXECUTED	9:0c64722b12c51dca3e64006372ccc5c8	createIndex indexName=application_action_event_page_idx, tableName=application_action_event; createIndex indexName=application_action_event_element_id_idx, tableName=application_action_event; createIndex indexName=application_action_event_page_cre...		\N	5.0.1	\N	\N	8446037746
1414872417007-265-document-summary-views	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.43725	329	EXECUTED	9:acc9a8db7b3db0cd2dcb3ce083a989a7	sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; sql; createView viewName=view_riksdagen_party_document_summary; createIndex indexName=idx_rpd_person_ref_id, tableName=view_riksdagen_politician_document; createIndex indexName=idx_rpd_doc_type_sub...		\N	5.0.1	\N	\N	8446037746
extend-view-riksdagen-politician-party-20231223	pethers	db-changelog-1.24.xml	2026-01-15 04:00:42.468621	330	EXECUTED	9:892d39a59b866cdaf0366544ffb0d3bd	dropView viewName=view_riksdagen_party; dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_party_document_summary; dropView viewName=view_riksdagen_politician; dropView viewName=view_riksdagen_party_member; createView...		\N	5.0.1	\N	\N	8446037746
1414872417007-204	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.483206	331	EXECUTED	9:69f5c89bfe631122c51b8dfe35022fa1	dropView viewName=view_riksdagen_committee_parliament_member_proposal; dropView viewName=view_riksdagen_committee; createView viewName=view_riksdagen_committee; createView viewName=view_riksdagen_committee_parliament_member_proposal; createIndex i...		\N	5.0.1	\N	\N	8446037746
1414872417007-225	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.494951	332	EXECUTED	9:e5519073bd6d48b443777395119afafb	dropView viewName=view_riksdagen_goverment; createView viewName=view_riksdagen_goverment; dropView viewName=view_riksdagen_goverment_role_member; createView viewName=view_riksdagen_goverment_role_member		\N	5.0.1	\N	\N	8446037746
party-role-member-1414872417007-227	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.504132	333	EXECUTED	9:fbe676dfd287959902eaeb407d2e1bd9	dropView viewName=view_riksdagen_party_role_member; createView viewName=view_riksdagen_party_role_member		\N	5.0.1	\N	\N	8446037746
committee-role-member-1414872417007-228	pether	db-changelog-1.24.xml	2026-01-15 04:00:42.514415	334	EXECUTED	9:033dba6bbd5a8556bd1ad33d81873080	dropView viewName=view_riksdagen_committee_role_member; createView viewName=view_riksdagen_committee_role_member		\N	5.0.1	\N	\N	8446037746
vote-data-324	pether	db-changelog-1.25.xml	2026-01-15 04:00:42.522383	335	EXECUTED	9:515f70411429a7a26e07be4b37742628	sql		\N	5.0.1	\N	\N	8446037746
20241224-ballot-summary-view	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.620315	336	EXECUTED	9:533172c9a2e8e77072ba3d377b95406e	sql; sql; sql; sql; sql; sql; sql; sql; sql		\N	5.0.1	\N	\N	8446037746
2024122623-extend-party-view	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.636351	337	EXECUTED	9:c0383a8d45978f6163e0ff64924f5ddc	dropView viewName=view_riksdagen_party; dropView viewName=view_riksdagen_party_summary; dropView viewName=view_riksdagen_party_document_summary; dropView viewName=view_riksdagen_politician; dropView viewName=view_riksdagen_party_member; createView...	Extend party view with additional political analysis metrics using existing data	\N	5.0.1	\N	\N	8446037746
20241226-improve-politician-view	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.647877	338	EXECUTED	9:f13bf14c659e3df9d2ba71facc1e144a	createView viewName=view_riksdagen_politician	Enhance politician view with comprehensive metrics and committee effectiveness	\N	5.0.1	\N	\N	8446037746
20241227-enhance-party-document-summary	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.659755	339	EXECUTED	9:ccc25367d2b7d5b9ffcbe5465066e6b0	createView viewName=view_riksdagen_party_document_summary	Enhance party document summary with comprehensive metrics while preserving existing functionality	\N	5.0.1	\N	\N	8446037746
20241227-enhance-party-summary-performance	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.670677	340	EXECUTED	9:d14911091c781dc9028e99cac0c0e9b8	createIndex indexName=idx_assignment_data_type_dates, tableName=assignment_data; sql	Add indexes to improve party summary view performance	\N	5.0.1	\N	\N	8446037746
2024122723-enhance-party-summary-performance	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.705585	341	EXECUTED	9:18fddfa9b12f93f8296ae4c494a712c8	sql	Add indexes to improve party summary view performance	\N	5.0.1	\N	\N	8446037746
8774122723-enhance-party	pethers	db-changelog-1.25.xml	2026-01-15 04:00:42.714353	342	EXECUTED	9:c7d05218977bc2edb5f16e8d878af5c3	createView viewName=view_riksdagen_party	Add indexes to improve party view	\N	5.0.1	\N	\N	8446037746
revert-jpa-model-update	pethers	db-changelog-1.26.xml	2026-01-15 04:00:42.757808	343	EXECUTED	9:ee172fa2cfbce95c78e2b983b0b507dc	dropView viewName=view_riksdagen_party; sql; dropView viewName=view_riksdagen_party_document_summary; dropView viewName=view_riksdagen_politician; dropView viewName=view_riksdagen_party_member; createView viewName=view_riksdagen_party_member; crea...		\N	5.0.1	\N	\N	8446037746
312321-view_riksdagen_politician_ballot_summary	pethers	db-changelog-1.27.xml	2026-01-15 04:00:42.775718	345	EXECUTED	9:14219bf99de56328f50c56503b50306c	createView viewName=view_riksdagen_politician_ballot_summary		\N	5.0.1	\N	\N	8446037746
672321-view_riksdagen_politician_experience_summary	pethers	db-changelog-1.28.xml	2026-01-15 04:00:42.795542	346	EXECUTED	9:3c2e1cfdcdcc06b9f96fd9f52d49c12a	createView viewName=view_riksdagen_politician_experience_summary		\N	5.0.1	\N	\N	8446037746
672321-view_riksdagen_politician_experience_summary-update	pethers	db-changelog-1.28.xml	2026-01-15 04:00:42.814494	347	EXECUTED	9:45e09b8b79cdd4f087c589f1773919f9	dropView viewName=view_riksdagen_politician_experience_summary; createView viewName=view_riksdagen_politician_experience_summary		\N	5.0.1	\N	\N	8446037746
672321-view_riksdagen_politician_experience_summary-update2	pethers	db-changelog-1.28.xml	2026-01-15 04:00:42.834068	348	EXECUTED	9:c47fecd951a060b4c17f34e08790d4b1	dropView viewName=view_riksdagen_politician_experience_summary; createView viewName=view_riksdagen_politician_experience_summary		\N	5.0.1	\N	\N	8446037746
672321-view_riksdagen_politician_experience_summary-update3	pethers	db-changelog-1.28.xml	2026-01-15 04:00:42.854655	349	EXECUTED	9:f2a8bbfb00ed3f67696a7bcca3d50c70	dropView viewName=view_riksdagen_politician_experience_summary; createView viewName=view_riksdagen_politician_experience_summary		\N	5.0.1	\N	\N	8446037746
672321-view_riksdagen_politician_experience_summary-update4	pethers	db-changelog-1.28.xml	2026-01-15 04:00:42.874599	350	EXECUTED	9:1126cc71ce6315bf43bcd967a39b9aa0	dropView viewName=view_riksdagen_politician_experience_summary; createView viewName=view_riksdagen_politician_experience_summary		\N	5.0.1	\N	\N	8446037746
67267321-view_riksdagen_politician_experience_summary-update5	pethers	db-changelog-1.28.xml	2026-01-15 04:00:42.899458	351	EXECUTED	9:dd2ca5f83bdadbb3fb61a8be06f4b873	dropView viewName=view_riksdagen_politician_experience_summary; createView viewName=view_riksdagen_politician_experience_summary		\N	5.0.1	\N	\N	8446037746
intops-2025111101-temporal-momentum	intelligence-ops	db-changelog-1.29.xml	2026-01-15 04:00:42.910676	352	EXECUTED	9:ebb0e4ff417fbb4dc385d74f99df287f	createView viewName=view_riksdagen_party_momentum_analysis	Temporal Momentum Analysis View\n        \n        Intelligence Purpose:\n        - Detect momentum shifts in party support\n        - Identify acceleration/deceleration patterns\n        - Calculate moving averages for trend smoothing\n        - Measur...	\N	5.0.1	\N	\N	8446037746
intops-2025111102-coalition-alignment	intelligence-ops	db-changelog-1.29.xml	2026-01-15 04:00:42.920764	353	EXECUTED	9:4754a1d61b41149c0d4fce931959b038	createView viewName=view_riksdagen_coalition_alignment_matrix	Coalition Voting Alignment Matrix\n        \n        Intelligence Purpose:\n        - Measure voting alignment between party pairs\n        - Identify natural coalition partners\n        - Detect coalition stress or breaking\n        - Predict coalition...	\N	5.0.1	\N	\N	8446037746
intops-2025111103-anomaly-detection	intelligence-ops	db-changelog-1.29.xml	2026-01-15 04:00:42.932169	354	EXECUTED	9:07a8006b3987714bbaf9a757605df73a	createView viewName=view_riksdagen_voting_anomaly_detection	Political Anomaly Detection View\n        \n        Intelligence Purpose:\n        - Detect unusual voting behavior by politicians\n        - Identify party discipline breakdowns\n        - Flag potential defection risks\n        - Monitor conscience vo...	\N	5.0.1	\N	\N	8446037746
intops-2025111104-influence-metrics	intelligence-ops	db-changelog-1.29.xml	2026-01-15 04:00:42.943675	355	EXECUTED	9:c2da8e7169d2c6c0358e4217462d4d7b	createView viewName=view_riksdagen_politician_influence_metrics	Politician Influence Network Metrics\n        \n        Intelligence Purpose:\n        - Calculate basic network centrality measures\n        - Identify key brokers and connectors\n        - Map informal influence beyond formal roles\n        - Detect p...	\N	5.0.1	\N	\N	8446037746
intops-2025111105-crisis-resilience	intelligence-ops	db-changelog-1.29.xml	2026-01-15 04:00:42.956133	356	EXECUTED	9:542aa60a4bce5eabfd5a50260a0bdf69	createView viewName=view_riksdagen_crisis_resilience_indicators	Political Crisis Resilience Indicators\n        \n        Intelligence Purpose:\n        - Assess politician performance during high-pressure periods\n        - Identify crisis-tested leaders\n        - Measure consistency under stress\n        - Evalua...	\N	5.0.1	\N	\N	8446037746
intops-2025111106-intelligence-dashboard	intelligence-ops	db-changelog-1.29.xml	2026-01-15 04:00:42.966857	357	EXECUTED	9:cbc88852e1308329aa065d9eb411639e	createView viewName=view_riksdagen_intelligence_dashboard	Intelligence Operations Summary Dashboard\n        \n        Intelligence Purpose:\n        - Provide executive-level overview of political landscape\n        - Aggregate key intelligence indicators\n        - Enable rapid situation assessment\n        ...	\N	5.0.1	\N	\N	8446037746
intops-2025111107-politician-risk-summary	intelligence-operative	db-changelog-1.29.xml	2026-01-15 04:00:42.978158	358	EXECUTED	9:d77a8fce731965ddc609c1104bcc0052	createView viewName=view_politician_risk_summary	Politician Risk Summary View\n        \n        Intelligence Purpose:\n        - Aggregate all risk indicators for each politician\n        - Provide comprehensive risk score based on rule violations, attendance (absence rates), voting effectiveness (...	\N	5.0.1	\N	\N	8446037746
intops-2025111107-politician-risk-idx	intelligence-operative	db-changelog-1.29.xml	2026-01-15 04:00:42.985942	359	EXECUTED	9:cdfed1beeb97276a94e40e00dfde6057	sql	Performance Indexes for Politician Risk Queries\n        \n        Indexes on frequently queried columns to support rapid risk assessment\n        and dashboard queries.	\N	5.0.1	\N	\N	8446037746
intops-2025111108-party-performance-metrics	intelligence-operative	db-changelog-1.29.xml	2026-01-15 04:00:43.007475	360	EXECUTED	9:001f5b3ea0c9f62f07d81f7b3e923f12	createView viewName=view_party_performance_metrics	Party Performance Metrics View\n        \n        Intelligence Purpose:\n        - Aggregate party-wide performance indicators\n        - Enable comparative party analysis\n        - Track organizational effectiveness\n        - Support coalition format...	\N	5.0.1	\N	\N	8446037746
intops-2025111109-committee-productivity	intelligence-operative	db-changelog-1.29.xml	2026-01-15 04:00:43.036141	361	EXECUTED	9:e9a0dd593d407533c4959eae10d1b42c	createView viewName=view_committee_productivity	Committee Productivity View\n        \n        Intelligence Purpose:\n        - Track committee legislative output\n        - Measure committee effectiveness\n        - Identify underperforming committees\n        - Support resource allocation decisions...	\N	5.0.1	\N	\N	8446037746
osint-2025111500-refresh-materialized-views	intelligence-operative	db-changelog-1.30.xml	2026-01-15 04:00:43.080297	362	EXECUTED	9:28eb73f089cc2b312024e56b87d7dd82	sql	Refresh Materialized Views for v1.30 Dependencies\n        \n        Purpose:\n        - Populate materialized views required by v1.30 OSINT views\n        - Ensure data availability for temporal analysis queries\n        - Execute in dependency order ...	\N	5.0.1	\N	\N	8446037746
osint-2025111501-politician-behavioral-trends	intelligence-operative	db-changelog-1.30.xml	2026-01-15 04:00:43.107895	363	EXECUTED	9:5d1ec1f15e980de2297f9a1ab18db6c6	createView viewName=view_politician_behavioral_trends	Politician Behavioral Trends View\n        \n        Intelligence Purpose:\n        - Track time-series behavioral patterns for each politician\n        - Monitor absence rates, voting effectiveness, party discipline\n        - Identify behavioral chan...	\N	5.0.1	\N	\N	8446037746
osint-2025111502-party-effectiveness-trends	intelligence-operative	db-changelog-1.30.xml	2026-01-15 04:00:43.135822	364	EXECUTED	9:e90b3cc562a26253599e50803c2ea928	createView viewName=view_party_effectiveness_trends	Party Effectiveness Trends View\n        \n        Intelligence Purpose:\n        - Monitor party-level performance metrics over time\n        - Track win rates, productivity, and collaboration patterns\n        - Identify organizational strengths and ...	\N	5.0.1	\N	\N	8446037746
osint-2025111503-risk-score-evolution	intelligence-operative	db-changelog-1.30.xml	2026-01-15 04:00:43.162786	365	EXECUTED	9:ec6554f6fd6a4de381f8a97d46bd5663	createView viewName=view_risk_score_evolution	Risk Score Evolution View\n        \n        Intelligence Purpose:\n        - Track historical changes in politician risk scores\n        - Monitor severity transitions (escalation/de-escalation)\n        - Identify risk patterns and triggers\n        -...	\N	5.0.1	\N	\N	8446037746
osint-2025111504-committee-productivity-matrix	intelligence-operative	db-changelog-1.30.xml	2026-01-15 04:00:43.189105	366	EXECUTED	9:07244f5572e4a6fe02140b2ee16c586f	createView viewName=view_committee_productivity_matrix	Committee Productivity Matrix View\n        \n        Intelligence Purpose:\n        - Monitor committee output and effectiveness by period\n        - Benchmark committee performance against historical data\n        - Identify productive vs. underperfo...	\N	5.0.1	\N	\N	8446037746
osint-2025111505-performance-indexes	intelligence-operative	db-changelog-1.30.xml	2026-01-15 04:00:43.213488	367	EXECUTED	9:e98bc86dc1435e8492ffaa48d97db931	sql	Performance Indexes for OSINT Intelligence Queries\n        \n        These indexes optimize the most common temporal queries used in\n        intelligence analysis and dashboard operations.	\N	5.0.1	\N	\N	8446037746
ministry-2025111701-effectiveness-trends	intelligence-operative	db-changelog-1.31.xml	2026-01-15 04:00:43.239421	368	EXECUTED	9:427996676625f2b8dce8395553aa0b77	createView viewName=view_ministry_effectiveness_trends	Ministry Effectiveness Trends View\n        \n        Intelligence Purpose:\n        - Track ministry-level performance metrics over time\n        - Monitor document production, legislative output, and staffing\n        - Identify productive vs. underp...	\N	5.0.1	\N	\N	8446037746
ministry-2025111702-productivity-matrix	intelligence-operative	db-changelog-1.31.xml	2026-01-15 04:00:43.265191	369	EXECUTED	9:179ccaf2df5469f386c4f0586073cea7	createView viewName=view_ministry_productivity_matrix	Ministry Productivity Matrix View\n        \n        Intelligence Purpose:\n        - Benchmark ministry performance against peers\n        - Identify most and least productive ministries\n        - Normalize metrics for fair comparison\n        - Suppo...	\N	5.0.1	\N	\N	8446037746
ministry-2025111703-risk-evolution	intelligence-operative	db-changelog-1.31.xml	2026-01-15 04:00:43.291875	370	EXECUTED	9:1a81b471095b1d7cb4d9c78e3393bb93	createView viewName=view_ministry_risk_evolution	Ministry Risk Evolution View\n        \n        Intelligence Purpose:\n        - Track historical changes in ministry risk scores\n        - Monitor risk severity transitions for ministries\n        - Identify risk patterns and triggers at ministry lev...	\N	5.0.1	\N	\N	8446037746
ministry-2025111704-performance-indexes	intelligence-operative	db-changelog-1.31.xml	2026-01-15 04:00:43.299945	371	EXECUTED	9:5186a2215528e93478d78b12fc23941b	sql	Performance Indexes for Ministry Intelligence Queries\n        \n        These indexes optimize the most common temporal queries used in\n        ministry intelligence analysis and government effectiveness dashboards.	\N	5.0.1	\N	\N	8446037746
fix-politician-risk-summary-1.32-001	intelligence-analyst	db-changelog-1.32.xml	2026-01-15 04:00:43.31101	372	EXECUTED	9:a5bd4210d7968d1004790f7d482867e8	dropView viewName=view_politician_risk_summary; createView viewName=view_politician_risk_summary	Fix view_politician_risk_summary to return data\n        \n        Root Cause: The JOIN condition on view_riksdagen_vote_data_ballot_politician_summary_annual\n        was filtering on an exact date (date_trunc('year', CURRENT_DATE - INTERVAL '1 year...	\N	5.0.1	\N	\N	8446037746
verify-other-views-1.32-002	intelligence-analyst	db-changelog-1.32.xml	2026-01-15 04:00:43.315093	373	EXECUTED	9:72d8a71e1aae0b931ebd79e83a10cf1d	sql	Verification changeset for other politician intelligence views\n        \n        After investigation, the following views should work correctly as they\n        query vote_data directly without restrictive date filters on aggregated views:\n        \n...	\N	5.0.1	\N	\N	8446037746
document-fixes-1.32-003	intelligence-analyst	db-changelog-1.32.xml	2026-01-15 04:00:43.318672	374	EXECUTED	9:8a10e26023ff9e74bf785363f03efdd6	sql	Documentation for v1.32 fixes\n        \n        This changeset documents the fixes applied to politician intelligence views\n        for inclusion in DATABASE_VIEW_INTELLIGENCE_CATALOG.md and \n        TROUBLESHOOTING_EMPTY_VIEWS.md.\n        \n       ...	\N	5.0.1	\N	\N	8446037746
fix-goverment-proposals-1.32-004	intelligence-analyst	db-changelog-1.32.xml	2026-01-15 04:00:43.326157	375	EXECUTED	9:c881ba745964943eb175bc0ff9c55520	dropView viewName=view_riksdagen_goverment_proposals; createView viewName=view_riksdagen_goverment_proposals	Fix view_riksdagen_goverment_proposals to return data\n        \n        Root Cause: The view filters by document_type = 'PROP' (uppercase), but\n        the actual data in document_data may use different case variations:\n        - 'prop' (lowercase)...	\N	5.0.1	\N	\N	8446037746
verify-ministry-dependencies-1.32-005	intelligence-analyst	db-changelog-1.32.xml	2026-01-15 04:00:43.33061	376	EXECUTED	9:344f3666045ec90f277df19c3812110c	sql	Verify Ministry View Dependencies\n        \n        The 3 ministry views created in v1.31 depend on:\n        1. assignment_data table with assignment_type = 'Departement'\n        2. view_riksdagen_politician_document materialized view\n        3. Ma...	\N	5.0.1	\N	\N	8446037746
document-ministry-troubleshooting-1.32-006	intelligence-analyst	db-changelog-1.32.xml	2026-01-15 04:00:43.334419	377	EXECUTED	9:11eae01f4f5f0d419aacbe440d4b60f3	sql	Document Ministry View Troubleshooting\n        \n        This changeset provides documentation for TROUBLESHOOTING_EMPTY_VIEWS.md\n        specific to ministry views created in v1.31.\n        \n        Ministry Views Fixed in v1.32:\n        1. view_r...	\N	5.0.1	\N	\N	8446037746
fix-member-proposals-1.33-001	intelligence-analyst	db-changelog-1.33.xml	2026-01-15 04:00:43.342205	378	EXECUTED	9:eac9e9cd1f0bd8fd4859b8d1954aa3c9	dropView viewName=view_riksdagen_member_proposals; createView viewName=view_riksdagen_member_proposals	Fix view_riksdagen_member_proposals to return data\n        \n        Root Cause: The view filters by document_type = 'MOT' (uppercase), but\n        the actual data in document_element contains 'mot' (lowercase).\n        \n        Solution: Use UPPER...	\N	5.0.1	\N	\N	8446037746
fix-committee-member-proposals-1.33-002	intelligence-analyst	db-changelog-1.33.xml	2026-01-15 04:00:43.35039	379	EXECUTED	9:ba278abacebf49620c4f2c950652b71d	dropView viewName=view_riksdagen_committee_parliament_member_proposal; createView viewName=view_riksdagen_committee_parliament_member_proposal	Fix view_riksdagen_committee_parliament_member_proposal to return data\n        \n        Root Cause: Same as view_riksdagen_member_proposals - the view filters \n        by document_type = 'MOT' (uppercase) but data contains 'mot' (lowercase).\n     ...	\N	5.0.1	\N	\N	8446037746
1414872417007-280	pether (generated)	db-changelog-1.7.xml	2026-01-24 15:25:45.113262	525	EXECUTED	9:63d7fcb99c22318c78bd80ac7ad9a2cb	addColumn tableName=document_element		\N	5.0.1	\N	\N	9264743943
fix-crisis-resilience-indicators-1.33-003	intelligence-analyst	db-changelog-1.33.xml	2026-01-15 04:00:43.362408	380	EXECUTED	9:a9fb314829a17674f001f82a1d233f70	sql; createView viewName=view_riksdagen_crisis_resilience_indicators	Fix view_riksdagen_crisis_resilience_indicators to return data\n        \n        Root Cause: The view filters vote values using exact case matches like\n        'Ja', 'Nej', 'Frånvarande', but the actual data contains 'JA', 'NEJ', \n        'FRÅNVARA...	\N	5.0.1	\N	\N	8446037746
recreate-intelligence-dashboard-1.33-003b	intelligence-analyst	db-changelog-1.33.xml	2026-01-15 04:00:43.371946	381	EXECUTED	9:cb8a00ecaa8b7e60d6d2420ad1992a1c	sql; createView viewName=view_riksdagen_intelligence_dashboard	Recreate view_riksdagen_intelligence_dashboard after crisis resilience fix\n        \n        This view was dropped by CASCADE when fixing view_riksdagen_crisis_resilience_indicators.\n        The view aggregates multiple intelligence indicators into...	\N	5.0.1	\N	\N	8446037746
fix-risk-score-evolution-1.33-004	intelligence-analyst	db-changelog-1.33.xml	2026-01-15 04:00:43.383667	382	EXECUTED	9:af3c81ce2158677248721986f3adb9c9	dropView viewName=view_risk_score_evolution; createView viewName=view_risk_score_evolution	Fix view_risk_score_evolution to return data\n        \n        Root Cause: The view requires data from view_riksdagen_vote_data_ballot_politician_summary_daily\n        within a 3-year window, which may not always have recent data if materialized vi...	\N	5.0.1	\N	\N	8446037746
document-fixes-1.33-005	intelligence-analyst	db-changelog-1.33.xml	2026-01-15 04:00:43.387438	383	EXECUTED	9:a18f424c14dc0f2c6d7754d7e378a882	sql	Documentation for v1.33 fixes\n        \n        This changeset documents the fixes applied to 4 empty intelligence views\n        for inclusion in TROUBLESHOOTING_EMPTY_VIEWS.md.\n        \n        Summary of Changes:\n        1. view_riksdagen_member_...	\N	5.0.1	\N	\N	8446037746
1.34-intro	database-architect	db-changelog-1.34.xml	2026-01-15 04:00:43.391147	384	EXECUTED	9:619faeb099abb68856d15be29569099b	sql	Database Changelog v1.34 - Comprehensive Empty View Fixes with Validation\n        \n        Consolidates fixes from issues #7882-#7885 for 12 empty views.\n        Adds pre-flight and post-flight validation for data availability.	\N	5.0.1	\N	\N	8446037746
1.34-preflight	database-architect	db-changelog-1.34.xml	2026-01-15 04:00:43.395416	385	EXECUTED	9:25a3bba16e7a992a8e29fbbe94a7bfcc	sql	Pre-flight: Verify source data exists before applying view fixes	\N	5.0.1	\N	\N	8446037746
1.34-ministry-verify-001	intelligence-analyst	db-changelog-1.34.xml	2026-01-15 04:00:43.399202	386	EXECUTED	9:3e9501744493d58dfd62f8e4eb7ce968	sql	Ministry views - Expected to be empty without ministry assignment data	\N	5.0.1	\N	\N	8446037746
1.34-gov-proposals-002	intelligence-analyst	db-changelog-1.34.xml	2026-01-15 04:00:43.406415	387	EXECUTED	9:98ee9cac78930ae7df9dea29166db568	dropView viewName=view_riksdagen_goverment_proposals; createView viewName=view_riksdagen_goverment_proposals	Fix view_riksdagen_goverment_proposals - broader document_type filter\n        Catches 'prop', 'PROP', 'Proposition' variations	\N	5.0.1	\N	\N	8446037746
1.34-member-proposals-003	intelligence-analyst	db-changelog-1.34.xml	2026-01-15 04:00:43.413836	388	EXECUTED	9:b13f7b02bfd25a9791548c069dc378de	dropView viewName=view_riksdagen_member_proposals; createView viewName=view_riksdagen_member_proposals	Fix view_riksdagen_member_proposals - broader document_type filter\n        Catches 'mot', 'MOT', 'Motion' variations	\N	5.0.1	\N	\N	8446037746
1.34-committee-proposals-004	intelligence-analyst	db-changelog-1.34.xml	2026-01-15 04:00:43.421422	389	EXECUTED	9:acfb00d23673ba2bfbcf78bc83c33bd7	dropView viewName=view_riksdagen_committee_parliament_member_proposal; createView viewName=view_riksdagen_committee_parliament_member_proposal	Fix view_riksdagen_committee_parliament_member_proposal\n        Same broader filter as member proposals	\N	5.0.1	\N	\N	8446037746
1.34-risk-summary-005	intelligence-analyst	db-changelog-1.34.xml	2026-01-15 04:00:43.430608	390	EXECUTED	9:4692e16ed702c30c880c5bf3d6afffcb	dropView viewName=view_politician_risk_summary; createView viewName=view_politician_risk_summary	Fix view_politician_risk_summary - simplified version using direct vote_data\n        Removes dependency on aggregated summary views that may not have data	\N	5.0.1	\N	\N	8446037746
1.34-verify-others-006	intelligence-analyst	db-changelog-1.34.xml	2026-01-15 04:00:43.435195	391	EXECUTED	9:7f86a1ea0b6dacdf6ce2eef8ce19ad8d	sql	Verify other politician and intelligence views exist (from v1.33)	\N	5.0.1	\N	\N	8446037746
1.34-postflight	database-architect	db-changelog-1.34.xml	2026-01-15 04:00:43.444663	392	EXECUTED	9:24f59e1f777d8317d7fb94354be07200	sql	Post-flight: Verify all 12 views and check row counts	\N	5.0.1	\N	\N	8446037746
1.34-documentation	database-architect	db-changelog-1.34.xml	2026-01-15 04:00:43.448232	393	EXECUTED	9:215e4217ceea39d393d357dfab47613b	sql	Documentation for v1.34 comprehensive view fixes\n        \n        Summary:\n        - Pre-flight validation checks source data\n        - Ministry views verified (expected empty without ministry data)\n        - Government proposals view fixed (broad...	\N	5.0.1	\N	\N	8446037746
1.35-intro	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.451668	394	EXECUTED	9:32e18dd7e0d25938d04971bf2130e750	sql	Database Changelog v1.35 - Party Decision Flow View\n        \n        Creates view_riksdagen_party_decision_flow for aggregating proposal\n        decision data by party, enabling party-level legislative effectiveness\n        and coalition analysis.	\N	5.0.1	\N	\N	8446037746
1.35-preflight	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.455644	395	EXECUTED	9:b1f1b765b24f30a4b717ee0da5b5e940	sql	Pre-flight: Verify source data exists before creating view	\N	5.0.1	\N	\N	8446037746
1.35-party-decision-flow-001	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.464678	396	EXECUTED	9:1055961759e4cdf793808144bc58b684	createView viewName=view_riksdagen_party_decision_flow	Create view_riksdagen_party_decision_flow\n        \n        Aggregates proposal decision data by party, committee, decision type, and time period.\n        Provides comprehensive party-level decision intelligence including:\n        - Total proposals...	\N	5.0.1	\N	\N	8446037746
1.35-party-decision-flow-index-002	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.472019	397	EXECUTED	9:1de6549e5d2e5b58509674d1ae8a55c6	sql	Create indexes on base tables for performance optimization of party decision flow queries\n        \n        Indexes on frequently joined and filtered columns:\n        - document_proposal_data.committee (for committee-specific queries)\n        - doc...	\N	5.0.1	\N	\N	8446037746
1.35-postflight	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.478271	398	EXECUTED	9:e091dfbf1f8a01f3049f40855ae047f3	sql	Post-flight: Verify view creation and check initial data	\N	5.0.1	\N	\N	8446037746
1.35-documentation	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.482198	399	EXECUTED	9:a995133e402948e807b1759d61b1b7c5	sql	Documentation for v1.35 party decision flow view\n        \n        Summary:\n        - Creates view_riksdagen_party_decision_flow for party-level decision aggregation\n        - Enables party scorecards, coalition analysis, committee effectiveness tr...	\N	5.0.1	\N	\N	8446037746
1.52-temporal	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.096732	467	EXECUTED	9:9074e222aa474749163a3b258ff0074d	createView viewName=view_election_cycle_temporal_trends		\N	5.0.1	\N	\N	8446037746
1.35-politician-decision-pattern-001	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.49211	400	EXECUTED	9:5db797f3c39c8716ceb1c7fac014ad7e	createView viewName=view_riksdagen_politician_decision_pattern	Create view_riksdagen_politician_decision_pattern\n        \n        Tracks individual politician decision patterns from document_proposal_data,\n        enabling analysis of politician-level proposal success rates, committee work\n        effectivene...	\N	5.0.1	\N	\N	8446037746
1.35-politician-decision-pattern-index-002	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.498527	401	EXECUTED	9:f3b01d3c763eaaa45863294b17329605	sql	Create indexes for performance optimization of politician decision pattern queries\n        \n        Index on person_id for efficient politician-specific queries.\n        These complement the existing base table indexes created in changeSet 002.	\N	5.0.1	\N	\N	8446037746
1.35-politician-decision-pattern-postflight	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.505012	402	EXECUTED	9:d2d93d66a90ac420fd94af497ef5f333	sql	Post-flight: Verify politician decision pattern view creation	\N	5.0.1	\N	\N	8446037746
1.35-decision-temporal-trends-001	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.516149	403	EXECUTED	9:edc0dbf51168562761bc8fc2c7681d44	createView viewName=view_decision_temporal_trends	Create view_decision_temporal_trends\n        \n        Temporal trends view for decision flow analysis from DOCUMENT_PROPOSAL_DATA,\n        enabling time-series analysis of decision patterns, seasonal variations,\n        and predictive forecasting ...	\N	5.0.1	\N	\N	8446037746
1.35-decision-temporal-trends-postflight	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.52266	404	EXECUTED	9:88dd7d0ff0afee32b8e6c317ef3f0577	sql	Post-flight: Verify temporal decision trends view creation and validate data	\N	5.0.1	\N	\N	8446037746
1.79-001	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.573997	666	EXECUTED	9:9be749f4b2ebc7201f998e61dc048eb9	createView viewName=view_riksdagen_party_summary	Fix view_riksdagen_party_summary: correct document join chain (ROOT CAUSE 5),\n        motion type classification using sub_type (ROOT CAUSE 6), and implement\n        previously hardcoded-zero columns (ROOT CAUSE 7).\n\n        Join fix: party_docume...	\N	5.0.2	\N	\N	4213844732
1.35-ministry-decision-impact-001	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.532718	405	EXECUTED	9:d2f6d28b97abf69bccd816fc3258aa8c	createView viewName=view_ministry_decision_impact	Create view_ministry_decision_impact\n        \n        Tracks ministry-initiated proposal outcomes from DOCUMENT_PROPOSAL_DATA,\n        enabling analysis of which government ministries have the highest/lowest\n        success rates for their legisla...	\N	5.0.1	\N	\N	8446037746
1.35-ministry-decision-impact-index-002	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.540372	406	EXECUTED	9:73ef6912726a17535c1aa90fb2427261	sql	Create indexes for performance optimization of ministry decision impact queries\n        \n        Index on document_data.org (ministry_code) and document_type for efficient\n        ministry-specific and government proposal queries.	\N	5.0.1	\N	\N	8446037746
1.35-ministry-decision-impact-postflight	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.545856	407	EXECUTED	9:2b1646700fdb341fd0c3a4e6a16fb8d9	sql	Post-flight: Verify ministry decision impact view creation and validate data	\N	5.0.1	\N	\N	8446037746
1.35-final-summary	intelligence-operative	db-changelog-1.35.xml	2026-01-15 04:00:43.549471	408	EXECUTED	9:3438e13b3d71b2e99c0220e56a54a810	sql	Final documentation summary for v1.35 database changelog\n        \n        Summary of v1.35 views created:\n        1. view_riksdagen_party_decision_flow - Party-level decision aggregation\n        2. view_riksdagen_politician_decision_pattern - Indi...	\N	5.0.1	\N	\N	8446037746
1736000000000-1	import-error-fix	db-changelog-1.36.xml	2026-01-15 04:00:43.555762	409	EXECUTED	9:1b1abcc812870fd0d7432002253e45f6	modifyDataType columnName=note, tableName=document_element	Increase document_element.note from VARCHAR(8192) to VARCHAR(65536) to accommodate long EU committee notes with HTML lists	\N	5.0.1	\N	\N	8446037746
1736000000000-2	import-error-fix	db-changelog-1.36.xml	2026-01-15 04:00:43.561979	410	EXECUTED	9:c3780abe8ec3d2fe6ef55ee2d25541de	modifyDataType columnName=note_title, tableName=document_element	Increase document_element.note_title from VARCHAR(8192) to VARCHAR(16384) for consistency and future-proofing	\N	5.0.1	\N	\N	8446037746
1736000000000-3	import-error-fix	db-changelog-1.36.xml	2026-01-15 04:00:43.568222	411	EXECUTED	9:7d4d1848977aaccfa3635879774c556e	modifyDataType columnName=summary, tableName=document_element	Increase document_element.summary from VARCHAR(8192) to VARCHAR(65536) to match note field size	\N	5.0.1	\N	\N	8446037746
fix-ministry-effectiveness-1.37-001	intelligence-operative	db-changelog-1.37.xml	2026-01-15 04:00:43.578681	412	EXECUTED	9:2eeb05933237619abae29188032cd4d0	dropView viewName=view_ministry_effectiveness_trends; createView viewName=view_ministry_effectiveness_trends	Fix view_ministry_effectiveness_trends with case-insensitive org_code matching\n        \n        Root Cause: org_code values in assignment_data and view_riksdagen_politician_document\n        have inconsistent casing (e.g., "Departement" vs "departe...	\N	5.0.1	\N	\N	8446037746
fix-ministry-productivity-1.37-002	intelligence-operative	db-changelog-1.37.xml	2026-01-15 04:00:43.587691	413	EXECUTED	9:ce261dfdef96131a2f75ddb6101ac655	dropView viewName=view_ministry_productivity_matrix; createView viewName=view_ministry_productivity_matrix	Fix view_ministry_productivity_matrix with case-insensitive org_code matching\n        \n        Same root cause and solution as effectiveness trends view.	\N	5.0.1	\N	\N	8446037746
fix-ministry-risk-evolution-1.37-003	intelligence-operative	db-changelog-1.37.xml	2026-01-15 04:00:43.596511	414	EXECUTED	9:1e6651e08f0b89e129e0ad572f056309	dropView viewName=view_ministry_risk_evolution; createView viewName=view_ministry_risk_evolution	Fix view_ministry_risk_evolution with case-insensitive org_code matching\n        \n        Same root cause and solution as other ministry views.	\N	5.0.1	\N	\N	8446037746
fix-coalition-alignment-1.37-004	intelligence-operative	db-changelog-1.37.xml	2026-01-15 04:00:43.605854	415	EXECUTED	9:71d9f34b8fc38a50bff1687da325dcdb	sql; createView viewName=view_riksdagen_coalition_alignment_matrix	Fix view_riksdagen_coalition_alignment_matrix with expanded date range\n        \n        Root Cause: 2-year date filter too restrictive - many parties only have older vote data.\n        \n        Solution: Expand from 2 years to 5 years to capture m...	\N	5.0.1	\N	\N	8446037746
fix-politician-risk-summary-1.37-005	intelligence-operative	db-changelog-1.37.xml	2026-01-15 04:00:43.615784	416	EXECUTED	9:a50ccc76324b132113a20f9eb3fcc4d6	dropView viewName=view_politician_risk_summary; createView viewName=view_politician_risk_summary	Fix view_politician_risk_summary using direct vote_data aggregation\n        \n        Root Cause: Dependency on materialized view annual summary creates timing issues\n        where data may not exist for specific year calculations.\n        \n       ...	\N	5.0.1	\N	\N	8446037746
fix-risk-score-evolution-1.38-001	intelligence-operative	db-changelog-1.38.xml	2026-01-15 04:00:43.627047	417	EXECUTED	9:157a899e1c10d9b4e540d8749a91b376	dropView viewName=view_risk_score_evolution; createView viewName=view_risk_score_evolution	Fix view_risk_score_evolution by removing materialized view dependency\n        \n        Root Cause: The view depends on view_riksdagen_vote_data_ballot_politician_summary_daily\n        (materialized) which is not populated in schema-only databases...	\N	5.0.1	\N	\N	8446037746
1.52-drop-comparative	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.104001	468	EXECUTED	9:3dae6e62c2c3f45f25b6b2b22e96392b	sql		\N	5.0.1	\N	\N	8446037746
fix-crisis-resilience-1.38-002	intelligence-operative	db-changelog-1.38.xml	2026-01-15 04:00:43.638217	418	EXECUTED	9:0f94269ccd9de3e53cbebdcff429df4e	dropView viewName=view_riksdagen_crisis_resilience_indicators; createView viewName=view_riksdagen_crisis_resilience_indicators	Fix view_riksdagen_crisis_resilience_indicators with expanded date range\n        \n        Root Cause: 2-year date filter is too restrictive for detecting crisis patterns.\n        Similar to coalition alignment view issue (fixed in 1.37).\n        \n...	\N	5.0.1	\N	\N	8446037746
fix-politician-influence-1.38-003	intelligence-operative	db-changelog-1.38.xml	2026-01-15 04:00:43.64793	419	EXECUTED	9:70aeb6db83f75b0e0f7fa78095c4b9d0	dropView viewName=view_riksdagen_politician_influence_metrics; createView viewName=view_riksdagen_politician_influence_metrics	Fix view_riksdagen_politician_influence_metrics with expanded parameters\n        \n        Root Cause: 1-year date filter with 20-vote minimum threshold is too restrictive,\n        filtering out meaningful network connections and influence patterns...	\N	5.0.1	\N	\N	8446037746
fix-voting-anomaly-1.38-004	intelligence-operative	db-changelog-1.38.xml	2026-01-15 04:00:43.657692	420	EXECUTED	9:2f4ac1c1a94ad8b9487e899f0751b225	dropView viewName=view_riksdagen_voting_anomaly_detection; createView viewName=view_riksdagen_voting_anomaly_detection	Fix view_riksdagen_voting_anomaly_detection with expanded date range\n        \n        Root Cause: 1-year date filter provides insufficient historical context for\n        reliable anomaly detection. Pattern recognition benefits from longer periods....	\N	5.0.1	\N	\N	8446037746
fix-coalition-alignment-1.38-0022	intelligence-operative	db-changelog-1.38.xml	2026-01-15 04:00:43.66699	421	EXECUTED	9:8dda1ad871d012db9cd2a4d950e07152	sql; createView viewName=view_riksdagen_coalition_alignment_matrix	Transformed coalition alignment view to match existing Java entity structure.\n        Maps new analytical logic to legacy column names for backwards compatibility.\n        \n        Mapping:\n        - total_votes -> shared_votes\n        - aligned_v...	\N	5.0.1	\N	\N	8446037746
fix-politician-risk-summary-matview-1.39-001	intelligence-operative	db-changelog-1.39.xml	2026-01-15 04:00:43.677796	422	EXECUTED	9:4c6fe59977c8548ce3b1bcceab3115b6	dropView viewName=view_politician_risk_summary; createView viewName=view_politician_risk_summary	Fix view_politician_risk_summary by removing materialized view dependency\n        \n        Root Cause: The view uses LEFT JOIN on view_riksdagen_politician_document\n        (materialized view). Even with LEFT JOIN, PostgreSQL cannot execute the\n  ...	\N	5.0.1	\N	\N	8446037746
fix-ministry-effectiveness-1.39-001	intelligence-operative	db-changelog-1.39.xml	2026-01-15 04:00:43.688335	423	EXECUTED	9:ebd5c2f743b97d91375d1814fed3d0bb	dropView viewName=view_ministry_effectiveness_trends; createView viewName=view_ministry_effectiveness_trends	Fix view_ministry_effectiveness_trends to return actual ministry data\n        \n        Root Cause: View filtered by LOWER(org_code) LIKE '%departement%' but actual\n        ministry org_codes are short codes ("KN", "N", etc.) that don't contain "de...	\N	5.0.1	\N	\N	8446037746
fix-ministry-productivity-1.39-002	intelligence-operative	db-changelog-1.39.xml	2026-01-15 04:00:43.697704	424	EXECUTED	9:ce83258b81e92bd942a9f97bb9a9f197	dropView viewName=view_ministry_productivity_matrix; createView viewName=view_ministry_productivity_matrix	Fix view_ministry_productivity_matrix to return actual ministry data\n        \n        Root Cause: Same as view_ministry_effectiveness_trends - view filtered by \n        LOWER(org_code) LIKE '%departement%' but actual ministry org_codes are short \n...	\N	5.0.1	\N	\N	8446037746
fix-ministry-risk-evolution-1.39-003	intelligence-operative	db-changelog-1.39.xml	2026-01-15 04:00:43.706868	425	EXECUTED	9:e8ee5d2c422b306e4e36882237f5a21b	dropView viewName=view_ministry_risk_evolution; createView viewName=view_ministry_risk_evolution	Fix view_ministry_risk_evolution to return actual ministry data\n        \n        Root Cause: Same as view_ministry_effectiveness_trends - view filtered by \n        LOWER(org_code) LIKE '%departement%' but actual ministry org_codes are short \n     ...	\N	5.0.1	\N	\N	8446037746
verify-ministry-views-1.39-004	intelligence-operative	db-changelog-1.39.xml	2026-01-15 04:00:43.712949	426	EXECUTED	9:0e011b36ae0faab2f83c753e34a86658	sql	Post-flight verification for all three ministry views\n        \n        Checks:\n        1. All three views exist and can be queried\n        2. Views return data when ministry assignments are present\n        3. Row counts are reasonable (>0 when dat...	\N	5.0.1	\N	\N	8446037746
document-ministry-fix-1.39-005	intelligence-operative	db-changelog-1.39.xml	2026-01-15 04:00:43.716544	427	EXECUTED	9:830bb959528e4365f577fc21b94da2a8	sql	Documentation for v1.39 ministry views fix\n        \n        Summary:\n        - Root cause: Incorrect filter on org_code LIKE '%departement%'\n        - Actual data: org_code has short codes like 'KN', 'N', not full names\n        - Solution: Remove ...	\N	5.0.1	\N	\N	8446037746
fix-crisis-resilience-1.40-001	intelligence-operative	db-changelog-1.40.xml	2026-01-15 04:00:43.728284	428	EXECUTED	9:e34edc89148de2ec4f4766d8cb341105	dropView viewName=view_riksdagen_crisis_resilience_indicators; createView viewName=view_riksdagen_crisis_resilience_indicators	Fix view_riksdagen_crisis_resilience_indicators to return data\n        \n        Root Cause: The crisis period detection algorithm had gaps - months between\n        1x and 1.5x average were not classified as either crisis or normal periods.\n       ...	\N	5.0.1	\N	\N	8446037746
verify-crisis-resilience-1.40-002	intelligence-operative	db-changelog-1.40.xml	2026-01-15 04:00:43.733722	429	EXECUTED	9:1c92d454c7d1820ed9dc736aa6e104b1	sql	Post-flight verification for view_riksdagen_crisis_resilience_indicators\n        \n        Checks:\n        1. View exists and can be queried\n        2. View returns data when vote_data and person_data are populated\n        3. Reports row count for ...	\N	5.0.1	\N	\N	8446037746
recreate-intelligence-dashboard-1.40-003	intelligence-operative	db-changelog-1.40.xml	2026-01-15 04:00:43.742516	430	EXECUTED	9:8817dc415e89d5760524c4ccf8b5b406	createView viewName=view_riksdagen_intelligence_dashboard	Recreate view_riksdagen_intelligence_dashboard after crisis resilience fix\n        \n        This view was dropped by CASCADE when fixing view_riksdagen_crisis_resilience_indicators\n        in changeSet fix-crisis-resilience-1.40-001 (cascadeConstr...	\N	5.0.1	\N	\N	8446037746
fix-risk-score-evolution-1.41-001	intelligence-operative	db-changelog-1.41.xml	2026-01-15 04:00:43.754522	431	EXECUTED	9:37f6b5ea6bb65b3a4437a01391adc4a6	dropView viewName=view_risk_score_evolution; createView viewName=view_risk_score_evolution	Fix view_risk_score_evolution to return data with correct rebel rate calculation\n        \n        Root Cause: The v1.38 fix used incorrect logic comparing vote type to party name:\n        `vd.vote != vd.party` (e.g., 'Ja' != 'S') which is always t...	\N	5.0.1	\N	\N	8446037746
verify-risk-score-evolution-1.41-002	intelligence-operative	db-changelog-1.41.xml	2026-01-15 04:00:43.760167	432	EXECUTED	9:aebf12c8545cfa083ec15d44c1563fb3	sql	Post-flight verification for view_risk_score_evolution\n        \n        Checks:\n        1. View exists and can be queried\n        2. View returns data when vote_data and person_data are populated\n        3. Reports row count for monitoring	\N	5.0.1	\N	\N	8446037746
1.52-comparative	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.113906	469	EXECUTED	9:1c92eb3a8ae1d27aae916154b273abf3	createView viewName=view_election_cycle_comparative_analysis		\N	5.0.1	\N	\N	8446037746
fix-ministry-productivity-matrix-matview-1.42-001	intelligence-operative	db-changelog-1.42.xml	2026-01-15 04:00:43.769393	433	EXECUTED	9:d412e3784f5066b454b4444785c9b190	dropView viewName=view_ministry_productivity_matrix; createView viewName=view_ministry_productivity_matrix	Fix view_ministry_productivity_matrix by removing materialized view dependency\n        \n        Root Cause: The view uses LEFT JOIN on view_riksdagen_politician_document\n        (materialized view). Even with LEFT JOIN, PostgreSQL cannot execute t...	\N	5.0.1	\N	\N	8446037746
fix-ministry-effectiveness-trends-matview-1.42-002	intelligence-operative	db-changelog-1.42.xml	2026-01-15 04:00:43.78003	434	EXECUTED	9:c312343fc7c7e60a7228cf621f2ef36c	dropView viewName=view_ministry_effectiveness_trends; createView viewName=view_ministry_effectiveness_trends	Fix view_ministry_effectiveness_trends by removing materialized view dependency\n        \n        Root Cause: The view uses LEFT JOIN on view_riksdagen_politician_document\n        (materialized view). Even with LEFT JOIN, PostgreSQL cannot execute ...	\N	5.0.1	\N	\N	8446037746
fix-ministry-risk-evolution-matview-1.42-003	intelligence-operative	db-changelog-1.42.xml	2026-01-15 04:00:43.789461	435	EXECUTED	9:cbb86defc0af14dbb2c2613c7a4eecb3	dropView viewName=view_ministry_risk_evolution; createView viewName=view_ministry_risk_evolution	Fix view_ministry_risk_evolution by removing materialized view dependency\n        \n        Root Cause: The view uses LEFT JOIN on view_riksdagen_politician_document\n        (materialized view). Even with LEFT JOIN, PostgreSQL cannot execute the\n  ...	\N	5.0.1	\N	\N	8446037746
verify-ministry-views-1.42-004	intelligence-operative	db-changelog-1.42.xml	2026-01-15 04:00:43.795719	436	EXECUTED	9:5364a65df2b784a3c738ef622c551a7a	sql	Post-flight verification for all three ministry views\n        \n        Checks:\n        1. Views exist and can be queried (no materialized view error)\n        2. Views return data when ministry assignments exist\n        3. Reports row counts for mo...	\N	5.0.1	\N	\N	8446037746
fix-risk-score-evolution-matview-1.42-005	intelligence-operative	db-changelog-1.42.xml	2026-01-15 04:00:43.807523	437	EXECUTED	9:17f7c63ce28d9c98df086fd35afa7991	dropView viewName=view_risk_score_evolution; createView viewName=view_risk_score_evolution	Fix view_risk_score_evolution by removing materialized view dependency\n        \n        Root Cause: The view was re-introduced in db-changelog-1.41.xml (for rebel rate fix)\n        but still referenced view_riksdagen_politician_document (materiali...	\N	5.0.1	\N	\N	8446037746
document-ministry-views-fix-1.42-006	intelligence-operative	db-changelog-1.42.xml	2026-01-15 04:00:43.810988	438	EXECUTED	9:0f5bf4edfd8c9b20d4ace561ff91751c	sql	Documentation for v1.42 views fix\n        \n        Summary:\n        - Root cause: Dependency on unpopulated materialized view view_riksdagen_politician_document\n        - Solution: Replace materialized view with direct base table queries in all vi...	\N	5.0.1	\N	\N	8446037746
fix-ministry-risk-evolution-periods-1.43-001	intelligence-operative	db-changelog-1.43.xml	2026-01-15 04:00:43.820575	439	EXECUTED	9:ec9451d1bc2a00638ab3ccaa919906fa	dropView viewName=view_ministry_risk_evolution; createView viewName=view_ministry_risk_evolution	Fix view_ministry_risk_evolution to return data for all ministries\n        \n        Root Cause: The view was filtering out rows where assessment_period IS NULL.\n        When ministries have no document data, the DATE_TRUNC from LEFT JOIN results\n ...	\N	5.0.1	\N	\N	8446037746
verify-ministry-risk-evolution-1.43-002	intelligence-operative	db-changelog-1.43.xml	2026-01-15 04:00:43.826229	440	EXECUTED	9:4db518cdbee1433304f8c27a4015d93c	sql	Post-flight verification for view_ministry_risk_evolution\n        \n        Checks:\n        1. View exists and can be queried\n        2. View returns rows when ministry assignments exist\n        3. Reports row count for validation	\N	5.0.1	\N	\N	8446037746
document-ministry-risk-evolution-fix-1.43-003	intelligence-operative	db-changelog-1.43.xml	2026-01-15 04:00:43.829826	441	EXECUTED	9:1dde305304eb6f05cc4091cf8a4fb217	sql	Documentation for v1.43 view_ministry_risk_evolution fix\n        \n        Summary:\n        - Root cause: View filtered out ministries with no documents due to NULL assessment_period\n        - Solution: Generate quarterly periods independently and ...	\N	5.0.1	\N	\N	8446037746
fix-forste-vice-talman-1.44-001	intelligence-operative	db-changelog-1.44.xml	2026-01-15 04:00:43.852336	442	EXECUTED	9:b279fddaf24fd37a38406eda28c39814	sqlFile path=view_riksdagen_politician_experience_summary_v1.44.sql	Fix view_riksdagen_politician_experience_summary to include Förste vice talman\n        in the talmansuppdrag role scoring alongside Andre and Tredje vice talman.\n        All three Deputy Speaker roles now weighted equally at 750.0.	\N	5.0.1	\N	\N	8446037746
fix-committee-referral-1.45-001	intelligence-operative	db-changelog-1.45.xml	2026-01-15 04:00:43.863264	443	EXECUTED	9:d7ef57d9369d4edbe41fed03e5280238	sqlFile path=view_decision_temporal_trends_v1.45.sql	Fix view_decision_temporal_trends to include committee_referral_decisions column.\n        Pattern: UPPER(chamber) ~~ '%UTSKOTT%' catches 7,049 records previously uncategorized.	\N	5.0.1	\N	\N	8446037746
fix-committee-referral-1.45-002	intelligence-operative	db-changelog-1.45.xml	2026-01-15 04:00:43.873525	444	EXECUTED	9:b87b9d81c1c3c16773c3c0daddb069a3	sqlFile path=view_ministry_decision_impact_v1.45.sql	Fix view_ministry_decision_impact to include committee_referral_proposals column\n        and committee_referral_rate. Also updates other_decisions to exclude committee referrals.\n        Pattern: UPPER(chamber) ~~ '%UTSKOTT%' catches 7,049 records...	\N	5.0.1	\N	\N	8446037746
add-grouped-role-tracking-1.46-001	intelligence-operative	db-changelog-1.46.xml	2026-01-15 04:00:43.892993	445	EXECUTED	9:24893749a205bdb277ad61e924927289	sqlFile path=view_riksdagen_politician_v1.46.sql	Add grouped committee leadership role tracking to view_riksdagen_politician.\n        Adds 12 new columns organized into 6 role categories (total + current for each):\n        - Committee Chair (Ordförande only, committee context)\n        - Committe...	\N	5.0.1	\N	\N	8446037746
fix-swedish-status-politician-risk-summary-1.47-001	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.903962	446	EXECUTED	9:5c5b1ee4f1ab3daed6d35bf6252a217f	sql; createView viewName=view_politician_risk_summary	Fix view_politician_risk_summary to use Swedish status values\n        \n        Root Cause: View filtered on status = 'active' but actual data uses Swedish values\n        Solution: Filter on Swedish status values for active politicians:\n          -...	\N	5.0.1	\N	\N	8446037746
fix-swedish-status-crisis-resilience-1.47-002	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.915801	447	EXECUTED	9:3568262c4405a3fe56b1d47695f1ae7a	sql; createView viewName=view_riksdagen_crisis_resilience_indicators	Fix view_riksdagen_crisis_resilience_indicators to use Swedish status values\n        \n        Root Cause: View filtered on status IN ('active', 'Active', 'ACTIVE') \n        but actual data uses Swedish values\n        \n        Solution: Filter on S...	\N	5.0.1	\N	\N	8446037746
fix-swedish-status-influence-metrics-1.47-003	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.925272	448	EXECUTED	9:d6faabdbd6a129c32d2fb102a0635d72	sql; createView viewName=view_riksdagen_politician_influence_metrics	Fix view_riksdagen_politician_influence_metrics to use Swedish status values\n        \n        Root Cause: View filtered on status IN ('active', 'Active', 'ACTIVE')\n        but actual data uses Swedish values\n        \n        Solution: Filter on Sw...	\N	5.0.1	\N	\N	8446037746
fix-swedish-status-voting-anomaly-1.47-004	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.934785	449	EXECUTED	9:8b18eec250b4170521adea8d2326f670	sql; createView viewName=view_riksdagen_voting_anomaly_detection	Fix view_riksdagen_voting_anomaly_detection to use Swedish status values\n        \n        Root Cause: View filtered on status IN ('active', 'Active', 'ACTIVE')\n        but actual data uses Swedish values\n        \n        Solution: Filter on Swedis...	\N	5.0.1	\N	\N	8446037746
fix-swedish-status-risk-evolution-1.47-005	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.946921	450	EXECUTED	9:0f80c8ee6622ee192f99865e293d2da2	sql; createView viewName=view_risk_score_evolution	Fix view_risk_score_evolution to use Swedish status values\n        \n        Root Cause: View filtered on status IN ('active', 'Active', 'ACTIVE')\n        but actual data uses Swedish values\n        \n        Solution: Filter on Swedish status value...	\N	5.0.1	\N	\N	8446037746
verify-swedish-status-fixes-1.47-006	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.955166	451	EXECUTED	9:5592a1b5dde1654d5065376cec26f06d	sql; sql; sql; sql; sql	Post-flight verification for all 5 fixed views - simple count query	\N	5.0.1	\N	\N	8446037746
fix-vote-case-influence-metrics-1.47-007	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.964871	452	EXECUTED	9:0fcbce98a67d51b68d6b02b42f9a7579	sql; createView viewName=view_riksdagen_politician_influence_metrics	Fix view_riksdagen_politician_influence_metrics vote case sensitivity and thresholds\n        \n        Root Cause #1: View filtered on vote IN ('Ja', 'Nej') but actual data uses\n        uppercase 'JA', 'NEJ'. This resulted in 0 co-voting pairs bein...	\N	5.0.1	\N	\N	8446037746
fix-ministry-productivity-join-1.47-008	intelligence-operative	db-changelog-1.47.xml	2026-01-15 04:00:43.974493	453	EXECUTED	9:fd90bb0cc6c9b7c763c485c0f398e7b3	sql; createView viewName=view_ministry_productivity_matrix	Fix view_ministry_productivity_matrix incorrect join condition\n        \n        Root Cause: View joined on LOWER(doc.org) = m.org_code_lower, but:\n        - Ministry org_codes are short codes like 'Ku', 'Fi', 'S'\n        - Document org values use ...	\N	5.0.1	\N	\N	8446037746
rebalance-thresholds-only-politician-risk-1.48-001	intelligence-operative	db-changelog-1.48.xml	2026-01-15 04:00:43.985186	454	EXECUTED	9:2c9a0905bacd6439df58120a2b2d5283	sql; createView viewName=view_politician_risk_summary	Rebalance politician risk score thresholds ONLY - formula unchanged from v1.47\n        \n        Changes:\n        - CRITICAL: ≥65 (down from ≥70)\n        - HIGH: ≥45 (down from ≥50) \n        - MEDIUM: ≥25 (down from ≥30)\n        - LOW: <25 (down fr...	\N	5.0.1	\N	\N	8446037746
rebalance-thresholds-risk-score-evolution-1.49-001	intelligence-operative	db-changelog-1.49.xml	2026-01-15 04:00:43.997168	455	EXECUTED	9:00807277e56d501693ff08ed9f6a55ee	sql; createView viewName=view_risk_score_evolution	Rebalance view_risk_score_evolution thresholds to match v1.48 changes\n        \n        Changes:\n        - CRITICAL: ≥65 (down from ≥70)\n        - HIGH: ≥45 (down from ≥50) \n        - MODERATE: ≥25 (down from ≥30)\n        - LOW: <25 (down from <30)...	\N	5.0.1	\N	\N	8446037746
fix-rebel-calculation-risk-score-evolution-1.50-001	intelligence-operative	db-changelog-1.50.xml	2026-01-15 04:00:44.010281	456	EXECUTED	9:5fe64a5feb9f29fbb32e609d9bacc58e	sql; createView viewName=view_risk_score_evolution	Fix view_risk_score_evolution rebel vote calculation from v1.49\n        \n        Root Cause: v1.49 used incorrect logic comparing vote type to party name:\n        `vd.vote != vd.party` (e.g., 'Ja' != 'S') which is always true.\n        \n        Cor...	\N	5.0.1	\N	\N	8446037746
1.51-intro	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.013953	457	EXECUTED	9:0a83ea96daebcd37c1daac41ec5e5e71	sql	Database Changelog v1.51 - Historical Election Cycle Trend Views (REVISED)\n        \n        Creates 6 META/META-level views aggregating existing advanced analytics\n        with Swedish parliamentary election cycle temporal dimensions.\n        \n   ...	\N	5.0.1	\N	\N	8446037746
1.51-election-cycle-temporal-001	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.024826	458	EXECUTED	9:bb78984050438e4350cb56e23b08177f	createView viewName=view_election_cycle_temporal_trends	REVISED: Enhanced with view_decision_temporal_trends and view_committee_productivity\n        \n        Framework: Temporal Analysis (35 supporting views, 20+ risk rules)\n        \n        Source Views (COMPREHENSIVE META level):\n        - view_polit...	\N	5.0.1	\N	\N	8446037746
1.51-election-cycle-comparative-002	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.033439	459	EXECUTED	9:59dca636389d99b4d02cd2ac8b8c398c	createView viewName=view_election_cycle_comparative_analysis	REVISED: Enhanced with view_party_performance_metrics and view_committee_productivity_matrix\n        \n        Framework: Comparative Analysis (26 supporting views, 15+ risk rules)\n        \n        Source Views (COMPREHENSIVE):\n        - view_riksd...	\N	5.0.1	\N	\N	8446037746
1.51-election-cycle-pattern-003	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.042507	460	EXECUTED	9:215e67e21fa62af1719d7925d2b94cd9	createView viewName=view_election_cycle_anomaly_pattern	REVISED: Enhanced with view_riksdagen_voting_anomaly_detection and view_politician_risk_summary\n        \n        Framework: Pattern Recognition (23 supporting views, 12/13 risk rules)\n        \n        Source Views (COMPREHENSIVE):\n        - view_r...	\N	5.0.1	\N	\N	8446037746
1.51-election-cycle-predictive-004	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.051001	461	EXECUTED	9:9ce83ab4f38694a6a8179c735d193676	createView viewName=view_election_cycle_predictive_intelligence	REVISED: Enhanced with view_ministry_risk_evolution and view_party_effectiveness_trends\n        \n        Framework: Predictive Intelligence (14 supporting views, 8/8 risk rules)\n        \n        Source Views (COMPREHENSIVE):\n        - view_risk_sc...	\N	5.0.1	\N	\N	8446037746
1.51-election-cycle-network-005	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.059059	462	EXECUTED	9:a86d07e91e78055147bdadc240506395	createView viewName=view_election_cycle_network_analysis	REVISED: Enhanced with view_riksdagen_politician_influence_metrics\n        \n        Framework: Network Analysis (11 supporting views, 3/4 risk rules)\n        \n        Source Views (COMPREHENSIVE):\n        - view_riksdagen_coalition_alignment_matri...	\N	5.0.1	\N	\N	8446037746
1.51-election-cycle-decision-006	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.067537	463	EXECUTED	9:d0b7f1d82b8f60cf365896c37e450668	createView viewName=view_election_cycle_decision_intelligence	REVISED: Enhanced with view_decision_temporal_trends and view_ministry_decision_impact\n        \n        Framework: Decision Intelligence (5 supporting views, 5/5 risk rules)\n        \n        Source Views (COMPREHENSIVE):\n        - view_riksdagen_p...	\N	5.0.1	\N	\N	8446037746
1.51-validation	intelligence-operative	db-changelog-1.51.xml	2026-01-15 04:00:44.073442	464	EXECUTED	9:19fe339fc7201bece36ae3a24021f556	sql	Validate that all 6 election cycle views were created successfully\n        with enhanced comprehensive META/META-level integration.	\N	5.0.1	\N	\N	8446037746
1.52-intro	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.077174	465	EXECUTED	9:d4b0f9ce138edd5a903956bc588bb30c	sql	v1.52 Statistical Enhancements	\N	5.0.1	\N	\N	8446037746
1.52-drop-temporal	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.084217	466	EXECUTED	9:0dbe86ecc11a8351966c5f65f40c7626	sql		\N	5.0.1	\N	\N	8446037746
1.52-anomaly	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.13108	471	EXECUTED	9:37c847cea2b1200579026498cd8cc993	createView viewName=view_election_cycle_anomaly_pattern		\N	5.0.1	\N	\N	8446037746
1.52-drop-predictive	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.138162	472	EXECUTED	9:d1cb7b0708ef6459dcb66e41d14b6361	sql		\N	5.0.1	\N	\N	8446037746
1.52-predictive	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.147641	473	EXECUTED	9:f2067de3346b752674c7e01acec65784	createView viewName=view_election_cycle_predictive_intelligence		\N	5.0.1	\N	\N	8446037746
1.52-drop-network	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.154259	474	EXECUTED	9:94358f3a3750a93fcc40a27aa952686c	sql		\N	5.0.1	\N	\N	8446037746
1.52-network	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.163045	475	EXECUTED	9:00b61c34f8f941fbc6e8550c81c067b1	createView viewName=view_election_cycle_network_analysis		\N	5.0.1	\N	\N	8446037746
1.52-drop-decision	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.16978	476	EXECUTED	9:2188e8b401150b744ff73f0b717339a7	sql		\N	5.0.1	\N	\N	8446037746
1.52-decision	intelligence-operative-analytics	db-changelog-1.52.xml	2026-01-15 04:00:44.17917	477	EXECUTED	9:085fba297cc8a8d3dfa640203a9c0c07	createView viewName=view_election_cycle_decision_intelligence		\N	5.0.1	\N	\N	8446037746
1.53-intro	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.141989	478	EXECUTED	9:51e2954d1bceee0aa41653aed4cf412c	sql	Database Changelog v1.53 (OPTIMIZED & ENHANCED) - Party Longitudinal Analysis\n        \n        Creates 3 views tracking party performance using Swedish parliament cycles,\n        building on existing optimized views (view_riksdagen_vote_data_ballo...	\N	5.0.1	\N	\N	8952677820
1.53-drop-party-performance	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.148782	479	EXECUTED	9:e597cc3a5205328ad149858ab31afde9	sql		\N	5.0.1	\N	\N	8952677820
1.53-party-longitudinal-001	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.186826	480	EXECUTED	9:f7a20e73ce0c531579fc4f4a0b523a03	createView viewName=view_riksdagen_party_longitudinal_performance	Party Longitudinal Performance View (2002-2026) - OPTIMIZED & ENHANCED\n        \n        Framework: Comparative Analysis (26 supporting views, 15+ risk rules)\n        \n        OPTIMIZATIONS:\n        - Builds on view_party_performance_metrics (curre...	\N	5.0.1	\N	\N	8952677820
1.53-drop-coalition-evolution	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.192503	481	EXECUTED	9:5cbc544d51f134307f796d9a81375c4c	sql		\N	5.0.1	\N	\N	8952677820
1.53-party-coalition-002	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.218961	482	EXECUTED	9:ec3bbe899a38b29840da3ec8c3985238	createView viewName=view_riksdagen_party_coalition_evolution	Party Coalition Evolution View (2002-2026) - OPTIMIZED & ENHANCED\n        \n        Framework: Comparative Analysis, Network Analysis\n        \n        OPTIMIZATIONS:\n        - Builds on view_riksdagen_vote_data_ballot_party_summary_annual\n        -...	\N	5.0.1	\N	\N	8952677820
1.53-drop-electoral-trends	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.227168	483	EXECUTED	9:f28b6a24f0a201f2afc5bd026db93a75	sql		\N	5.0.1	\N	\N	8952677820
1.53-party-electoral-003	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.2577	484	EXECUTED	9:141eb5d459791ae1848b861a93771dff	createView viewName=view_riksdagen_party_electoral_trends	Party Electoral Trends View (2002-2026) - OPTIMIZED & ENHANCED\n        \n        Framework: Comparative Analysis, Predictive Intelligence\n        \n        OPTIMIZATIONS:\n        - Builds on view_party_performance_metrics (active members, documents)...	\N	5.0.1	\N	\N	8952677820
1.53-validation	intelligence-operative	db-changelog-1.53.xml	2026-01-21 00:44:39.274031	485	EXECUTED	9:ea784e6f23a71274961868a13943099e	sql	Validate that all 3 optimized party longitudinal views were created successfully\n        with comprehensive KPIs, advanced statistical functions, and building on existing views.	\N	5.0.1	\N	\N	8952677820
1.54-intro	intelligence-operative-analytics	db-changelog-1.54.xml	2026-01-21 00:44:39.279894	486	EXECUTED	9:5297d4ff56363d1961c9ed8022a2f2e0	sql	v1.54 Government Body Data Integration from ESV	\N	5.0.1	\N	\N	8952677820
1.54-create-government-body-data-table	intelligence-operative-analytics	db-changelog-1.54.xml	2026-01-21 00:44:39.293187	487	EXECUTED	9:ececfb8343eaa00195e944896c729962	createTable tableName=government_body_data	Create government_body_data table to store Swedish government body information from ESV	\N	5.0.1	\N	\N	8952677820
1.54-add-government-body-indexes	intelligence-operative-analytics	db-changelog-1.54.xml	2026-01-21 00:44:39.304063	488	EXECUTED	9:d4611e87e7ecd3c83c57c45009042471	createIndex indexName=idx_gov_body_year, tableName=government_body_data; createIndex indexName=idx_gov_body_name, tableName=government_body_data; createIndex indexName=idx_gov_body_ministry, tableName=government_body_data; createIndex indexName=id...	Add indexes to government_body_data table for query performance	\N	5.0.1	\N	\N	8952677820
1.54-add-government-body-comments	intelligence-operative-analytics	db-changelog-1.54.xml	2026-01-21 00:44:39.31771	489	EXECUTED	9:72d9e688519a72ac461c65c1842e91b9	sql	Add documentation comments to government_body_data table and columns	\N	5.0.1	\N	\N	8952677820
1.55-intro	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.323403	490	EXECUTED	9:ae819f4af55b6727e7d064337c41a277	sql	v1.55 Seasonal Trend Analysis - Q4 Pre-Election Activity Patterns	\N	5.0.1	\N	\N	8952677820
1.55-create-seasonal-quarterly-activity-view	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.336862	491	EXECUTED	9:54fb979a48e340a89206c252a0b5277d	createView viewName=view_riksdagen_seasonal_quarterly_activity	Create view_riksdagen_seasonal_quarterly_activity for quarterly pattern analysis.\n            Aggregates Q1-Q4 activity patterns across 24 years (2002-2026) for election cycle analysis.\n            Includes baseline calculation for non-election ye...	\N	5.0.1	\N	\N	8952677820
1.55-document-seasonal-quarterly-activity-view	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.34552	492	EXECUTED	9:33b75f2117ac0ec9c516453e836bebb2	sql	Add documentation for view_riksdagen_seasonal_quarterly_activity	\N	5.0.1	\N	\N	8952677820
1.55-create-q4-election-comparison-view	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.355271	493	EXECUTED	9:4411891d81db15a98164476b5c168c61	createView viewName=view_riksdagen_q4_election_year_comparison	Create view_riksdagen_q4_election_year_comparison for Q4 pre-election activity analysis.\n            Compares Q4 activity in election years vs non-election years to detect pre-election surge patterns.\n            Supports predictive modeling of fu...	\N	5.0.1	\N	\N	8952677820
1.55-document-q4-election-comparison-view	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.363889	494	EXECUTED	9:b497414f0b045cb3bb0ccb8e6961aed3	sql	Add documentation for view_riksdagen_q4_election_year_comparison	\N	5.0.1	\N	\N	8952677820
1.59-document-seasonal-activity-patterns-view	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.58592	514	EXECUTED	9:da72424adcb1598d06e94c7ad780e140	sql	Add documentation for view_riksdagen_seasonal_activity_patterns	\N	5.0.1	\N	\N	8952677820
1414872417007-281	pether (generated)	db-changelog-1.7.xml	2026-01-24 15:25:45.118433	526	EXECUTED	9:083be705e77430564ef073b8269b53d2	createTable tableName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0		\N	5.0.1	\N	\N	9264743943
1.55-create-seasonal-anomaly-detection-view	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.376176	495	EXECUTED	9:807748b3d9dfc5d73c61bb4ef057d8c8	createView viewName=view_riksdagen_seasonal_anomaly_detection	Create view_riksdagen_seasonal_anomaly_detection for identifying activity anomalies.\n            Flags quarters with activity significantly above or below baseline using z-score thresholds (moderate >1.5, high >2, critical >3 standard deviations) ...	\N	5.0.1	\N	\N	8952677820
1.55-document-seasonal-anomaly-detection-view	intelligence-operative	db-changelog-1.55.xml	2026-01-21 00:44:39.385957	496	EXECUTED	9:25e86a40c1e43484634d4d66a3b5058f	sql	Add documentation for view_riksdagen_seasonal_anomaly_detection	\N	5.0.1	\N	\N	8952677820
career-trajectory-1.56-001	intelligence-operative	db-changelog-1.56.xml	2026-01-21 00:44:39.401269	497	EXECUTED	9:bb6a81dcfbb98fab873de3bcafc0f378	sqlFile path=view_riksdagen_politician_career_trajectory_v1.56.sql	Create view_riksdagen_politician_career_trajectory to track politician performance \n        across election cycles (2002-2026). Provides attendance rates, win rates, leadership roles,\n        and documents authored per cycle. Classifies career sta...	\N	5.0.1	\N	\N	8952677820
role-evolution-1.56-002	intelligence-operative	db-changelog-1.56.xml	2026-01-21 00:44:39.415066	498	EXECUTED	9:4d9b6569f4d7641f51c98e71f0466302	sqlFile path=view_riksdagen_politician_role_evolution_v1.56.sql	Create view_riksdagen_politician_role_evolution to track politician career progression\n        through different positions. Classifies roles into tiers (minister/speaker/party leader/\n        committee chair/member/substitute), assigns role weight...	\N	5.0.1	\N	\N	8952677820
longevity-analysis-1.56-003	intelligence-operative	db-changelog-1.56.xml	2026-01-21 00:44:39.428724	499	EXECUTED	9:2cca784ebb22c73f83c9efe82fa05b0c	sqlFile path=view_riksdagen_politician_longevity_analysis_v1.56.sql	Create view_riksdagen_politician_longevity_analysis to measure politician career\n        duration and engagement levels. Calculates total career years, election cycles active,\n        activity intensity (votes/assignments per year), and career con...	\N	5.0.1	\N	\N	8952677820
1.57-intro	intelligence-operative	db-changelog-1.57.xml	2026-01-21 00:44:39.433563	500	EXECUTED	9:700f30f068f52279dbeffe3e121e51f2	sql	v1.57 Historical Politician Party Transitions - Track MPs leaving their party while serving in Riksdagen	\N	5.0.1	\N	\N	8952677820
create-party-transition-history-view-1.57-001	intelligence-operative	db-changelog-1.57.xml	2026-01-21 00:44:39.445809	501	EXECUTED	9:dc5cb39faeb206f3971c9bcb6f7650bd	createView viewName=view_riksdagen_party_transition_history	Create view_riksdagen_party_transition_history to track all party changes since 2002.\n        \n        Purpose: Identify politicians who switched parties while serving as active MPs \n        (status = 'Tjänstgörande riksdagsledamot'). This tracks ...	\N	5.0.1	\N	\N	8952677820
create-party-defector-analysis-view-1.57-002	intelligence-operative	db-changelog-1.57.xml	2026-01-21 00:44:39.456984	502	EXECUTED	9:8352cbc3030dcb7716007e1b89d68db7	createView viewName=view_riksdagen_party_defector_analysis	Create view_riksdagen_party_defector_analysis to analyze defector characteristics.\n        \n        Purpose: Analyze behavioral patterns before/after party transitions - attendance, \n        document production, voting patterns. Classify defection...	\N	5.0.1	\N	\N	8952677820
create-party-switcher-outcomes-view-1.57-003	intelligence-operative	db-changelog-1.57.xml	2026-01-21 00:44:39.466933	503	EXECUTED	9:6862c4e3283795138ffa775c0df28ee4	createView viewName=view_riksdagen_party_switcher_outcomes	Create view_riksdagen_party_switcher_outcomes to measure post-transition career success.\n        \n        Purpose: Track political career trajectory after party switch - did they continue \n        serving, get re-elected, attain leadership positio...	\N	5.0.1	\N	\N	8952677820
1.58-intro	intelligence-operative	db-changelog-1.58.xml	2026-01-21 00:44:39.471743	504	EXECUTED	9:482916d6fabfef0c90cdd90dab8f32fb	sql	Database Changelog v1.58 - 10-Level Career Path Classification & Trajectory Analysis\n        \n        Enhancement of #8211 career tracking views with comprehensive 10-level hierarchical\n        role classification, party leader/speaker tracking, t...	\N	5.0.1	\N	\N	8952677820
drop-career-path-10level-1.58-001	intelligence-operative	db-changelog-1.58.xml	2026-01-21 00:44:39.476927	505	EXECUTED	9:6fcf5b8336443661dba4d47d733e5f9c	sql		\N	5.0.1	\N	\N	8952677820
career-path-10level-1.58-001	intelligence-operative	db-changelog-1.58.xml	2026-01-21 00:44:39.499389	506	EXECUTED	9:f877b25c7cfedc83354e30be65bffb9d	createView viewName=view_riksdagen_politician_career_path_10level	Create view_riksdagen_politician_career_path_10level with comprehensive 10-level \n        hierarchical role classification system.\n        \n        Framework: Predictive Intelligence (Framework 4) + Temporal Analysis (Framework 1)\n        \n       ...	\N	5.0.1	\N	\N	8952677820
1.58-validation	intelligence-operative	db-changelog-1.58.xml	2026-01-21 00:44:39.507516	507	EXECUTED	9:16d181fc6d8375fda140c490bd306de4	sql	Validate that the 10-level career path view was created successfully with\n        comprehensive career classification, trajectory analysis, and KPI metrics.	\N	5.0.1	\N	\N	8952677820
1.59-intro	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.512354	508	EXECUTED	9:44d236d76025bc9ab297858107d170df	sql	v1.59 Election Proximity Trend Analysis - Quarterly Q4 Pre-Election Focus	\N	5.0.1	\N	\N	8952677820
create-election-proximity-trends-view-1.59-001	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.528621	509	EXECUTED	9:1d89eb720b7c0272ace71368c3344031	createView viewName=view_riksdagen_election_proximity_trends	Create view_riksdagen_election_proximity_trends to track politician activity \n        by months until election, with Q4 pre-election focus.\n        \n        Purpose: Analyze behavioral changes in the 12 months before Swedish elections \n        (ty...	\N	5.0.1	\N	\N	8952677820
1.59-document-election-proximity-trends-view	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.537027	510	EXECUTED	9:cce77ebd4dd48c385219c3cd9a3baf7e	sql	Add documentation for view_riksdagen_election_proximity_trends	\N	5.0.1	\N	\N	8952677820
create-pre-election-quarterly-activity-view-1.59-002	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.554737	511	EXECUTED	9:e6e770e87bd4189dd4c61160c67042dc	createView viewName=view_riksdagen_pre_election_quarterly_activity	Create view_riksdagen_pre_election_quarterly_activity to aggregate Q4 activity \n        vs baseline across all politicians.\n        \n        Purpose: Aggregate-level view showing Q4 activity patterns in pre-election periods \n        compared to ba...	\N	5.0.1	\N	\N	8952677820
1.59-document-pre-election-quarterly-activity-view	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.564477	512	EXECUTED	9:506b35ff5f379430a321421b17714969	sql	Add documentation for view_riksdagen_pre_election_quarterly_activity	\N	5.0.1	\N	\N	8952677820
create-seasonal-activity-patterns-view-1.59-003	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.577057	513	EXECUTED	9:4849ba7c458720937bb7887e27cb70d5	createView viewName=view_riksdagen_seasonal_activity_patterns	Create view_riksdagen_seasonal_activity_patterns to identify Q1-Q4 behavior \n        shifts across election cycles.\n        \n        Purpose: Comprehensive seasonal pattern analysis identifying behavioral shifts \n        across Q1-Q4 for all elect...	\N	5.0.1	\N	\N	8952677820
1.59-validation	intelligence-operative	db-changelog-1.59.xml	2026-01-21 00:44:39.593316	515	EXECUTED	9:1f152f338cd1176b82be6617ff7c31a1	sql	Validate that all 3 election proximity views were created successfully\n        with comprehensive temporal analysis and seasonal pattern detection.	\N	5.0.1	\N	\N	8952677820
1.60-intro	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.59815	516	EXECUTED	9:31cffed9c4c5843f2649b76414dd3d61	sql	v1.60 Election Year Behavioral Pattern Analysis - Annual Comparison Across 7 Election Cycles	\N	5.0.1	\N	\N	8952677820
create-election-year-behavioral-patterns-view-1.60-001	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.611271	517	EXECUTED	9:685db28ff8044fb7380539706bd9f7d9	createView viewName=view_riksdagen_election_year_behavioral_patterns	Create view_riksdagen_election_year_behavioral_patterns for comparing \n        behavioral metrics across all 7 election years (2002-2026) vs midterm years.\n        \n        Purpose: Systematic comparison of election year behavioral patterns vs \n  ...	\N	5.0.1	\N	\N	8952677820
1.60-document-election-year-behavioral-patterns-view	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.619779	518	EXECUTED	9:103fefb7a2d9d4ce2293f95c2df0b993	sql	Add documentation for view_riksdagen_election_year_behavioral_patterns	\N	5.0.1	\N	\N	8952677820
create-election-year-vs-midterm-view-1.60-002	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.63079	519	EXECUTED	9:5d870fadc3c6d12033a166ff6c215dc8	createView viewName=view_riksdagen_election_year_vs_midterm	Create view_riksdagen_election_year_vs_midterm for aggregate comparison \n        of election years vs midterm years.\n        \n        Purpose: Provide high-level comparison showing aggregate behavioral \n        differences between election years a...	\N	5.0.1	\N	\N	8952677820
1.60-document-election-year-vs-midterm-view	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.638887	520	EXECUTED	9:13b795f8584e48425531e9fa4abe0007	sql	Add documentation for view_riksdagen_election_year_vs_midterm	\N	5.0.1	\N	\N	8952677820
create-election-year-anomalies-view-1.60-003	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.649823	521	EXECUTED	9:688d553035c20f1c20070cf95848756e	createView viewName=view_riksdagen_election_year_anomalies	Create view_riksdagen_election_year_anomalies to identify statistically \n        unusual patterns in election years.\n        \n        Purpose: Identify election years with statistically unusual behavioral \n        patterns compared to typical elec...	\N	5.0.1	\N	\N	8952677820
1.60-document-election-year-anomalies-view	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.658124	522	EXECUTED	9:645f4b378ccc02510962eb2749be432c	sql	Add documentation for view_riksdagen_election_year_anomalies	\N	5.0.1	\N	\N	8952677820
1.60-validation	intelligence-operative	db-changelog-1.60.xml	2026-01-21 00:44:39.666048	523	EXECUTED	9:2bf4ec20cf9e261ff6ea0984d62ed157	sql	Validate that all 3 election year analysis views were created successfully\n        with comprehensive annual comparison and anomaly detection capabilities.	\N	5.0.1	\N	\N	8952677820
1414872417007-282	pether (generated)	db-changelog-1.7.xml	2026-01-24 15:25:45.122557	527	EXECUTED	9:0d24e99f24183005670d5e3d4445be97	addPrimaryKey constraintName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0_pkey, tableName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0		\N	5.0.1	\N	\N	9264743943
1414872417007-283	pether (generated)	db-changelog-1.7.xml	2026-01-24 15:25:45.12686	528	EXECUTED	9:3db9a552cdc9cc50f060dd5a467243db	addForeignKeyConstraint baseTableName=USER_ACCOUNT_GOOGLE_AUTH_SCR_0, constraintName=fk_8931ymg13vy6vfkrichdsd4, referencedTableName=user_account		\N	5.0.1	\N	\N	9264743943
1414872417007-284	pether (generated)	db-changelog-1.7.xml	2026-01-24 15:25:45.131086	529	EXECUTED	9:68a1690b5c267ec324fb414df94fef1c	addColumn tableName=user_account; addColumn tableName=user_account		\N	5.0.1	\N	\N	9264743943
1414872417007-285	pether (generated)	db-changelog-1.8.xml	2026-01-24 15:25:45.135165	530	EXECUTED	9:a28ecc2e242187c481c2726325c67bc6	addColumn tableName=language_data		\N	5.0.1	\N	\N	9264743943
1414872417007-286	pether (generated)	db-changelog-1.8.xml	2026-01-24 15:25:45.139197	531	EXECUTED	9:14d37229b20583fab65c4b9ccd61cbf6	addColumn tableName=language_content_data		\N	5.0.1	\N	\N	9264743943
1414872417007-287	quartz.generated	db-changelog-1.9.xml	2026-01-24 15:25:45.145924	532	EXECUTED	9:33c854d56c17f442b136d767015a4026	createTable tableName=QRTZ_CALENDARS; createTable tableName=QRTZ_CRON_TRIGGERS; createTable tableName=QRTZ_FIRED_TRIGGERS; createTable tableName=QRTZ_PAUSED_TRIGGER_GRPS; createTable tableName=QRTZ_SCHEDULER_STATE; createTable tableName=QRTZ_LOCKS...		\N	5.0.1	\N	\N	9264743943
1414872417007-288	quartz.generated	db-changelog-1.9.xml	2026-01-24 15:25:45.150295	533	EXECUTED	9:9e05c11d0073fe498c1b8efa9fc189ac	addForeignKeyConstraint baseTableName=QRTZ_CRON_TRIGGERS, constraintName=FK_QRTZ_CRON_TRIGGERS_QRTZ_TRIGGERS, referencedTableName=QRTZ_TRIGGERS; addForeignKeyConstraint baseTableName=QRTZ_SIMPLE_TRIGGERS, constraintName=FK_QRTZ_SIMPLE_TRIGGERS_QRT...		\N	5.0.1	\N	\N	9264743943
1414872417007-289	quartz.generated	db-changelog-1.9.xml	2026-01-24 15:25:45.155522	534	EXECUTED	9:f018d19e4b6045fcbc2e6672c7ba873d	createIndex indexName=IDX_QRTZ_T_J, tableName=QRTZ_TRIGGERS; createIndex indexName=IDX_QRTZ_T_JG, tableName=QRTZ_TRIGGERS; createIndex indexName=IDX_QRTZ_T_C, tableName=QRTZ_TRIGGERS; createIndex indexName=IDX_QRTZ_T_G, tableName=QRTZ_TRIGGERS; cr...		\N	5.0.1	\N	\N	9264743943
1414872417007-300	createextenions-only-works-if-superuser-so-create-before	db-changelog-1.16.xml	2026-01-24 15:25:45.160375	535	EXECUTED	9:3e937406a4d82aec00fddeafc4c55380	sql; sql; sql		\N	5.0.1	\N	\N	9264743943
1.61-intro	copilot	db-changelog-1.61.xml	2026-01-24 15:25:45.171475	536	EXECUTED	9:a17b84034c30ec69157f773a97d0fe8d	sql	Database Changelog v1.61 - Recreate Party Longitudinal Analysis Views\n        \n        CRITICAL FIX: Recreates 4 views that were dropped in v1.53/v1.6 but never recreated.\n        These views have JPA entities mapped in persistence.xml, causing ap...	\N	5.0.1	\N	\N	9264743943
1.61-drop-party-summary	copilot	db-changelog-1.61.xml	2026-01-24 15:25:45.175857	537	EXECUTED	9:8b553bbf244cc44cf1bc5719856af1ce	sql	Drop view_riksdagen_party_summary if exists before recreation	\N	5.0.1	\N	\N	9264743943
1.61-create-party-summary	copilot	db-changelog-1.61.xml	2026-01-24 15:25:45.179903	538	EXECUTED	9:ea4ae45de8d0256839dde827f5af0be9	createView viewName=view_riksdagen_party_summary	Recreate view_riksdagen_party_summary - Party Assignment and Document Aggregation\n        \n        Original: Dropped in db-changelog-1.6.xml (changeset 1414872417007-278)\n        JPA Entity: ViewRiksdagenPartySummary (party.impl package)\n        P...	\N	5.0.1	\N	\N	9264743943
1.61-comment-party-summary	copilot	db-changelog-1.61.xml	2026-01-24 15:25:45.183631	539	EXECUTED	9:a7e687cfd143f6402e0a3357352fd7d9	sql		\N	5.0.1	\N	\N	9264743943
1.61-drop-party-longitudinal	copilot	db-changelog-1.61.xml	2026-01-24 15:25:45.187391	540	EXECUTED	9:e597cc3a5205328ad149858ab31afde9	sql	Drop view_riksdagen_party_longitudinal_performance if exists	\N	5.0.1	\N	\N	9264743943
...
1.79-022	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.963382	687	EXECUTED	9:ac6e4880b82bbd4d801683b19ff1dd07	sql	Fix view_riksdagen_politician_career_trajectory: was empty (0 rows)\n        because LEFT JOINs to assignment_data and document_person_reference_da_0\n        without time constraints created massive fan-out with vote_data (3.7M rows).\n        Fix: ...	\N	5.0.2	\N	\N	4213844732
...
1.62-create-electoral-trends	stack-specialist	db-changelog-1.62.xml	2026-01-24 15:25:45.215444	546	EXECUTED	9:33118cbd088237b6ccead322126a57d2	createView viewName=view_riksdagen_party_electoral_trends	Recreate view_riksdagen_party_electoral_trends (originally from v1.53, recreated in v1.61)\n        \n        JPA Entity: ViewRiksdagenPartyElectoralTrends\n        Primary Key: Composite (party, election_cycle_id, semester)\n        Columns: 49 total...	\N	5.0.1	\N	\N	9264743943
1.62-create-intelligence-dashboard	stack-specialist	db-changelog-1.62.xml	2026-01-24 15:25:45.219449	547	EXECUTED	9:c2671630b8cd08ff9548f33aa462e612	sql	Recreate view_riksdagen_intelligence_dashboard (originally from v1.29)\n        \n        This view aggregates multiple intelligence indicators into a single dashboard.\n        It was recreated in v1.40 after being dropped by CASCADE but may be miss...	\N	5.0.1	\N	\N	9264743943
1.63-intro	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.223303	548	EXECUTED	9:bb38d76f267d4111557bc2958aa8d751	sql	Database Changelog v1.63 - Priority 1 Foreign Key Indexes (28 Critical Indexes)\n        \n        CRITICAL PERFORMANCE FIX: Creates 28 missing foreign key indexes that are the\n        #1 performance bottleneck causing 5-300 second query times.\n    ...	\N	5.0.1	\N	\N	9264743943
1.63-001-idx-person-assignment-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.227004	549	EXECUTED	9:7fe5ab674ab49647ed90faca8f3af3e1	createIndex indexName=idx_person_data_assignment_fk, tableName=person_data	Create index on person_data.person_assignment_data_perso_0 foreign key column.\n        Improves JOIN performance with person_assignment_data table.	\N	5.0.1	\N	\N	9264743943
1.63-002-idx-person-detail-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.230852	550	EXECUTED	9:75a3fb2d2d04f40ad18e99014fb6426c	createIndex indexName=idx_person_data_detail_fk, tableName=person_data	Create index on person_data.person_detail_data_person_da_0 foreign key column.\n        Improves JOIN performance with person_detail_data table.	\N	5.0.1	\N	\N	9264743943
1.63-003-idx-doc-status-document-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.234989	551	EXECUTED	9:7cfbe9dce8d113bb35d7f2d6022df89d	createIndex indexName=idx_doc_status_document_fk, tableName=document_status_container	Create index on document_status_container.document_document_status_con_0 foreign key.\n        Improves JOIN performance with document_data table.	\N	5.0.1	\N	\N	9264743943
1.63-004-idx-doc-status-activity-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.239211	552	EXECUTED	9:50001f6c31aa94ef351be299b3990241	createIndex indexName=idx_doc_status_activity_fk, tableName=document_status_container	Create index on document_status_container.document_activity_container__0 foreign key.\n        Improves JOIN performance with document_activity_container table.	\N	5.0.1	\N	\N	9264743943
1.63-005-idx-doc-status-attachment-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.243012	553	EXECUTED	9:af96d3a42c8328acb8064438c5ca42b9	createIndex indexName=idx_doc_status_attachment_fk, tableName=document_status_container	Create index on document_status_container.document_attachment_containe_0 foreign key.\n        Improves JOIN performance with document_attachment_container table.	\N	5.0.1	\N	\N	9264743943
1.63-006-idx-doc-status-detail-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.246748	554	EXECUTED	9:bda3dbf6989d85130b1a1569bc54c868	createIndex indexName=idx_doc_status_detail_fk, tableName=document_status_container	Create index on document_status_container.document_detail_container_do_0 foreign key.\n        Improves JOIN performance with document_detail_container table.	\N	5.0.1	\N	\N	9264743943
1.63-007-idx-doc-status-person-ref-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.250543	555	EXECUTED	9:86d012b093b2fdb62152778eeed45944	createIndex indexName=idx_doc_status_person_ref_fk, tableName=document_status_container	Create index on document_status_container.document_person_reference_co_1 foreign key.\n        Improves JOIN performance with document_person_reference table.	\N	5.0.1	\N	\N	9264743943
1.63-008-idx-doc-status-proposal-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.254201	556	EXECUTED	9:0b2c38842f380b2e356e3aa5e26f149b	createIndex indexName=idx_doc_status_proposal_fk, tableName=document_status_container	Create index on document_status_container.document_proposal_document_s_0 foreign key.\n        Improves JOIN performance with document_proposal_data table.	\N	5.0.1	\N	\N	9264743943
1.63-009-idx-doc-status-reference-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.257867	557	EXECUTED	9:e415ed11432c3259118fd5ca722e96b7	createIndex indexName=idx_doc_status_reference_fk, tableName=document_status_container	Create index on document_status_container.document_reference_container_0 foreign key.\n        Improves JOIN performance with document_reference_container table.	\N	5.0.1	\N	\N	9264743943
1.63-010-idx-person-container-person-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.261585	558	EXECUTED	9:21fdad232a2a9af9ab412fda11206955	createIndex indexName=idx_person_container_person_fk, tableName=person_container_data	Create index on person_container_data.person_person_container_data_0 foreign key.\n        Improves JOIN performance with person_data table.	\N	5.0.1	\N	\N	9264743943
1.63-011-idx-committee-proposal-against-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.265402	559	EXECUTED	9:5eed4a6c6b958c3c8e029f104084fd01	createIndex indexName=idx_committee_proposal_against_fk, tableName=committee_proposal_component_0	Create index on committee_proposal_component_0.against_proposal_container_c_0 foreign key.\n        Improves JOIN performance with proposal_container table.	\N	5.0.1	\N	\N	9264743943
1.63-012-idx-committee-proposal-container-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.269261	560	EXECUTED	9:37e0994170916328db782da4d24da9e2	createIndex indexName=idx_committee_proposal_container_fk, tableName=committee_proposal_component_0	Create index on committee_proposal_component_0.committee_proposal_container_0 foreign key.\n        Improves JOIN performance with committee_proposal_container table.	\N	5.0.1	\N	\N	9264743943
1.63-013-idx-committee-proposal-document-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.272946	561	EXECUTED	9:cc9309a385247f047986241a76629f5c	createIndex indexName=idx_committee_proposal_document_fk, tableName=committee_proposal_component_0	Create index on committee_proposal_component_0.document_committee_proposal__0 foreign key.\n        Improves JOIN performance with document table.	\N	5.0.1	\N	\N	9264743943
1.63-014-idx-committee-proposal-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.276817	562	EXECUTED	9:b34d48c72c7e16c86ba3810ec8f9d064	createIndex indexName=idx_committee_proposal_list_fk, tableName=committee_proposal_data	Create index on committee_proposal_data.committee_proposal_list_comm_0 foreign key.\n        Improves JOIN performance with committee_proposal_list table.	\N	5.0.1	\N	\N	9264743943
1.63-015-idx-assignment-data-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.28047	563	EXECUTED	9:4799c76c8f955a1e7c2283898c3b6436	createIndex indexName=idx_assignment_data_list_fk, tableName=assignment_data	Create index on assignment_data.assignment_list_person_assig_0 foreign key.\n        Improves JOIN performance with assignment_list table.	\N	5.0.1	\N	\N	9264743943
1.63-016-idx-assignment-element-uppdrag-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.284234	564	EXECUTED	9:3a3620cfb87f61b5328b72e15188ed16	createIndex indexName=idx_assignment_element_uppdrag_fk, tableName=assignment_element	Create index on assignment_element.uppdrag_person_assignment_el_0 foreign key.\n        Improves JOIN performance with uppdrag table.	\N	5.0.1	\N	\N	9264743943
1.63-017-idx-doc-activity-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.287989	565	EXECUTED	9:9085a65de2ec859d6230d13758c413bd	createIndex indexName=idx_doc_activity_list_fk, tableName=document_activity_data	Create index on document_activity_data.document_activities_document_0 foreign key.\n        Improves JOIN performance with document_activities table.	\N	5.0.1	\N	\N	9264743943
1.63-018-idx-doc-attachment-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.294373	566	EXECUTED	9:65b6577136fc8a89eb97cd21f75e9436	createIndex indexName=idx_doc_attachment_list_fk, tableName=document_attachment	Create index on document_attachment.document_attachment_list_doc_0 foreign key.\n        Improves JOIN performance with document_attachment_list table.	\N	5.0.1	\N	\N	9264743943
1.63-019-idx-doc-detail-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.298117	567	EXECUTED	9:2228eb5055459659ac51a010cc73ffc3	createIndex indexName=idx_doc_detail_list_fk, tableName=document_detail_data	Create index on document_detail_data.document_detail_list_documen_0 foreign key.\n        Improves JOIN performance with document_detail_list table.	\N	5.0.1	\N	\N	9264743943
1.63-020-idx-doc-element-container-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.301873	568	EXECUTED	9:d70904047735c612eb8e8d9d8d5ea14c	createIndex indexName=idx_doc_element_container_fk, tableName=document_element	Create index on document_element.dokument_document_container__0 foreign key.\n        Improves JOIN performance with document_container table.	\N	5.0.1	\N	\N	9264743943
1.63-021-idx-doc-person-ref-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.305679	569	EXECUTED	9:c16ebdad0101145e692972a071c6ce21	createIndex indexName=idx_doc_person_ref_list_fk, tableName=document_person_reference_da_0	Create index on document_person_reference_da_0.document_person_reference_li_1 foreign key.\n        Improves JOIN performance with document_person_reference_list table.	\N	5.0.1	\N	\N	9264743943
1.63-022-idx-doc-proposal-container-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.309489	570	EXECUTED	9:8bea548867ca7f3a99619b4816770388	createIndex indexName=idx_doc_proposal_container_fk, tableName=document_proposal_container	Create index on document_proposal_container.proposal_document_proposal_c_0 foreign key.\n        Improves JOIN performance with proposal table.	\N	5.0.1	\N	\N	9264743943
1.63-023-idx-doc-reference-list-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.313275	571	EXECUTED	9:afa6bf1e305502f2c72b3dcdda7e56f6	createIndex indexName=idx_doc_reference_list_fk, tableName=document_reference_data	Create index on document_reference_data.document_reference_list_docu_0 foreign key.\n        Improves JOIN performance with document_reference_list table.	\N	5.0.1	\N	\N	9264743943
1.63-024-idx-person-detail-element-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.31782	572	EXECUTED	9:8f3e3de912d8b2f7698329ca6cc2931f	createIndex indexName=idx_person_detail_element_fk, tableName=person_detail_element	Create index on person_detail_element.detail_list_person_detail_el_0 foreign key.\n        Improves JOIN performance with detail_list table.	\N	5.0.1	\N	\N	9264743943
1.63-025-idx-person-element-assignment-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.321496	573	EXECUTED	9:e2860208231f1ac1bc8a1a1c8044f304	createIndex indexName=idx_person_element_assignment_fk, tableName=person_element	Create index on person_element.person_assignment_element_pe_0 foreign key.\n        Improves JOIN performance with person_assignment_element table.	\N	5.0.1	\N	\N	9264743943
1.63-026-idx-person-element-detail-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.325287	574	EXECUTED	9:329818f46d70cb091ff761e8d9f3c80f	createIndex indexName=idx_person_element_detail_fk, tableName=person_element	Create index on person_element.person_detail_element_person_0 foreign key.\n        Improves JOIN performance with person_detail_element table.	\N	5.0.1	\N	\N	9264743943
1.63-027-idx-person-element-container-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.329136	575	EXECUTED	9:f68ece91112d1dbed7a0c9226f6d9a7b	createIndex indexName=idx_person_element_container_fk, tableName=person_element	Create index on person_element.person_person_container_elem_0 foreign key.\n        Improves JOIN performance with person_container table.	\N	5.0.1	\N	\N	9264743943
1.63-028-idx-sweden-party-region-fk	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.3328	576	EXECUTED	9:576071dc9e00a9013ecabcd89adff9cd	createIndex indexName=idx_sweden_party_region_fk, tableName=sweden_political_party	Create index on sweden_political_party.parties_sweden_election_regi_0 foreign key.\n        Improves JOIN performance with sweden_election_region table.	\N	5.0.1	\N	\N	9264743943
1.63-completion	performance-engineer	db-changelog-1.63.xml	2026-01-24 15:25:45.341324	578	EXECUTED	9:094384036a2254c066d83dec05d62db1	sql	Completion changeset: Updates table statistics after index creation.\n        \n        Running ANALYZE ensures PostgreSQL's query planner is aware of the new indexes\n        and can use them effectively in query plans. This is critical for achievin...	\N	5.0.1	\N	\N	9264743943
1.64-intro	performance-engineer	db-changelog-1.64.xml	2026-01-24 15:25:45.345125	579	EXECUTED	9:8d2e9209b45203a7f6c9094a87d6c97e	sql	Database Changelog v1.64 - Temporal Analysis Performance Indexes (4 Indexes)\n        \n        PERFORMANCE OPTIMIZATION: Creates 4 missing composite temporal indexes for\n        vote_data table to optimize 35 temporal analysis views.\n        \n     ...	\N	5.0.1	\N	\N	9264743943
1.64-001-idx-vote-data-ballot-date	performance-engineer	db-changelog-1.64.xml	2026-01-24 15:25:45.348827	580	EXECUTED	9:6612decce858a250e4a7e5c40f574aa6	sql	Create composite index on vote_data (ballot_id, vote_date) for temporal ballot queries.\n        \n        Benefits:\n        - Optimizes ballot summary views with date filtering\n        - Supports ORDER BY vote_date with ballot_id filtering\n        ...	\N	5.0.1	\N	\N	9264743943
1.64-002-idx-vote-data-party-date	performance-engineer	db-changelog-1.64.xml	2026-01-24 15:25:45.352415	581	EXECUTED	9:7d6a0863d3b74a640059e9a7c459c928	sql	Create composite index on vote_data (party, vote_date) for temporal party analysis.\n        \n        Benefits:\n        - Optimizes party summary views with temporal filtering\n        - Supports party voting trend analysis over time\n        - Enabl...	\N	5.0.1	\N	\N	9264743943
1.64-003-idx-vote-data-politician-date	performance-engineer	db-changelog-1.64.xml	2026-01-24 15:25:45.356157	582	EXECUTED	9:83ea74ca9d80863cc4b3ddbcfb1ed595	sql	Create composite index on vote_data (intressent_id, vote_date) for temporal politician analysis.\n        \n        Benefits:\n        - Optimizes politician voting history queries with date filters\n        - Supports politician performance tracking ...	\N	5.0.1	\N	\N	9264743943
1.64-004-idx-vote-data-aggregation-cover	performance-engineer	db-changelog-1.64.xml	2026-01-24 15:25:45.36001	583	EXECUTED	9:2e91efbdf2fa156952513ba4d3c5e0e8	sql	Create covering index on vote_data for common temporal aggregation queries.\n        \n        This is a covering index that includes frequently accessed columns to enable\n        index-only scans without touching the table heap.\n        \n        Be...	\N	5.0.1	\N	\N	9264743943
1.64-999-verification	performance-engineer	db-changelog-1.64.xml	2026-01-24 15:25:45.364273	584	EXECUTED	9:e25c3ff872703444d0ed2a6369db90e1	sql	Verification queries for temporal analysis performance indexes.\n        \n        This changeset documents verification steps but doesn't execute them.\n        Run these queries manually to verify index effectiveness:\n        \n        1. Verify all...	\N	5.0.1	\N	\N	9264743943
1.65-intro	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.368026	585	EXECUTED	9:6d532b767c92e9276bfaca26ea56eda7	sql	Database Changelog v1.65 - Pattern Recognition Performance Optimization\n        \n        PERFORMANCE OPTIMIZATION: Creates 5 missing critical indexes and resolves\n        5 high-impact performance bottlenecks in Pattern Recognition framework.\n    ...	\N	5.0.1	\N	\N	9264743943
1.65-001-idx-vote-network-ballot-person	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.371532	586	EXECUTED	9:2ea801ef4ac636076adf3dfecf6f6e7d	sql	Create composite index for network analysis self-join optimization.\n        \n        Critical for view_riksdagen_politician_influence_metrics which performs\n        O(n²) self-join on vote_data table. With 3.5M votes and 350 politicians,\n        t...	\N	5.0.1	\N	\N	9264743943
1.65-002-idx-document-made-public-date	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.375398	587	EXECUTED	9:1b8ebf1019e8234b54750764c7e0f627	sql	Create index on document_data.made_public_date for temporal trend queries.\n        \n        Critical for view_decision_temporal_trends which uses 8 window functions\n        with overlapping frames (7/30/90-day moving averages) over 5 years of data...	\N	5.0.1	\N	\N	9264743943
1.65-003-idx-vote-person-party-date	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.379378	588	EXECUTED	9:a66cb44481293612de06993a468aac4f	sql	Create composite index for politician behavioral pattern analysis.\n        \n        Critical for view_politician_behavioral_trends which uses 7 window functions\n        with LAG and 3-month moving averages. Performs 90,000 window computations\n    ...	\N	5.0.1	\N	\N	9264743943
1.65-004-idx-violation-party-date	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.383394	589	EXECUTED	9:6c6055c33e22154325cefa422ba3ac2f	sql	Create composite index for party effectiveness trend analysis.\n        \n        Critical for view_party_effectiveness_trends which performs multi-source\n        aggregation (voting + documents + violations) with complex JOINs.\n        Query time: ...	\N	5.0.1	\N	\N	9264743943
1.65-005-idx-assignment-ministry-person-dates	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.387636	590	EXECUTED	9:3fe60c277812f1566b526c8fe2a71f23	sql	Create composite index for ministry assignment lookups.\n        \n        Optimizes queries that join assignment_data for ministry-related analysis.\n        Used in multiple pattern recognition views that track politician roles\n        and ministry...	\N	5.0.1	\N	\N	9264743943
1.65-010-fix-election-cycle-cartesian-join	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.391437	591	EXECUTED	9:583f964341e5f4eac044bbb6ef6b5ca3	sql	Fix CRITICAL Cartesian join in view_election_cycle_anomaly_pattern.\n        \n        Issue: Two LEFT JOINs use ON (1 = 1) causing Cartesian product explosion.\n        - LEFT JOIN view_riksdagen_voting_anomaly_detection vad ON (1 = 1)\n        - LEF...	\N	5.0.1	\N	\N	9264743943
1.65-999-verification	performance-engineer	db-changelog-1.65.xml	2026-01-24 15:25:45.39523	592	EXECUTED	9:393b4f2ca2b02b96fa6e5b33050e477b	sql	Verification queries for pattern recognition performance optimization.\n        \n        This changeset documents verification steps but doesn't execute them.\n        Run these queries manually to verify optimization effectiveness:\n        \n       ...	\N	5.0.1	\N	\N	9264743943
1.66-intro	performance-engineer	db-changelog-1.66.xml	2026-01-24 15:25:45.399049	593	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.66 - Network Analysis Framework Performance Indexes\n        \n        Creates 4 performance indexes for Network Analysis framework to support\n        future data growth and enable testing with sample data.\n        \n        Cur...	\N	5.0.1	\N	\N	9264743943
1.66-idx-vote-data-covoting	performance-engineer	db-changelog-1.66.xml	2026-01-24 15:25:45.402985	594	EXECUTED	9:3949f1f6078cc3a82fd22aec24be3064	createIndex indexName=idx_vote_data_covoting, tableName=vote_data	Composite index for co-voting analysis in view_riksdagen_politician_influence_metrics.\n        \n        Supports: Politician influence network analysis, co-voting pair detection\n        Expected Impact: 10-20% improvement with larger datasets\n    ...	\N	5.0.1	\N	\N	9264743943
1.68-intro	stack-specialist	db-changelog-1.68.xml	2026-01-24 15:25:45.470309	611	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.68 - Fix Unpopulated Materialized Views + Create Ministry View\n        \n        Creates missing view_riksdagen_ministry to achieve 100% Comparative Analysis\n        framework coverage (26/26 views). Documents population requi...	\N	5.0.1	\N	\N	9264743943
1.66-idx-vote-data-coalition	performance-engineer	db-changelog-1.66.xml	2026-01-24 15:25:45.406818	595	EXECUTED	9:4b90c22caa8a692d2593f86954d48000	createIndex indexName=idx_vote_data_coalition, tableName=vote_data	Composite index for coalition matrix in view_riksdagen_coalition_alignment_matrix.\n        \n        Supports: Party coalition alignment analysis, voting pattern heatmaps\n        Expected Impact: 5-10% improvement with larger datasets\n        \n    ...	\N	5.0.1	\N	\N	9264743943
1.66-idx-politician-doc-summary	performance-engineer	db-changelog-1.66.xml	2026-01-24 15:25:45.410601	596	EXECUTED	9:17e7b555073d436f63a535fa450c7d1c	createIndex indexName=idx_politician_doc_summary_person_id, tableName=view_riksdagen_politician_document_summary	Index on materialized view for party member queries (after population).\n        \n        Supports: view_riksdagen_party_member queries\n        Expected Impact: 50-80% improvement in party member queries\n        \n        Prerequisite: Materialized ...	\N	5.0.1	\N	\N	9264743943
1.66-idx-ballot-party-summary	performance-engineer	db-changelog-1.66.xml	2026-01-24 15:25:45.414451	597	EXECUTED	9:839f70ea04095387d3d05244a7ff5a8b	createIndex indexName=idx_ballot_party_summary_party, tableName=view_riksdagen_vote_data_ballot_party_summary	Index on materialized view for party support queries (after population).\n        \n        Supports: Coalition evolution analysis\n        Expected Impact: 50-80% improvement in party support queries\n        \n        Prerequisite: Materialized view ...	\N	5.0.1	\N	\N	9264743943
1.66-summary	performance-engineer	db-changelog-1.66.xml	2026-01-24 15:25:45.418567	598	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.66 - Network Analysis Performance Summary\n        \n        Indexes Created: 2 on vote_data table (available for testing)\n        Indexes Awaiting: 2 on materialized views (requires population via Issue #8282)\n        \n       ...	\N	5.0.1	\N	\N	9264743943
1.67-intro	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.422579	599	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.67 - Predictive Intelligence Framework Performance Indexes\n        \n        Creates 10 performance indexes for Predictive Intelligence framework to optimize\n        14 views supporting electoral forecasting and trend predicti...	\N	5.0.1	\N	\N	9264743943
1.67-idx-vote-data-temporal	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.426582	600	EXECUTED	9:9504a89e620f692626f89385718e10b8	createIndex indexName=idx_vote_data_temporal, tableName=vote_data	Temporal composite index for electoral forecasting and voting trend analysis.\n        \n        Supports: Electoral forecasting views (5 views), voting trend predictions\n        Query Optimization: Time-series queries with date ranges and person fi...	\N	5.0.1	\N	\N	9264743943
1.67-idx-vote-data-party-temporal	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.430464	601	EXECUTED	9:82164cec6edea4f33ae949c2e102cdfc	createIndex indexName=idx_vote_data_party_temporal, tableName=vote_data	Party-centric temporal index for trend prediction and coalition evolution analysis.\n        \n        Supports: Trend prediction views (4 views), party coalition evolution\n        Query Optimization: Party-level aggregations over time\n        Expec...	\N	5.0.1	\N	\N	9264743943
1.67-idx-assignment-data-temporal	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.43461	602	EXECUTED	9:df7ce63ae391d22160ff19c8592b2210	createIndex indexName=idx_assignment_data_temporal, tableName=assignment_data	Temporal index for role evolution tracking and influence prediction.\n        \n        Supports: Influence prediction views (2 views), role evolution analysis\n        Query Optimization: Time-series role assignments and career progression\n        E...	\N	5.0.1	\N	\N	9264743943
1.67-idx-assignment-data-person-roles	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.438456	603	EXECUTED	9:06b0bd1bacd205b4960810a802a3061f	createIndex indexName=idx_assignment_data_person_roles, tableName=assignment_data	Person-centric role index for career trajectory and behavioral prediction.\n        \n        Supports: Behavioral prediction views, career analysis\n        Query Optimization: Per-person role filtering and aggregations\n        Expected Impact: 30%+...	\N	5.0.1	\N	\N	9264743943
1.67-idx-document-data-temporal	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.442729	604	EXECUTED	9:41f74267310fd96f2ba2683bbbbb79b4	createIndex indexName=idx_document_data_temporal, tableName=document_data	Temporal index for document activity trends and scenario analysis.\n        \n        Supports: Scenario analysis views (3 views), policy impact forecasting\n        Query Optimization: Time-series document activity and policy trends\n        Expected...	\N	5.0.1	\N	\N	9264743943
1.67-idx-person-data-party	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.446692	605	EXECUTED	9:18c403e28c18b8fbb8ca23331d9d452a	createIndex indexName=idx_person_data_party, tableName=person_data	Party membership index for coalition scenario analysis.\n        \n        Supports: Coalition scenario analysis, party member queries\n        Query Optimization: Party-based filtering and member counts\n        Expected Impact: 20%+ improvement in c...	\N	5.0.1	\N	\N	9264743943
1.67-idx-vote-data-window-partition	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.450456	606	EXECUTED	9:e654959908dcbc003777003d6bab9a75	createIndex indexName=idx_vote_data_window_partition, tableName=vote_data	Window function optimization index for PARTITION BY operations.\n        \n        Supports: All predictive views using window functions (14 views)\n        Query Optimization: RANK, LAG, LEAD, NTILE, moving averages\n        Expected Impact: 30-50% i...	\N	5.0.1	\N	\N	9264743943
1.67-idx-assignment-window-partition	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.454323	607	EXECUTED	9:5e0077040d30bb453f573569407a60d7	createIndex indexName=idx_assignment_window_partition, tableName=assignment_data	Window function optimization for career trajectory analysis.\n        \n        Supports: Career trajectory and role evolution views\n        Query Optimization: LAG/LEAD operations on role transitions\n        Expected Impact: 25%+ improvement in car...	\N	5.0.1	\N	\N	9264743943
1.67-idx-document-data-org-temporal	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.458085	608	EXECUTED	9:9fd1ea821376931b5f6869c3f0569561	createIndex indexName=idx_document_data_org_temporal_full, tableName=document_data	Organization-temporal index for ministry effectiveness trend analysis.\n        \n        Supports: Ministry effectiveness views, organizational trend analysis\n        Query Optimization: Per-ministry temporal aggregations\n        Expected Impact: 4...	\N	5.0.1	\N	\N	9264743943
1.67-idx-vote-data-ballot-temporal	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.461871	609	EXECUTED	9:500d24eaa3dcd86ce1ff79d9913d88f2	createIndex indexName=idx_vote_data_ballot_temporal, tableName=vote_data	Ballot-centric temporal index for detailed electoral forecasting.\n        \n        Supports: Ballot-level analysis and detailed voting patterns\n        Query Optimization: Per-ballot temporal analysis\n        Expected Impact: 35%+ improvement in b...	\N	5.0.1	\N	\N	9264743943
1.67-summary	performance-engineer	db-changelog-1.67.xml	2026-01-24 15:25:45.466223	610	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.67 - Predictive Intelligence Performance Summary\n        \n        Indexes Created: 10 indexes across 4 core tables\n        - vote_data: 5 indexes (temporal, party, window, ballot analysis)\n        - assignment_data: 3 indexes...	\N	5.0.1	\N	\N	9264743943
1.68-001-create-ministry-view	stack-specialist	db-changelog-1.68.xml	2026-01-24 15:25:45.474371	612	EXECUTED	9:0b5f4e2d37c2855656302ed2b3b69dbb	createView viewName=view_riksdagen_ministry	Create view_riksdagen_ministry for ministry-level comparative analysis.\n        \n        Purpose: Provides aggregated ministry data for comparative analysis framework\n        Pattern: Based on view_riksdagen_party structure (aggregation with stati...	\N	5.0.1	\N	\N	9264743943
1.68-002-document-materialized-view-population	stack-specialist	db-changelog-1.68.xml	2026-01-24 15:25:45.478313	613	EXECUTED	9:29b7952f3dbe29b1b6a3b60a14405ce2	sql	Document materialized view population requirements.\n        \n        This changeset serves as documentation for the operational task of populating\n        materialized views. The actual REFRESH commands must be executed outside of\n        Liquibas...	\N	5.0.1	\N	\N	9264743943
1.68-summary	stack-specialist	db-changelog-1.68.xml	2026-01-24 15:25:45.482238	614	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.68 - Summary\n        \n        Views Created: 1 new view\n        - view_riksdagen_ministry: Ministry aggregation view\n        \n        Materialized Views to Populate (operational task):\n        - view_riksdagen_committee_decis...	\N	5.0.1	\N	\N	9264743943
1.69-intro	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.48622	615	EXECUTED	9:d41d8cd98f00b204e9800998ecf8427e	empty	Database Changelog v1.69 - Convert 5 High-Impact Views to Materialized Views\n        \n        Converts 5 high-impact views to materialized views for 60-80% performance\n        improvement. Each view is converted from regular VIEW to MATERIALIZED V...	\N	5.0.1	\N	\N	9264743943
1.69-001-materialize-decision_temporal_trends	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.490406	616	EXECUTED	9:b4e81062db49447c17d9d46727178972	sql	Convert view_decision_temporal_trends to MATERIALIZED VIEW for 40% performance improvement.\n        \n        Issue: 8 window functions with overlapping frames (7/30/90-day moving averages)\n        Current: 3 seconds\n        Target: 1.8 seconds\n   ...	\N	5.0.1	\N	\N	9264743943
1.69-001-01-index-decision_temporal_day	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.494231	617	EXECUTED	9:c10a9c9d82235f6c417de9c0692f1338	createIndex indexName=idx_decision_temporal_day, tableName=view_decision_temporal_trends	Create index idx_decision_temporal_day on view_decision_temporal_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-001-02-index-decision_temporal_quarter	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.499117	618	EXECUTED	9:347841623e106e4b02c32739d64d69ca	createIndex indexName=idx_decision_temporal_quarter, tableName=view_decision_temporal_trends	Create index idx_decision_temporal_quarter on view_decision_temporal_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-002-materialize-riksdagen_politician_influence_metrics	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.503266	619	EXECUTED	9:338fd8f6c830e98f2feb99f1bad499c0	sql	Convert view_riksdagen_politician_influence_metrics to MATERIALIZED VIEW for 80% performance improvement.\n        \n        Issue: Network analysis with self-join on vote_data (O(n²) complexity)\n        Current: 5-15 seconds\n        Target: 1-3 sec...	\N	5.0.1	\N	\N	9264743943
1.69-002-01-index-influence_person	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.50725	620	EXECUTED	9:8b7439d7063cdad422458e2180c385af	createIndex indexName=idx_influence_person, tableName=view_riksdagen_politician_influence_metrics	Create index idx_influence_person on view_riksdagen_politician_influence_metrics for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-002-02-index-influence_score	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.511317	621	EXECUTED	9:1b483bcaff2740af26ea459fa8ac8638	createIndex indexName=idx_influence_score, tableName=view_riksdagen_politician_influence_metrics	Create index idx_influence_score on view_riksdagen_politician_influence_metrics for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-003-materialize-party_effectiveness_trends	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.51543	622	EXECUTED	9:16f5143b4a3cb77e458cebae563a7af6	sql	Convert view_party_effectiveness_trends to MATERIALIZED VIEW for 50% performance improvement.\n        \n        Issue: Multi-source aggregation (voting + documents + violations) with 5 windows\n        Current: 800ms-1.5s\n        Target: <800ms\n    ...	\N	5.0.1	\N	\N	9264743943
1.69-003-01-index-effectiveness_party_period	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.519476	623	EXECUTED	9:203d0084c65c844bc16b20e649cef5df	createIndex indexName=idx_effectiveness_party_period, tableName=view_party_effectiveness_trends	Create index idx_effectiveness_party_period on view_party_effectiveness_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-003-02-index-effectiveness_score	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.523429	624	EXECUTED	9:38f342b8d2f631829604386260db0925	createIndex indexName=idx_effectiveness_score, tableName=view_party_effectiveness_trends	Create index idx_effectiveness_score on view_party_effectiveness_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-004-materialize-politician_behavioral_trends	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.530243	625	EXECUTED	9:4c40067a44abb66c46a17ea068bec690	sql	Convert view_politician_behavioral_trends to MATERIALIZED VIEW for 35% performance improvement.\n        \n        Issue: 7 window functions with LAG and 3-month moving averages\n        Current: 4 seconds\n        Target: 2.6 seconds\n        Refresh:...	\N	5.0.1	\N	\N	9264743943
1.69-004-01-index-behavioral_person_period	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.534439	626	EXECUTED	9:0c2ec965339ddcbda65bc7816a310c37	createIndex indexName=idx_behavioral_person_period, tableName=view_politician_behavioral_trends	Create index idx_behavioral_person_period on view_politician_behavioral_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-004-02-index-behavioral_trend_score	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.53846	627	EXECUTED	9:85ad83600d86027e76edc5bfdef119ed	createIndex indexName=idx_behavioral_trend_score, tableName=view_politician_behavioral_trends	Create index idx_behavioral_trend_score on view_politician_behavioral_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-005-materialize-election_cycle_temporal_trends	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.542658	628	EXECUTED	9:b1ca003e63f34d50d8560276d716a849	sql	Convert view_election_cycle_temporal_trends to MATERIALIZED VIEW for 70% performance improvement.\n        \n        Issue: 6 CTEs with 5 JOINs, complex temporal aggregation\n        Current: 5-10 seconds\n        Target: <3 seconds\n        Refresh: P...	\N	5.0.1	\N	\N	9264743943
1.69-005-01-index-election_temporal_cycle	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.546634	629	EXECUTED	9:d8ddbe5cbdcddee1da41935c0b1796a3	createIndex indexName=idx_election_temporal_cycle, tableName=view_election_cycle_temporal_trends	Create index idx_election_temporal_cycle on view_election_cycle_temporal_trends for query optimization	\N	5.0.1	\N	\N	9264743943
1.69-005-02-index-election_temporal_year	performance-engineer	db-changelog-1.69.xml	2026-01-24 15:25:45.55051	630	EXECUTED	9:adaeba72188a01a96b9c71b379cd1d41	createIndex indexName=idx_election_temporal_year, tableName=view_election_cycle_temporal_trends	Create index idx_election_temporal_year on view_election_cycle_temporal_trends for query optimization	\N	5.0.1	\N	\N	9264743943
create-mv-annual-voting-metrics-1.70-001	intelligence-operative	db-changelog-1.70.xml	2026-02-09 02:52:09.563388	631	EXECUTED	9:1c6ba82d5cc87c9ecfb5004371d8fdfb	sql	Create materialized view for annual voting metrics (pre-aggregated from vote_data).\n            \n            Purpose: Eliminate cartesian product by aggregating vote_data by year FIRST,\n            before joining with document metrics.\n           ...	\N	5.0.1	\N	\N	0601926302
create-mv-annual-document-metrics-1.70-002	intelligence-operative	db-changelog-1.70.xml	2026-02-09 02:52:09.644648	632	EXECUTED	9:018b8d311034d1c40019c0d9e5926635	sql	Create materialized view for annual document metrics (pre-aggregated from document_data).\n            \n            Purpose: Separate document aggregation from voting aggregation to eliminate \n            cartesian product when joining on year.\n   ...	\N	5.0.1	\N	\N	0601926302
recreate-view-riksdagen-election-year-behavioral-patterns-1.70-003	intelligence-operative	db-changelog-1.70.xml	2026-02-09 02:52:09.669701	633	EXECUTED	9:b4fab1ed989ee2d0f474c0a649e01650	sql	Recreate view_riksdagen_election_year_behavioral_patterns using materialized views.\n            \n            Problem: Original view had cartesian product:\n            - vote_data (3.5M rows) LEFT JOIN document_data (109K rows) ON year\n            ...	\N	5.0.1	\N	\N	0601926302
fix-view-election-cycle-network-analysis-timeout-1.71-001	intelligence-operative	db-changelog-1.71.xml	2026-02-09 02:52:09.688648	634	EXECUTED	9:725e4aa85084af5ae6fd68b29b31e0b5	sql	Fix view_election_cycle_network_analysis timeout by correcting JOIN logic.\n            \n            Problem: LEFT JOIN pim ON (1 = 1) is always-true, creating cartesian product\n            where every cycle/coalition combination joins with EVERY p...	\N	5.0.1	\N	\N	0601926302
insert-minimal-test-data-party-transitions-1.72-001	intelligence-operative	db-changelog-1.72.xml	2026-02-09 02:52:09.70743	635	EXECUTED	9:b58ccbba139421be117e3695e1d1caef	sql	Insert minimal test data for party transition views.\n            \n            Test Scenario: 2 politicians who switched parties while serving\n            - Person 1: Social Democrats (S) → Moderates (M) in 2011\n            - Person 2: Left Party (...	\N	5.0.1	\N	\N	0601926302
fix-view-riksdagen-voting-anomaly-detection-1.73-001	intelligence-operative	db-changelog-1.73.xml	2026-02-09 02:52:09.719637	636	EXECUTED	9:acceb76ea168f97217d66bdb8fb5e82c	sql	Fix view_riksdagen_voting_anomaly_detection - Remove hardcoded 3-year date filter\n            \n            BUG: Hardcoded filter (CURRENT_DATE - '3 years'::interval) excludes all historical data\n            FIX: Change to 20-year lookback to captu...	\N	5.0.1	\N	\N	0601926302
fix-view-riksdagen-party-transition-history-1.73-002	intelligence-operative	db-changelog-1.73.xml	2026-02-09 02:52:09.731835	637	EXECUTED	9:a0f99d56572cfc51346514d44a86ed23	sql	Fix view_riksdagen_party_transition_history - Add deterministic ordering to window functions\n            \n            BUG: Window function ORDER BY from_date alone causes non-deterministic results when multiple\n            assignments have same fr...	\N	5.0.1	\N	\N	0601926302
fix-coalition-alignment-date-filter-1.74-001	intelligence-operative	db-changelog-1.74.xml	2026-02-11 23:36:34.06684	638	EXECUTED	9:2b6e2798390c5b4a4f097d229f2bb852	sql	Fix view_riksdagen_coalition_alignment_matrix - Extend date filter from 5 to 25 years\n            \n            ISSUE: View empty with full production database (2002-2026 data)\n            CAUSE: Filter (CURRENT_DATE - '5 years'::interval) excludes...	\N	5.0.1	\N	\N	0849392598
fix-decision-temporal-trends-date-filter-1.75-001	copilot	db-changelog-1.75.xml	2026-02-11 23:36:34.085982	639	EXECUTED	9:8b624ce155b434ed3ccf17efa4584f64	sql	Fix view_decision_temporal_trends date filter: extend from 5 to 25 years.\n            \n            Enables analysis of document decision trends with 24 years of historical data \n            (2002-2026) instead of limiting to 2021-2026.\n           ...	\N	5.0.1	\N	\N	0849392598
fix-committee-role-member-date-filter-1.75-002	copilot	db-changelog-1.75.xml	2026-02-11 23:36:34.108669	640	EXECUTED	9:6e66916660d3c44cc4480fd3d0f9adb3	sql	Fix view_riksdagen_committee_role_member date filter: extend from 5 to 25 years.\n            \n            Enables analysis of committee member document production with 24 years of \n            historical data instead of limiting to recent 5 years....	\N	5.0.1	\N	\N	0849392598
fix-crisis-resilience-date-filter-1.75-003	copilot	db-changelog-1.75.xml	2026-02-11 23:36:34.12787	641	EXECUTED	9:ae15425323b485f5da2a48935e3bac3b	sql	Fix view_riksdagen_crisis_resilience_indicators date filter: extend from 5 to 25 years.\n            \n            Enables analysis of politician behavior during crisis periods with 24 years of \n            historical voting data, capturing multiple...	\N	5.0.1	\N	\N	0849392598
fix-goverment-role-member-date-filter-1.75-004	copilot	db-changelog-1.75.xml	2026-02-11 23:36:34.139412	642	EXECUTED	9:f13d8db50c91b338765efa5a95ff94e9	sql	Fix view_riksdagen_goverment_role_member date filter: extend from 5 to 25 years.\n            \n            Enables analysis of government minister document production with 24 years of \n            historical data, capturing multiple government form...	\N	5.0.1	\N	\N	0849392598
fix-party-role-member-date-filter-1.75-005	copilot	db-changelog-1.75.xml	2026-02-11 23:36:34.15416	643	EXECUTED	9:981ccb37cd8df2d5cd77c5247d272782	sql	Fix view_riksdagen_party_role_member date filter: extend from 5 to 25 years.\n            \n            Enables analysis of party leadership document production with 24 years of \n            historical data across multiple party leaderships and coal...	\N	5.0.1	\N	\N	0849392598
create-mv-party-decision-flow-1.76-001	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.056168	644	EXECUTED	9:754be4ff4a843fead98f0cd92a67cfb5	sql	Create materialized view mv_party_decision_flow from view_riksdagen_party_decision_flow.\n            \n            This precomputes the expensive document proposal joins that cause timeout in\n            view_election_cycle_decision_intelligence. T...	\N	5.0.2	\N	\N	3266413407
create-mv-decision-temporal-trends-1.76-002	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.066551	645	EXECUTED	9:6dabd762b3f68405b43933f58fad0ede	sql	Create materialized view mv_decision_temporal_trends from view_decision_temporal_trends.\n            \n            Precomputes daily decision statistics with moving averages to avoid expensive\n            recomputation in view_election_cycle_decisi...	\N	5.0.2	\N	\N	3266413407
create-mv-ministry-decision-impact-1.76-003	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.072354	646	EXECUTED	9:062ba52c0a06b0e847dc62b64b92b5a8	sql	Create materialized view mv_ministry_decision_impact from view_ministry_decision_impact.\n            \n            Precomputes ministry-level decision impact metrics to avoid expensive\n            recomputation in view_election_cycle_decision_intel...	\N	5.0.2	\N	\N	3266413407
fix-election-cycle-decision-intelligence-1.76-004	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.090057	647	EXECUTED	9:66e1b63d562d8c3955bfe39e06451d1a	sql	Rewrite view_election_cycle_decision_intelligence to use materialized sub-views.\n            \n            BUG: View joins 3 complex regular views causing query timeout with production data.\n            FIX: Use precomputed materialized views (mv_p...	\N	5.0.2	\N	\N	3266413407
fix-coalition-evolution-1.76-005	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.12129	648	EXECUTED	9:77345349e75191c8859afbf3c90f3d72	sql	Fix view_riksdagen_party_coalition_evolution: use _monthly matview instead of _annual.\n            \n            BUG: The _annual materialized view truncates all dates to January 1 of each year.\n                 This causes two failures:\n          ...	\N	5.0.2	\N	\N	3266413407
fix-voting-anomaly-detection-1.76-006	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.136505	649	EXECUTED	9:f64d9ef277f5827a52937deceb32b387	sql	Fix view_riksdagen_voting_anomaly_detection: remove wrong person_data.status filter.\n            \n            BUG: The WHERE clause filters person_data.status with values like\n                 'Tjänstgörande riksdagsledamot' which are assignment_d...	\N	5.0.2	\N	\N	3266413407
fix-party-transition-history-1.76-007	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.152208	650	EXECUTED	9:64d242c7c32d84a81a1ecbe941c37ba1	sql	Fix view_riksdagen_party_transition_history: use vote_data for party detection.\n            \n            BUG: Previous approach filtered assignment_data with:\n                 - assignment_type='uppdrag' (committee assignments, not chamber members...	\N	5.0.2	\N	\N	3266413407
fix-party-defector-analysis-1.76-008	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.163296	651	EXECUTED	9:21a325860c251921771e360b4dcb2af8	sql	Fix view_riksdagen_party_defector_analysis: include all transition types.\n            \n            BUG: Previous version only included transition_type='SWITCHED_WHILE_SERVING'\n                 which required gap <= 30 days between assignments. Wit...	\N	5.0.2	\N	\N	3266413407
recreate-party-switcher-outcomes-1.76-009	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.173943	652	EXECUTED	9:80d17e12d6371de03c20da03039b5647	sql	Recreate view_riksdagen_party_switcher_outcomes after transition_history was\n            replaced. This view tracks outcomes for politicians who switched parties,\n            including subsequent assignments, leadership positions, and election cyc...	\N	5.0.2	\N	\N	3266413407
refresh-materialized-views-1.76-010	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.59312	653	EXECUTED	9:94b1021625edc022b67fc08003deeef8	sql	Refresh all new materialized views to populate them with data.\n            Uses CONCURRENTLY where possible to avoid blocking reads.\n            \n            Note: IF NOT EXISTS on CREATE MATERIALIZED VIEW means these may already\n            conta...	\N	5.0.2	\N	\N	3266413407
fix-politician-risk-summary-1.76-011	intelligence-operative	db-changelog-1.76.xml	2026-03-11 23:00:15.606852	654	EXECUTED	9:1ad61cd302134d753412c6632ca9c56f	sql	Fix view_politician_risk_summary: rebel_votes compared vote value (JA/NEJ/etc.)\n            to party code (S/M/etc.), inflating rebel rates for every non-absent vote.\n            \n            FIX: Compute rebel votes by comparing member's vote to ...	\N	5.0.2	\N	\N	3266413407
fix-party-transition-history-1.77-001	intelligence-operative	db-changelog-1.77.xml	2026-03-20 22:26:31.90019	655	EXECUTED	9:9ec48f512625c11dcef56e662f038965	sql	Fix view_riksdagen_party_transition_history: detect party transitions using\n            assignment_data with party org_codes instead of vote_data.\n\n            ROOT CAUSE ANALYSIS:\n            Previous approach (1.76-007) used vote_data.party LAG ...	\N	5.0.2	\N	\N	4041990070
fix-party-defector-analysis-1.77-002	intelligence-operative	db-changelog-1.77.xml	2026-03-20 22:26:31.914669	656	EXECUTED	9:62c36d3c0a45a7e1cd90b652e00ad7cb	sql	Recreate view_riksdagen_party_defector_analysis after transition_history\n            was rebuilt. Analyzes performance changes around party transitions using\n            vote_data for attendance metrics.\n\n            Includes all transition types ...	\N	5.0.2	\N	\N	4041990070
recreate-party-switcher-outcomes-1.77-003	intelligence-operative	db-changelog-1.77.xml	2026-03-20 22:26:31.931964	657	EXECUTED	9:0264c0975efbc66bde5b60f42caa2333	sql	Recreate view_riksdagen_party_switcher_outcomes after transition_history\n            was rebuilt. Tracks career outcomes for politicians who switched parties.\n\n            Matches all subsequent assignments after transition date and evaluates:\n   ...	\N	5.0.2	\N	\N	4041990070
fix-election-cycle-anomaly-pattern-timeout-1.77-004	intelligence-operative	db-changelog-1.77.xml	2026-03-20 22:26:31.948369	658	EXECUTED	9:1f04453c8c3f900daf82d20706ffeab0	sql	Fix view_election_cycle_anomaly_pattern timeout caused by Cartesian joins.\n\n            ROOT CAUSE: full_schema.sql contains the pre-1.65 view definition with\n            ON (1=1) Cartesian joins, but databasechangelog marks 1.65-010 as EXECUTED.\n...	\N	5.0.2	\N	\N	4041990070
fix-risk-score-evolution-upper-1.78-001	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:31.965099	659	EXECUTED	9:ab56bbe2e96438c1329cb61bcc32215c	sql	Fix view_risk_score_evolution: use UPPER() for all vote comparisons.\n            \n            ROOT CAUSE: vote_data.vote is stored as 'JA','NEJ','FRÅNVARANDE','AVSTÅR'\n            (uppercase) but the view compares to title-case 'Ja','Nej','Frånvar...	\N	5.0.2	\N	\N	4041990070
fix-politician-risk-summary-reapply-1.78-002	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:31.980668	660	EXECUTED	9:06ff440bb9a9f0e4d6573e6fd8dae673	sql	Re-apply 1.76-011 fix for view_politician_risk_summary (stale schema issue)\n            AND fix rule_group categories to match actual DRL rule_group values.\n            \n            STALE SCHEMA: full_schema.sql has pre-1.76 definition that compar...	\N	5.0.2	\N	\N	4041990070
fix-party-performance-metrics-1.78-003	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:32.001252	661	EXECUTED	9:9b100ea7af8e131cd188772dcba359e0	sql	Fix view_party_performance_metrics: wrong status and document type filters.\n            \n            BUGS:\n            1. party_member_counts: status = 'active'/'inactive' — actual values are\n               Swedish strings like 'Tjänstgörande riks...	\N	5.0.2	\N	\N	4041990070
fix-party-effectiveness-trends-1.78-004	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:32.01774	662	EXECUTED	9:bc24a92469cf467fb386621834b70040	sql	Fix view_party_effectiveness_trends:\n            1. rv.status = 'ACTIVE' → IN ('MINOR','MAJOR','CRITICAL')\n            2. document_type = 'Motion' → 'mot'\n            \n            Uses CREATE OR REPLACE since output columns unchanged.	\N	5.0.2	\N	\N	4041990070
recreate-election-cycle-comparative-1.78-005	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:32.114331	663	EXECUTED	9:de5eadea41de68d8b5b9c6de443229a2	sql	Recreate view_election_cycle_comparative_analysis after CASCADE from\n            party_performance_metrics fix. Uses original definition from full_schema.sql.	\N	5.0.2	\N	\N	4041990070
recreate-party-electoral-trends-1.78-006	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:32.198939	664	EXECUTED	9:da841f712904d654951c27decc53be55	sql	Recreate view_riksdagen_party_electoral_trends dropped by CASCADE.\n            Original from db-changelog-1.62.xml but referencing the now-fixed\n            party_performance_metrics view for active_members data.	\N	5.0.2	\N	\N	4041990070
recreate-party-longitudinal-perf-1.78-007	intelligence-operative	db-changelog-1.78.xml	2026-03-20 22:26:32.273166	665	EXECUTED	9:5c5badad5d44bfce9616af5018311826	sql	Recreate view_riksdagen_party_longitudinal_performance dropped by CASCADE.\n            Now references fixed party_performance_metrics (active_members populated).	\N	5.0.2	\N	\N	4041990070
1.79-002	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.597499	667	EXECUTED	9:c135d31c0bcbe0642790598a65c010f1	createView viewName=view_politician_behavioral_trends	Fix view_politician_behavioral_trends: rule_violation.status = 'ACTIVE'\n        changed to IN ('MINOR','MAJOR','CRITICAL') (ROOT CAUSE 8).\n\n        The Status enum in the Java model (Status.java) defines: OK, MINOR, MAJOR, CRITICAL.\n        'ACTIV...	\N	5.0.2	\N	\N	4213844732
1.79-003	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.614213	668	EXECUTED	9:d26359f6a83be2f3f144a77209c6e9f2	createView viewName=view_riksdagen_party_role_member	Fix view_riksdagen_party_role_member: replace document_type 'ip'/'frs'\n        counts with sub_type based motion classification (ROOT CAUSE 9).\n\n        Document types 'ip' (interpellation) and 'frs' (written question) do not exist\n        in docu...	\N	5.0.2	\N	\N	4213844732
1.79-004	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.632014	669	EXECUTED	9:9ed8dc1ca9d98f6dfdbe9eae59d98bc5	createView viewName=view_riksdagen_committee_role_member	Fix view_riksdagen_committee_role_member: replace document_type 'bet'/'yttr'\n        counts with sub_type based motion classification (ROOT CAUSE 9).\n\n        'bet' (committee reports) and 'yttr' (statements) have no person_reference linkage\n     ...	\N	5.0.2	\N	\N	4213844732
1.79-005	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.667095	670	EXECUTED	9:a494dbe8c9ce7296d08d2473b95055e8	sql	Fix mv_annual_voting_metrics: vote values stored UPPERCASE (JA, NEJ, FRÅNVARANDE, AVSTÅR)\n        but view compared with title-case ('Ja','Nej','Frånvarande','Avstår').\n        This caused avg_yes_rate, avg_no_rate, avg_abstain_rate to always be 0...	\N	5.0.2	\N	\N	4213844732
1.79-006	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.68432	671	EXECUTED	9:3b5969206f1b36c92a8154be76f259c7	sql	Recreate view_riksdagen_election_year_behavioral_patterns dropped by CASCADE\n        when fixing mv_annual_voting_metrics. View definition unchanged — reads from\n        corrected materialized view.	\N	5.0.2	\N	\N	4213844732
1.79-007	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.702698	672	EXECUTED	9:fdccb41b49b4bd9dc78e7725dd749c70	createView viewName=view_riksdagen_seasonal_quarterly_activity	Fix view_riksdagen_seasonal_quarterly_activity: vote comparison used\n        title-case 'Frånvarande' but data is UPPERCASE 'FRÅNVARANDE'.\n        This caused attendance_rate to always be ~100%, making q_stddev_attendance = 0\n        and attendanc...	\N	5.0.2	\N	\N	4213844732
1.79-008	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.72024	673	EXECUTED	9:5b2987486af6db7030e839280a8b20fc	createView viewName=view_committee_productivity	Fix view_committee_productivity: committee_documents CTE used\n        sub_type = 'mot'/'prop' but committee_document_data sub_types are: bet, (empty), utl, ap.\n        Also label LIKE '%bet%' should be sub_type = 'bet' for reports.\n        Changed...	\N	5.0.2	\N	\N	4213844732
1.79-009	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.73396	674	EXECUTED	9:63441aa7a079644ca7667d21469cc0fb	createView viewName=view_committee_productivity_matrix	Fix view_committee_productivity_matrix: document_type 'Utskottsbetänkande'\n        and 'Motion' don't exist. Actual types are lowercase: 'bet', 'mot'.\n        Changed to use correct lowercase document_type codes.	\N	5.0.2	\N	\N	4213844732
1.79-010	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.750688	675	EXECUTED	9:6e6217dd770f178d3da72369efd11b8f	createView viewName=view_ministry_effectiveness_trends	Fix view_ministry_effectiveness_trends: wrong document join\n        (dsc.hjid = dprc.hjid instead of dsc.document_person_reference_co_1 = dprc.hjid)\n        and non-existent document_type 'ds'. Only: mot, bet, prop, kammakt.\n        Replaced 'ds' ...	\N	5.0.2	\N	\N	4213844732
1.79-011	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.765613	676	EXECUTED	9:b30678ed9d73790e682142f9ddd442e3	createView viewName=view_ministry_risk_evolution	Fix view_ministry_risk_evolution: same wrong document join\n        and non-existent 'ds' type. Replaced with correct FK join and 'bet' type.	\N	5.0.2	\N	\N	4213844732
1.79-012	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.779165	677	EXECUTED	9:61617f87cb6d8d865bcd1b8312a1a53e	createView viewName=view_ministry_productivity_matrix	Fix view_ministry_productivity_matrix: same wrong document join\n        and non-existent 'ds' type. Replaced with correct FK join and 'bet' type.	\N	5.0.2	\N	\N	4213844732
1.79-013	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.799634	678	EXECUTED	9:52c0ec65310067e1ead16b999b5558b6	sql	Fix view_risk_score_evolution: document join still uses\n        dsc.hjid = dprc.hjid instead of correct FK dsc.document_person_reference_co_1.\n        The 1.78-001 fix corrected vote case sensitivity but not the document join,\n        causing docu...	\N	5.0.2	\N	\N	4213844732
1.79-014	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.817091	679	EXECUTED	9:ef9ff05cfc0ccfbc6c69ff46b2117d29	sql	Fix view_riksdagen_crisis_resilience_indicators: vote comparison used\n        title-case 'Frånvarande' but data is UPPERCASE 'FRÅNVARANDE'.\n        This caused crisis_absence_rate, crisis_party_discipline, normal_absence_rate\n        to always be ...	\N	5.0.2	\N	\N	4213844732
1.79-015	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.832613	680	EXECUTED	9:4f108d779c8afbf96a2af69e0d2675b0	sql	Fix view_riksdagen_party_momentum_analysis: vote comparisons used\n        title-case values but data is UPPERCASE. Applied UPPER() to all vote comparisons.\n        This caused participation_rate to always be 0.	\N	5.0.2	\N	\N	4213844732
1.79-016	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.855132	681	EXECUTED	9:8e23e26b236e2ea84e6524dbb12269ae	sql	Fix view_riksdagen_pre_election_quarterly_activity: role_tier case\n        sensitivity. Source view generates UPPERCASE (MINISTER, SPEAKER, PARTY_LEADER)\n        but this view compared with lowercase. This caused politicians_with_new_roles\n       ...	\N	5.0.2	\N	\N	4213844732
1.79-017	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.876966	682	EXECUTED	9:6b233177f7f530eac8502792dd754c93	sql	Fix view_riksdagen_election_proximity_trends: role_tier case\n        sensitivity. Source view generates UPPERCASE values but this view compared\n        with lowercase ('minister', 'speaker', 'party_leader', 'committee_chair',\n        'committee_me...	\N	5.0.2	\N	\N	4213844732
1.79-018	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.893909	683	EXECUTED	9:4e31348ee8dc68176df835c94c728f0c	sql	Fix view_election_cycle_network_analysis: replaced hardcoded\n        placeholder zeros for influential_politicians, avg_network_centrality,\n        and power_broker_count with actual calculated values derived from\n        vote participation data a...	\N	5.0.2	\N	\N	4213844732
1.79-019	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.906607	684	EXECUTED	9:0172721c03687e5e7d1f0ebc32a2af17	createView viewName=view_riksdagen_committee_role_member	Fix view_riksdagen_committee_role_member: initiatives count used\n        non-existent document type 'frs' (written questions). Only mot, bet, prop,\n        kammakt exist in document_data. Replace 'frs' with 'kammakt'.	\N	5.0.2	\N	\N	4213844732
1.79-020	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.928102	685	EXECUTED	9:0c0d1c51a4a204ee0c2dcf46dbe94554	sql	Fix view_riksdagen_party_electoral_trends: seat_count_proxy and\n        documents_produced came from non-temporal view_party_performance_metrics,\n        making them constant per party. All LAG-based change columns were 0.\n        Fix: derive seat...	\N	5.0.2	\N	\N	4213844732
1.79-021	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.946468	686	EXECUTED	9:8607a499b8465771311a7c49dd6d5f39	sql	Fix view_riksdagen_party_longitudinal_performance: active_members\n        and documents_last_year came from non-temporal view_party_performance_metrics,\n        making LAG-based change columns (membership_change, documents_change,\n        discipli...	\N	5.0.2	\N	\N	4213844732
1.79-023	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.977801	688	EXECUTED	9:35b9250dd2205b34e4041bb6b97a67d9	sql	Fix view_election_cycle_comparative_analysis: used CROSS JOIN to\n        non-temporal view_party_performance_metrics making active_members and\n        documents_last_year constant per party, so LAG-based change_performance_pct\n        and change_d...	\N	5.0.2	\N	\N	4213844732
1.79-024	copilot	db-changelog-1.79.xml	2026-03-22 22:10:46.992742	689	EXECUTED	9:ab1d95267aeb7ae873582a6f86c7d466	createView viewName=view_committee_productivity	Fix view_committee_productivity: column alias mismatch where\n        committee_document_data sub_type='bet' reports were aliased as motions_count,\n        'utl' statements as propositions_count, and other_documents as reports_count.\n        The JP...	\N	5.0.2	\N	\N	4213844732
1.79-025	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.004397	690	EXECUTED	9:d26359f6a83be2f3f144a77209c6e9f2	createView viewName=view_riksdagen_party_role_member	Fix view_riksdagen_party_role_member: total_interpellations actually\n        contains party_motions and total_written_questions contains committee_motions.\n        Since interpellations/written questions don't exist in document_data, keep the\n    ...	\N	5.0.2	\N	\N	4213844732
1.79-026	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.017053	691	EXECUTED	9:0172721c03687e5e7d1f0ebc32a2af17	createView viewName=view_riksdagen_committee_role_member	Fix view_riksdagen_committee_role_member: total_committee_reports\n        actually contains committee_motions count and total_statements contains\n        propositions count. Keep JPA column names but document actual data.	\N	5.0.2	\N	\N	4213844732
1.79-027	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.034916	692	EXECUTED	9:89b748b3f6ca202bbf7c57e59cd0537a	sql	Fix view_riksdagen_party_electoral_trends: add missing columns\n        that the JPA entity ViewRiksdagenPartyElectoralTrends expects:\n        rank_by_engagement, rank_by_effectiveness, quartile_by_performance,\n        electoral_trend, party_size_c...	\N	5.0.2	\N	\N	4213844732
1.79-028	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.057264	693	EXECUTED	9:e45748fc4a172035db2a1a1fd12a3397	sql	Fix view_riksdagen_party_longitudinal_performance: add missing\n        columns that the JPA entity expects: rank_by_win_rate, rank_by_participation,\n        percentile_win_rate, percentile_participation, percentile_approval,\n        quartile_by_wi...	\N	5.0.2	\N	\N	4213844732
1.79-029	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.066982	694	EXECUTED	9:70da82766e0f3c341193eb6db9fb6d35	sql	Recreate missing unique index on mv_annual_voting_metrics.year\n        that was lost during view recreation in prior changesets.	\N	5.0.2	\N	\N	4213844732
1.79-030	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.090667	695	EXECUTED	9:6bf8eaf81c052d6f8dd3aa7f787e8950	sql	Fix view_riksdagen_party_electoral_trends: JPA type mismatches\n        identified in second review round. seat_change_absolute must be bigint (JPA Long),\n        seat_forecast must be text (JPA String), performance_forecast must be text (JPA Strin...	\N	5.0.2	\N	\N	4213844732
1.79-031	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.110602	696	EXECUTED	9:f4371e4fdb9198e21e705516f3933d3a	sql	Fix view_riksdagen_party_longitudinal_performance: JPA type mismatches.\n        membership_change must be bigint (JPA Long), trend_position must be text (JPA String).\n        These were previously typed as integer/numeric which breaks Hibernate va...	\N	5.0.2	\N	\N	4213844732
1.79-032	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.127151	697	EXECUTED	9:a606f671efdfddcbef1286780f440556	sql; createView viewName=view_election_cycle_comparative_analysis	Fix view_election_cycle_comparative_analysis: document_z_score used\n        election_stddev_docs even for midterm years, making midterm z-scores inconsistent.\n        Added midterm_stddev_docs to election_baseline and use it for the non-election-y...	\N	5.0.2	\N	\N	4213844732
1.79-033	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.141016	698	EXECUTED	9:5dadb38bfa1a0889aaec70c5c620cf92	sql	Fix view_riksdagen_crisis_resilience_indicators: crisis_rebellions and\n        normal_rebellions compared vote value (JA/NEJ) to party code (S/M/SD), which\n        always evaluates to true for non-abstain votes — making the metric meaningless.\n   ...	\N	5.0.2	\N	\N	4213844732
1.79-034	copilot	db-changelog-1.79.xml	2026-03-22 22:10:47.154098	699	EXECUTED	9:61a846f69c461e3afcf453554cb0b3cd	sql	Fix view_riksdagen_election_year_behavioral_patterns: midterm document\n        z-score used election_stddev_docs instead of midterm_stddev_docs, making midterm\n        z-scores inconsistent. Added midterm_stddev_docs to election_baseline CTE and\n ...	\N	5.0.2	\N	\N	4213844732
1.80-001	copilot	db-changelog-1.80.xml	2026-04-11 14:04:49.371205	700	EXECUTED	9:7f70e5f040c22a2337a2da3737dd08da	createView viewName=view_riksdagen_party_summary	Fix view_riksdagen_party_summary: eliminate fan-out JOIN between\n        view_riksdagen_politician and party_documents (ROOT CAUSE 10).\n\n        The previous query joined each politician row with all documents for that\n        party (LEFT JOIN par...	\N	5.0.2	\N	\N	5909087554
\.
...
\unrestrict L13uxewN80xjdACn5dbJvgLefssuk1zGEawwJOdmmwSZzqhW0guUfSUgQ7aDuF8
