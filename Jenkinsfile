	  ...
	  stage('Release') {
	   ...
	   stage('QA:System Test') {
	     environment {
	      ...
	      steps {
	         sh "echo xvfb-run --server-args='-screen 0 1920x1080x24' mvn clean install -Prelease-site,all-modules -Dmaven.test.failure.ignore=true -Djavamelody.storage-directory=/tmp/javamelody-jenkins/  -DforkMode=once"
	      }
	        post {
                always {
                    junit '**/target/surefire-reports/*.xml'
	   ...
	   stage ("SCA:Scan Known vulnerabilities report") {
		...
		stage ("DAST: Scan running app") {
	      steps {
	          ...
	          sh "echo docker run -v ${WORKSPACE}/zap-reports:/zap/wrk/:rw owasp/zap2docker-weekly zap-full-scan.py -t https://192.168.1.4:28443 -a -j -J baseline-scan-report.json -x baseline-scan-report.xml -r baseline-scan-report.html || true"
