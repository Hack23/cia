	   ...
	   stage ("SCA:Scan Known vulnerabilities report") {
		...
		stage ("DAST: Scan running app") {
	      steps {
	          ...
	          sh "docker run -v ${WORKSPACE}/zap-reports:/zap/wrk/:rw owasp/zap2docker-weekly zap-baseline.py -t https://192.168.1.4:28443 -a -j -J baseline-scan-report.json -x baseline-scan-report.xml -r baseline-scan-report.html || true"
