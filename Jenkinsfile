	   ...
	   stage ("SCA:Scan Known vulnerabilities report") { 
		...
		stage ("DAST: Scan running app") {  
	      steps {
	          ...
	          sh "docker run -v ${pwd}:/zap/wrk/:rw owasp/zap2docker-weekly zap-baseline.py  -t https://192.168.1.12:28443  -J ${WORKSPACE}/baseline-scan-report.json report_json -x ${WORKSPACE}/baseline-scan-report.xml -r ${WORKSPACE}/baseline-scan-report.html || true"
		      }
		}
