package com.hack23.cia.web.impl.ui.application.views.user.party.pagemode;

/**
 * The Interface PartyViewConstants.
 */
public interface PartyViewConstants {

    // Overview
    String OVERVIEW_HEADER = "Party Overview";
    String OVERVIEW_TITLE = "Party Information"; 
    String OVERVIEW_DESC = "Explore detailed information about political parties";

    // Role Ghant
    String ROLE_GHANT_HEADER = "Role Timeline";
    String ROLE_GHANT_TITLE = "Party Role Chart";
    String ROLE_GHANT_DESC = "View party role assignments over time";

    // Member History
    String MEMBER_HISTORY_HEADER = "Member History";
    String MEMBER_HISTORY_TITLE = "Party Members";
    String MEMBER_HISTORY_DESC = "Explore the history of party members and their roles.";

    // Leader History
    String LEADER_HISTORY_HEADER = "Leader History";
    String LEADER_HISTORY_TITLE = "Party Leaders";
    String LEADER_HISTORY_DESC = "History of party leadership positions.";

    // Current Leaders
    String CURRENT_LEADERS_HEADER = "Current Leaders";
    String CURRENT_LEADERS_TITLE = "Party Leadership";
    String CURRENT_LEADERS_DESC = "Current party leadership and their roles.";

    // Government Roles
    String GOVERNMENT_ROLES_HEADER = "Government Roles";
    String GOVERNMENT_ROLES_TITLE = "Government Members";
    String GOVERNMENT_ROLES_DESC = "Explore the roles and members of the government within the party.";

    // Document Activity
    String DOCUMENT_ACTIVITY_HEADER = "Document Activity";
    String DOCUMENT_ACTIVITY_TITLE = "Recent Documents";
    String DOCUMENT_ACTIVITY_DESC = "Recent document activities of the party.";

    // Document History
    String DOCUMENT_HISTORY_HEADER = "Document History";
    String DOCUMENT_HISTORY_TITLE = "Party Documents";
    String DOCUMENT_HISTORY_DESC = "Historical record of party documents.";

    // Page Visit History
    String VISIT_HISTORY_HEADER = "Page Visit History";
    String VISIT_HISTORY_TITLE = "Visit History";
    String VISIT_HISTORY_DESC = "Review the history of page visits for the selected party.";

    // Daily Summary Chart
    String DAILY_SUMMARY_HEADER = "Party Won Daily Summary Chart";
    String DAILY_SUMMARY_TITLE = "Daily Summary";
    String DAILY_SUMMARY_DESC = "Analyze the daily summary of party wins over time.";

    // Party Support
    String SUPPORT_HEADER = "Party Support";
    String SUPPORT_TITLE = "Party Support Details";
    String SUPPORT_DESC = "Review the party support history for the selected party.";

    // Party Support Summary
    String SUPPORT_SUMMARY_HEADER = "Party Support Summary";
    String SUPPORT_SUMMARY_TITLE = "Support Analysis";
    String SUPPORT_SUMMARY_DESC = "Analysis of party support trends and patterns";

    // Vote History
    String VOTE_HISTORY_HEADER = "Vote History";
    String VOTE_HISTORY_TITLE = "Ballots";
    String VOTE_HISTORY_DESC = "Review the voting history for the selected party.";

    // Coalition Analysis
    String COALITIONS_AGAINST_HEADER = "Coalitions Against Party";
    String COALITIONS_AGAINST_TITLE = "Coalition Analysis";
    String COALITIONS_AGAINST_DESC = "Analysis of voting patterns against the party";

    // Committee Ballot Decisions
    String BALLOT_DECISIONS_HEADER = "Committee Ballot Decisions";
    String BALLOT_DECISIONS_TITLE = "Ballot Summary";
    String BALLOT_DECISIONS_DESC = "Summary of committee ballot decisions";

    // Committee Roles
    String COMMITTEE_ROLES_HEADER = "Committee Roles"; 
    String COMMITTEE_ROLES_TITLE = "Committee Assignments";
    String COMMITTEE_ROLES_DESC = "Party members' roles in committees";

    // Current Members
    String CURRENT_MEMBERS_HEADER = "Current Members";
    String CURRENT_MEMBERS_TITLE = "Party Members";
    String CURRENT_MEMBERS_DESC = "Discover the current members of the party and their roles.";

    // Grid Labels
    String GRID_LABEL_CURRENT_LEADERS = "Current Leaders";
    String GRID_LABEL_COMMITTEE_BALLOT_SUMMARY = "Committee Ballot Decision Party Summary"; 
    String GRID_LABEL_ROLE_CHART = "Role chart";
    String GRID_LABEL_DOCUMENT_HISTORY = "Member Document history";
    String GRID_LABEL_CURRENT_POLITICIANS = "Politicians";
    String GRID_LABEL_LEADER_HISTORY = "Leader History";
    String GRID_LABEL_GOVERNMENT_ROLES = "Government Roles";
    String GRID_LABEL_MEMBER_HISTORY = "Member History";
    String GRID_LABEL_VOTE_HISTORY = "Vote History";
    String GRID_LABEL_COMMITTEE_ROLES = "Committee Roles";

    // Additional Grid Labels
    String GRID_LABEL_SUPPORT_SUMMARY = "Support Summary";
    String GRID_LABEL_PARTY_WON = "Party Won Summary";
    String GRID_LABEL_AGAINST_COALITIONS = "Coalitions Against";

    // Party Won Daily Summary
    String DAILY_WON_HEADER = "Party Won Daily Summary";
    String DAILY_WON_TITLE = "Daily Won Summary";
    String DAILY_WON_DESC = "Daily summary of party voting wins";

    // Additional View Labels
    String DOCUMENT_ACTIVITY_SUBTITLE = "Recent Document Activities";
    String DOCUMENT_HISTORY_SUBTITLE = "Historical Documents";
    String ROLES_SUBTITLE = "Committee and Government Role Details";

    // View Subtitles 
    String VOTE_HISTORY_SUBTITLE = "Historical Voting Records";
    String CURRENT_MEMBER_SUBTITLE = "Active Party Members";
    String LEADER_HISTORY_SUBTITLE = "Historical Leadership";
    String COMMITTEE_BALLOT_SUBTITLE = "Committee Decisions";

    // Additional View Subtitles
    String VISIT_HISTORY_SUBTITLE = "Page Visit Analysis";
    String GENERAL_SUBTITLE = "Party Information";
    String ACTIVITY_SUBTITLE = "Activity Summary";
    
    // Chart Labels
    String CHART_SUPPORT_LABEL = "Party Support Trends";
    String CHART_VOTES_LABEL = "Voting Patterns";
    String CHART_BALLOTS_LABEL = "Ballot Summary";
}

